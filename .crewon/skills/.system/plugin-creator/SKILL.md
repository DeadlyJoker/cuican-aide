---
name: plugin-creator
description: Create and scaffold plugin directories for Crewon with a required `.crewon-plugin/plugin.json`, optional plugin folders/files, valid manifest defaults, and personal-marketplace entries by default. Use when Crewon needs to create a new personal plugin, add optional plugin structure, generate or update marketplace entries for plugin ordering and availability metadata, or update an existing local plugin during development with the cachebuster and reinstall flow.
---
...
2. Edit `<plugin-path>/.crewon-plugin/plugin.json` when the request gives specific metadata.
   The scaffold starts with valid defaults and must not contain `[TODO: ...]` placeholders.

3. Generate or update the personal marketplace entry when the plugin should appear in Crewon UI ordering:
...
- Always creates `/<parent-plugin-directory>/<plugin-name>/.crewon-plugin/plugin.json`.
...
- If the user specifies a different marketplace path, treat that marketplace as needing explicit configuration in the Crewon plugin manager.
...
- Treat plugin order in `plugins[]` as render order in Crewon. Append new entries unless a user explicitly asks to reorder the list.
...
- Do not remove required structure; keep `.crewon-plugin/plugin.json` present.
...
- If Crewon would need approval to write the marketplace file, ask for that approval before
...
- Do not tell the user to configure the default personal-marketplace
...
- If the user provided a non-default `--marketplace-path`, make sure that marketplace is installed
  before giving reinstall instructions. Tell the user to add that marketplace in the Crewon plugin
  manager when that explicit marketplace has not been configured yet.
- When the workflow created or updated a marketplace-backed plugin, end the final user-facing
  response with a short Crewon app handoff. Say `To view this in the Crewon app:` and write
  `View <normalized plugin name>` and `Share <normalized plugin name>` as Markdown links, not raw
  URLs or code spans.
- The View deeplink uses `crewon://plugins/<normalized plugin name>?marketplacePath=<absolute marketplace.json path>`.
...
- Do not add `pluginName` or `hostId` query parameters to these deeplinks. Crewon derives both after
