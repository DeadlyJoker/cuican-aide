...
These prompting principles apply to the built-in `image_gen` tool.
...
## Transparent images
- Use built-in `image_gen` for transparent-image requests.
...
- If the subject is too complex for clean chroma-key removal or local removal fails validation, explain the limitation and ask how the user wants to proceed.
