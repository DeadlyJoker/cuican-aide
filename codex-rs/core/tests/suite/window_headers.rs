...
async fn window_id_advances_after_compact_persists_on_resume_and_resets_on_fork() -> Result<()> {
    ...
    let initial_thread = Arc::clone(&initial.crewon);
    ...
    let resumed = builder
        ...
        .await?;
    submit_user_turn(&resumed.crewon, "after resume").await?;
    shutdown_thread(&resumed.crewon).await?;
...
async fn submit_user_turn(codex: &Arc<CrewonThread>, text: &str) -> Result<()> {
    codex
        .submit(Op::UserInput {
...
async fn submit_compact_turn(codex: &Arc<CrewonThread>) -> Result<()> {
...
async fn shutdown_thread(codex: &Arc<CrewonThread>) -> Result<()> {
