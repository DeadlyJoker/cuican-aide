...
async fn refresh_models_on_models_etag_mismatch_and_avoid_duplicate_models_fetch() -> Result<()> {
    ...
    // 1) On spawn, Crewon fetches /models and stores the ETag.
    ...
    let codex = Arc::clone(&test.crewon);
    ...
    // 2) If the server sends a different X-Models-Etag on /responses, Crewon refreshes /models.
    ...
    // Second /responses request (tool output) includes the same X-Models-Etag; Crewon should not
    ...
    let refresh_req = refresh_models_mock
        ...
        .next()
        .expect("one request");
    // Ensure Crewon includes client_version on refresh. (This is a stable signal that we're using the /models client.)
    assert!(
        refresh_req
            .url
            .query_pairs()
