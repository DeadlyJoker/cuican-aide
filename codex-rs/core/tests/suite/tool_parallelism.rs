...
async fn run_turn(test: &TestCrewon, prompt: &str) -> anyhow::Result<()> {
...
async fn run_turn_and_measure(test: &TestCrewon, prompt: &str) -> anyhow::Result<Duration> {
...
async fn build_crewon_with_test_tool(server: &wiremock::MockServer) -> anyhow::Result<TestCrewon> {
    let mut builder = test_crewon().with_model("test-gpt-5.1-codex");
...
async fn read_file_tools_run_in_parallel() -> anyhow::Result<()> {
    ...
    let test = build_crewon_with_test_tool(&server).await?;
