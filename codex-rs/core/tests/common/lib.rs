...
pub use crewon_utils_absolute_path::test_support::PathBufExt;
pub use crewon_utils_absolute_path::test_support::PathExt;
...
pub mod fault_thread_store;
pub mod hooks;
pub mod process;
pub mod responses;
pub mod streaming_sse;
pub mod test_crewon;
pub mod test_crewon_exec;
pub mod tracing;
pub mod zsh_fork;
...
fn enable_deterministic_unified_exec_process_ids_for_tests() {
    crewon_core::test_support::set_thread_manager_test_mode(/*enabled*/ true);
    crewon_core::test_support::set_deterministic_process_ids(/*enabled*/ true);
}

#[ctor]
fn configure_arg0_dispatch_for_test_binaries() {
    let _ = TEST_ARG0_PATH_ENTRY.get_or_init(crewon_arg0::arg0_dispatch);
}
...
fn configure_insta_workspace_root_for_snapshot_tests() {
    ...
    let workspace_root = crewon_utils_cargo_bin::repo_root()
        .ok()
...
fn default_test_overrides() -> ConfigOverrides {
    ConfigOverrides {
        crewon_linux_sandbox_exe: Some(
            find_crewon_linux_sandbox_exe().expect("should find binary for crewon-linux-sandbox"),
        ),
...
#[cfg(target_os = "linux")]
pub fn find_crewon_linux_sandbox_exe() -> Result<PathBuf, CargoBinError> {
    ...
    crewon_utils_cargo_bin::cargo_bin("crewon-linux-sandbox")
}

pub async fn wait_for_event<F>(
    crewon: &CrewonThread,
    predicate: F,
) -> crewon_protocol::protocol::EventMsg
where
    F: FnMut(&crewon_protocol::protocol::EventMsg) -> bool,
{
    use tokio::time::Duration;
    wait_for_event_with_timeout(crewon, predicate, Duration::from_secs(1)).await
}

/// Waits for a configured MCP server to finish startup and requires it to be ready.
pub async fn wait_for_mcp_server(crewon: &CrewonThread, server_name: &str) -> anyhow::Result<()> {
...
pub async fn submit_thread_settings(
    ...
    let submission_id = crewon
        .submit(Op::ThreadSettings { thread_settings })
        .await?;
    loop {
        let ev = timeout(Duration::from_secs(10), crewon.next_event())
            .await
...
pub async fn wait_for_event_match<T, F>(crewon: &CrewonThread, matcher: F) -> T
where
    F: Fn(&crewon_protocol::protocol::EventMsg) -> Option<T>,
{
    let ev = wait_for_event(crewon, |ev| matcher(ev).is_some()).await;
    ...
    F: FnMut(&crewon_protocol::protocol::EventMsg) -> bool,
{
    ...
    loop {
        ...
        let ev = timeout(wait_time.max(Duration::from_secs(10)), crewon.next_event())
            .await
...
pub fn sandbox_env_var() -> &'static str {
    crewon_core::spawn::CODEX_SANDBOX_ENV_VAR
}

pub fn sandbox_network_env_var() -> &'static str {
    crewon_core::spawn::CODEX_SANDBOX_NETWORK_DISABLED_ENV_VAR
}

const REMOTE_ENV_ENV_VAR: &str = "CREWON_TEST_REMOTE_ENV";
...
pub fn get_remote_test_env() -> Option<RemoteEnvConfig> {
    let Ok(container_name) = std::env::var(REMOTE_ENV_ENV_VAR) else {
...
pub fn format_with_current_shell(command: &str) -> Vec<String> {
    crewon_core::shell::default_user_shell()
...
pub fn stdio_server_bin() -> Result<String, CargoBinError> {
    crewon_utils_cargo_bin::cargo_bin("test_stdio_server").map(|p| p.to_string_lossy().to_string())
}
...
macro_rules! skip_if_no_network {
    () => {{
        if ::std::env::var($crate::sandbox_network_env_var()).is_ok() {
            println!(
                "Skipping test because it cannot execute when network is disabled in a Crewon sandbox."
            );
            return;
        }
    }};
...
#[macro_export]
macro_rules! crewon_linux_sandbox_exe_or_skip {
    () => {{
        #[cfg(target_os = "linux")]
        {
            match $crate::find_crewon_linux_sandbox_exe() {
                Ok(path) => Some(path),
                Err(err) => {
                    eprintln!("crewon-linux-sandbox binary not available, skipping test: {err}");
                    return;
                }
            }
        }
