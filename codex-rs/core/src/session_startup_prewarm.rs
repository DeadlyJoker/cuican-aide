...
use crewon_otel::STARTUP_PREWARM_AGE_AT_FIRST_TURN_METRIC;
use crewon_otel::STARTUP_PREWARM_DURATION_METRIC;
...
pub(crate) struct SessionStartupPrewarmHandle {
    task: AbortOnDropHandle<CrewonResult<ModelClientSession>>,
    started_at: Instant,
    timeout: Duration,
}
...
impl SessionStartupPrewarmHandle {
    pub(crate) fn new(
        task: JoinHandle<CrewonResult<ModelClientSession>>,
        started_at: Instant,
        timeout: Duration,
    ) -> Self {
        Self {
    ...
    fn resolution_from_join_result(
        result: std::result::Result<CrewonResult<ModelClientSession>, tokio::task::JoinError>,
        started_at: Instant,
    ) -> SessionStartupPrewarmResolution {
        match result {
...
) -> CrewonResult<ModelClientSession> {
