...
/// Start an interactive sub-Crewon thread and return IO channels.
///
/// The returned `events_rx` yields non-approval events emitted by the sub-agent.
/// Approval requests are handled via `parent_session` and are not surfaced.
/// The returned `ops_tx` allows the caller to submit additional `Op`s to the sub-agent.
#[allow(clippy::too_many_arguments)]
pub(crate) async fn run_crewon_thread_interactive(
    config: Config,
    ...
    let CrewonSpawnOk { engine, .. } = Box::pin(Crewon::spawn(CrewonSpawnArgs {
        config,
    ...
    .await??;
    let thread_config = engine.thread_config_snapshot().await;
    ...
    let engine_for_ops = Arc::clone(&engine);
    tokio::spawn(async move {
        forward_ops(engine_for_ops, rx_ops, cancel_token_ops).await;
    });

    Ok(Crewon {
        tx_sub: tx_ops,
        rx_event: rx_sub,
        agent_status: engine.agent_status.clone(),
        session: Arc::clone(&engine.session),
        session_loop_termination: engine.session_loop_termination.clone(),
    })
}
...
#[allow(clippy::too_many_arguments)]
pub(crate) async fn run_crewon_thread_one_shot(
    config: Config,
    auth_manager: Arc<AuthManager>,
    models_manager: SharedModelsManager,
    input: Vec<UserInput>,
    ...
    let io = Box::pin(run_crewon_thread_interactive(
        config,
        auth_manager,
        models_manager,
        parent_session,
...
async fn forward_events(
    ...
    loop {
        tokio::select! {
            _ = &mut cancelled => {
                shutdown_delegate(&engine).await;
                break;
            }
            event = engine.next_event() => {
                ...
                match event {
                    ...
                    other => {
                        if !forward_event_or_shutdown(&engine, &tx_sub, &cancel_token, other).await
                        {
                            break;
                        }
                    }
...
async fn shutdown_delegate(engine: &Crewon) {
    let _ = engine.submit(Op::Interrupt).await;
    let _ = engine.submit(Op::Shutdown {}).await;
    ...
    let guardian_decision = if routes_approval_to_guardian(parent_ctx) {
        ...
        let patch = changes
            ...
            .map(|(path, change)| match change {
                crewon_protocol::protocol::FileChange::Add { content } => {
                    format!("*** Add File: {}\n{}", path.display(), content)
                }
                crewon_protocol::protocol::FileChange::Delete { content } => {
                    format!("*** Delete File: {}\n{}", path.display(), content)
                }
                crewon_protocol::protocol::FileChange::Update {
                    unified_diff,
                    move_path,
                } => {
...
async fn handle_request_user_input(
    ...
    .await
    {
        let _ = engine.submit(Op::UserInputAnswer { id, response }).await;
        return;
    }
...
async fn maybe_auto_review_mcp_request_user_input(
    ...
    Some(RequestUserInputResponse {
        answers: HashMap::from([(
            question.id.clone(),
            crewon_protocol::request_user_input::RequestUserInputAnswer {
...
) -> crewon_protocol::protocol::ReviewDecision
where
    F: core::future::Future<Output = crewon_protocol::protocol::ReviewDecision>,
{
    tokio::select! {
        biased;
        _ = cancel_token.cancelled() => {
            ...
            parent_session
                .notify_approval(approval_id, crewon_protocol::protocol::ReviewDecision::Abort)
                .await;
            crewon_protocol::protocol::ReviewDecision::Abort
        }
        decision = fut => {
            decision
        }
