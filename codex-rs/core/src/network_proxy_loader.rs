...
use crewon_config::permissions_toml::overlay_network_domain_permissions;
...
use crewon_network_proxy::NetworkProxyConstraintError;
...
async fn build_config_state_with_mtimes() -> Result<(ConfigState, Vec<LayerMtime>)> {
    let codex_home = find_crewon_home().context("failed to resolve CREWON_HOME")?;
    let config_overrides = Vec::new();
...
fn upsert_network_domain(
    config: &mut NetworkProxyConfig,
    host: String,
    permission: crewon_network_proxy::NetworkDomainPermission,
) {
    config
        .network
