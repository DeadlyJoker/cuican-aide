...
const SANDBOX_STATE_META_CAPABILITY: &str = "crewon/sandbox-state-meta";
...
impl TestToolServer {
    ...
    /// Tool intended for manual testing of Crewon rendering for MCP image tool results.
    ///
    ...
    ///
    /// Manual testing approach (Crewon):
    /// - Build this binary: `cargo build -p crewon-rmcp-client --bin test_stdio_server`
    /// - Register it in config.toml as an MCP server named `mcpimg`.
    /// - Then in Crewon, ask it to call:
...
/// Scenarios for `image_scenario`, intended to exercise Crewon handling of MCP image outputs.
///
/// The key behavior under test is that the client should render an image output cell if *any*
