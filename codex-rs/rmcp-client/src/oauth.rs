...
//!
//! If the keyring is not available or fails, we fall back to the Crewon home `.credentials.json` file.
...
const KEYRING_SERVICE: &str = "Crewon MCP Credentials";
...
fn fallback_file_path() -> Result<PathBuf> {
    Ok(find_crewon_home()?.join(FALLBACK_FILENAME).to_path_buf())
}
...
mod tests {
    ...
    impl TempCrewonHome {
        fn new() -> Self {
            ...
            let dir = tempdir().expect("create CREWON_HOME temp dir");
            unsafe {
                std::env::set_var("CREWON_HOME", dir.path());
            }
            Self {
                _guard: guard,
                _dir: dir,
