//! Defines the protocol for a Crewon session between a client and an agent.
//!
...
pub const USER_MESSAGE_BEGIN: &str = "## My request for Crewon:";
...
pub enum Op {
    ...
    /// Request Crewon to drop the last N user turns from in-memory context.
    ///
...
/// running the command proposed by Crewon.
#[derive(
    Debug,
    Clone,
    Copy,
...
/// Crewon errors that we expose to clients.
...
impl fmt::Display for SessionSource {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            SessionSource::LegacyCli => f.write_str("cli"),
...
impl SessionSource {
    ...
    pub fn restriction_product(&self) -> Option<Product> {
        match self {
            ...
            | SessionSource::Exec
            | SessionSource::Mcp
            | SessionSource::Unknown => Some(Product::Crewon),
...
pub struct SessionMeta {
    ...
    #[serde(rename = "client_version", alias = "cli_version")]
    #[ts(rename = "client_version")]
...
pub struct TurnContextItem {
    ...
    // Compatibility-only field written with a default value so older Crewon
    ...
    pub fn token_budget(&self) -> usize {
        match self {
            TruncationPolicy::Bytes(bytes) => {
                usize::try_from(crewon_utils_string::approx_tokens_from_byte_count(*bytes))
                    .unwrap_or(usize::MAX)
            }
    ...
    pub fn byte_budget(&self) -> usize {
        match self {
            ...
            TruncationPolicy::Tokens(tokens) => {
                crewon_utils_string::approx_bytes_for_tokens(*tokens)
            }
        }
    }
}
...
pub enum Product {
    #[serde(alias = "CHATGPT")]
    Chatgpt,
    #[serde(alias = "CODEX", alias = "CREWON", alias = "codex")]
    Crewon,
    #[serde(alias = "ATLAS")]
    Atlas,
}
...
mod tests {
    ...
    #[test]
    fn session_meta_serializes_client_version_and_accepts_legacy_cli_version() {
        let mut meta = SessionMeta {
            id: ThreadId::default(),
            timestamp: "2026-06-14T00:00:00Z".to_string(),
            cwd: PathBuf::from("/workspace"),
            originator: "crewon-web".to_string(),
            client_version: "1.2.3".to_string(),
            ..Default::default()
        };

        let serialized = serde_json::to_value(&meta).expect("serialize session meta");
        assert_eq!(serialized.get("client_version"), Some(&json!("1.2.3")));
        assert_eq!(serialized.get("cli_version"), None);
        ...
        meta.client_version = "4.5.6".to_string();
    ...
    #[test]
    fn session_source_restriction_product_defaults_non_subagent_sources_to_crewon() {
        assert_eq!(
            SessionSource::LegacyCli.restriction_product(),
            Some(Product::Crewon)
        );
        assert_eq!(
    ...
    fn session_source_restriction_product_maps_custom_sources_to_products() {
        ...
        assert_eq!(
            SessionSource::Custom("crewon".to_string()).restriction_product(),
            Some(Product::Crewon)
        );
        assert_eq!(
    ...
    fn session_source_matches_product_restriction() {
        ...
        assert!(
            !SessionSource::Custom("chatgpt".to_string())
                .matches_product_restriction(&[Product::Crewon])
        );
        assert!(SessionSource::VSCode.matches_product_restriction(&[Product::Crewon]));
        assert!(
    ...
    fn restricted_file_system_policy_treats_root_with_carveouts_as_scoped_access() {
        ...
        let canonical_cwd =
            crewon_utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
