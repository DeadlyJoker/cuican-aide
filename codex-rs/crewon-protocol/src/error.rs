...
use crewon_utils_string::truncate_middle_with_token_budget;
...
pub enum CodexErr {
    ...
    #[error(
        "Crewon ran out of room in the model's context window. Start a new thread or clear earlier history before retrying."
    )]
    ...
    /// surface a polite FunctionCallOutput back to the model instead of crashing the client.
    ...
    QuotaExceeded,
    #[error(
        "To use Crewon with your ChatGPT plan, upgrade to Plus: https://chatgpt.com/explore/plus."
    )]
    UsageNotIncluded,
    ...
    #[error("crewon-linux-sandbox was required but not provided")]
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        let message = match self.plan_type.as_ref() {
            ...
            Some(PlanType::Known(KnownPlan::Free)) | Some(PlanType::Known(KnownPlan::Go)) => {
                format!(
                    "You've hit your usage limit. Upgrade to Plus to continue using Crewon (https://chatgpt.com/explore/plus),{}",
                    retry_suffix_after_or(self.resets_at.as_ref())
                )
            }
