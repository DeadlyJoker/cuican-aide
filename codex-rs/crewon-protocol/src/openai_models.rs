//! Shared model metadata types exchanged between Crewon services and clients.
//!
//! These types are serialized across core, app-server, and client boundaries, so field defaults
...
/// Metadata describing a Crewon-supported model.
#[derive(Debug, Clone, Deserialize, Serialize, TS, JsonSchema, PartialEq)]
...
/// Model metadata returned by the Crewon backend `/models` endpoint.
