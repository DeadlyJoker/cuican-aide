...
/// Initial collaboration mode to use when a client starts.
#[derive(
...
pub const CLIENT_VISIBLE_COLLABORATION_MODES: [ModeKind; 2] = [ModeKind::Default, ModeKind::Plan];
...
/// Collaboration mode for a Crewon session.
...
mod tests {
    ...
    #[test]
    fn client_visible_collaboration_modes_match_mode_kind_visibility() {
        let expected = [ModeKind::Default, ModeKind::Plan];
        assert_eq!(expected, CLIENT_VISIBLE_COLLABORATION_MODES);

        for mode in CLIENT_VISIBLE_COLLABORATION_MODES {
            assert!(mode.is_client_visible());
        }

        assert!(!ModeKind::PairProgramming.is_client_visible());
        assert!(!ModeKind::Execute.is_client_visible());
    }
