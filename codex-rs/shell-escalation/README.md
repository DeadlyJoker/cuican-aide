# crewon-shell-escalation

This crate contains the Unix shell-escalation protocol implementation and the
`crewon-execve-wrapper` executable.

`crewon-execve-wrapper` receives the arguments to an intercepted `execve(2)` call and delegates the
...
- `Run`: `crewon-execve-wrapper` should invoke `execve(2)` on itself to run the original command
...
- `Escalate`: forward the file descriptors of the current process so the command can be run
  ...
  back to `crewon-execve-wrapper`.
- `Deny`: the server has declared the proposed command to be forbidden, so
  `crewon-execve-wrapper` prints an error to `stderr` and exits with `1`.
