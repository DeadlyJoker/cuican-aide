# Bazel in the Crewon Rust Workspace
...
- `rules_rs` imports third-party crates from `codex-rs/Cargo.toml` and
  ...
  `@crates`.
- `../defs.bzl` provides `crewon_rust_crate`, which wraps `rust_library`,
...
- Each crate in `codex-rs/*/BUILD.bazel` typically uses `crewon_rust_crate` and
...
Crewon uses BuildBuddy for a shared Bazel cache and remoted builds and tests. To use it
...
2. Create a `BUILD.bazel` that calls `crewon_rust_crate` (see nearby crates for
   examples).
3. If a dependency needs special handling (compile/runtime data, additional binaries
   ...
   `crewon_rust_crate` to configure it.
