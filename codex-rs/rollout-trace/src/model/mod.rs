...
/// Crewon conversation/session UUID.
...
/// Process/session ID returned by Crewon's terminal runtime.
...
/// Canonical reduced graph for one Crewon rollout.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub struct RolloutTrace {
    pub schema_version: u32,
    /// Unique identity for this trace capture.
    ///
    /// `rollout_id` names the Crewon rollout/session being observed. `trace_id`
    ...
    pub trace_id: String,
    /// Client-visible rollout/run identity. Higher-level experiment/sample IDs wrap this object.
