...
fn disabled_thread_context_accepts_trace_calls_without_writing() -> anyhow::Result<()> {
    ...
    thread_trace.record_crewon_turn_event("turn-1", &EventMsg::ShutdownComplete);
    ...
    let token_usage: Option<crewon_protocol::protocol::TokenUsage> = None;
...
#[test]
fn rollout_trace_root_env_prefers_crewon_and_falls_back_to_legacy_codex() {
    assert_eq!(
        rollout_trace_root_from_env_values(
            Some(OsString::from("/tmp/crewon-traces")),
            Some(OsString::from("/tmp/codex-traces")),
        ),
        Some(OsString::from("/tmp/crewon-traces"))
    );
    assert_eq!(
        rollout_trace_root_from_env_values(None, Some(OsString::from("/tmp/codex-traces"))),
        Some(OsString::from("/tmp/codex-traces"))
    );
    assert_eq!(rollout_trace_root_from_env_values(None, None), None);
}
