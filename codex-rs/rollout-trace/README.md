...
> **Privacy:** Rollout tracing is not telemetry. Crewon does **not** upload or
> report these traces; it writes local bundles only when
> `CREWON_ROLLOUT_TRACE_ROOT` is set. `CODEX_ROLLOUT_TRACE_ROOT` remains as a
> legacy fallback. Those local bundles can contain prompts,
...
during a Crewon session. It records raw runtime evidence into a local bundle on
...
Hot-path Crewon code does not try to build the final graph while the session is
...
model-visible conversation plus the runtime objects that explain how Crewon got
there.
...
```mermaid
flowchart TD
    subgraph Runtime["crewon-core runtime"]
...
one from `CREWON_ROLLOUT_TRACE_ROOT`; fresh spawned child threads derive their
...
Crewon session fail just because diagnostic recording failed. Core emits raw
...
- `state.json`: optional reducer output written by the trace reducer.

`trace_id` identifies this diagnostic artifact. `rollout_id` identifies the
Crewon rollout/session being observed. Keeping those separate lets us reason about
the stored trace without confusing it with the product-level session identity.

To reduce a bundle from Rust:

Call `crewon_rollout_trace::replay_bundle` directly. The reducer writes
`<trace-bundle>/state.json` when configured to persist reduced state.
