//! Support for `-c key=value` overrides shared across Crewon tools.
//!
//! This module provides a [`ConfigOverrides`] struct that can be embedded
//! into a `clap`-derived options struct using `#[clap(flatten)]`. Each
//! occurrence of `-c key=value` (or `--config key=value`) will be collected as a raw
...
/// Option that captures arbitrary configuration overrides specified as
...
pub struct ConfigOverrides {
    ...
    /// `$CREWON_HOME/config.toml`. Use a dotted path (`foo.bar.baz`) to override
...
impl ConfigOverrides {
    ...
    /// Parse the raw strings captured from process options into a list of `(path,
...
mod tests {
    ...
    fn prepends_root_overrides() {
        let mut subcommand_overrides = ConfigOverrides {
            raw_overrides: vec![r#"model="gpt-5.2""#.to_string()],
        };
        subcommand_overrides.prepend_root_overrides(ConfigOverrides {
