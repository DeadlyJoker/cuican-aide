...
mod tests {
    ...
    fn render_replaces_placeholders_with_and_without_whitespace() {
        let rendered = render(
            ...
            [("name", "Crewon"), ("place", "crewon-rs")],
        )
        .unwrap();

        assert_eq!(
            rendered,
            "Hello, Crewon. You are in crewon-rs. Crewon is repeated."
        );
    }
    ...
    fn parsed_templates_can_be_reused() {
        ...
        assert_eq!(
            template.render([("greeting", "Hello"), ("name", "Crewon")]),
            Ok("Hello, Crewon!".to_string())
        );
        assert_eq!(
    ...
    fn render_supports_literal_delimiter_escapes() {
        ...
        assert_eq!(
            rendered,
            "literal open: {{, literal close: }}, value: Crewon"
        );
    }
    ...
    fn render_errors_when_extra_value_is_provided() {
        ...
        assert_eq!(
            template.render([("name", "Crewon"), ("unused", "extra")]),
    ...
    fn render_errors_when_duplicate_value_is_provided() {
        ...
        assert_eq!(
            template.render([("name", "Crewon"), ("name", "other")]),
    ...
    fn render_function_wraps_parse_errors() {
        let err = render("Hello, }} world.", [("name", "Crewon")]).unwrap_err();
    ...
    fn render_function_wraps_render_errors() {
        let err = render("Hello, {{ name }}.", [("extra", "Crewon")]).unwrap_err();
