...
pub(crate) struct PreToolUseCommandInput {
    ...
    /// Crewon extension: expose the active turn id to internal turn-scoped hooks.
    pub turn_id: String,
...
pub(crate) struct StopCommandOutputWire {
    ...
    #[serde(default)]
    pub decision: Option<BlockDecisionWire>,
    /// The compatibility contract requires `reason` when `decision` is `block`;
    /// we enforce that semantic rule during output parsing rather than in the JSON schema.
    #[serde(default)]
    pub reason: Option<String>,
}
...
mod tests {
    ...
    #[test]
    fn turn_scoped_hook_inputs_include_crewon_turn_id_extension() {
        // The generated schema intentionally exposes the active turn so
        // internal hook consumers can key off turn-scoped state.
