...
Runtime orchestration for Phase 1 and Phase 2 still lives in `crewon-core` under
the core memory modules.

## Crates

- `memories/read` (`crewon-memories-read`) owns the read path:
  memory developer-instruction injection, memory citation parsing, and
  read-usage telemetry classification.
- `memories/write` (`crewon-memories-write`) owns the write path:
...
- In this repo, edit those undated template files in place.
...
- keeps the memories root itself as a git-baseline directory, initialized under
  the configured Crewon home by `crewon-git-utils`
