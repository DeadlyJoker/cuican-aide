//! Aggregates MCP server connections for Crewon.
//!
...
use crate::crewon_apps::CrewonAppsToolsCacheKey;
...
use crewon_protocol::protocol::McpStartupCompleteEvent;
use crewon_protocol::protocol::McpStartupFailure;
use crewon_protocol::protocol::McpStartupStatus;
use crewon_protocol::protocol::McpStartupUpdateEvent;
...
pub struct McpConnectionManager {
    ...
    host_owned_crewon_apps_enabled: bool,
...
impl McpConnectionManager {
    ...
    pub async fn new(
        ...
        crewon_home: PathBuf,
        crewon_apps_tools_cache_key: CrewonAppsToolsCacheKey,
        host_owned_crewon_apps_enabled: bool,
        prefix_mcp_tool_names: bool,
        ...
        let crewon_apps_auth_provider = auth
            .filter(|auth| auth.uses_crewon_backend())
            .map(crewon_model_provider::auth_provider_from_auth);
            ...
            .filter(|(_, server)| server.enabled())
        {
            ...
            .await;
            let crewon_apps_tools_cache_context = if server_name == CREWON_APPS_MCP_SERVER_NAME {
                Some(CrewonAppsToolsCacheContext {
                    crewon_home: crewon_home.clone(),
                    user_key: crewon_apps_tools_cache_key.clone(),
                })
            } else {
                None
            };
            ...
            let runtime_auth_provider =
                if server_name == CREWON_APPS_MCP_SERVER_NAME && !uses_env_bearer_token {
                    crewon_apps_auth_provider.clone()
                } else {
                    None
                };
    ...
    pub fn is_host_owned_crewon_apps_server(&self, server_name: &str) -> bool {
        self.host_owned_crewon_apps_enabled && server_name == CREWON_APPS_MCP_SERVER_NAME
    }
    ...
    /// Force-refresh crewon apps tools by bypassing the in-process cache.
    ///
    ...
    pub async fn hard_refresh_crewon_apps_tools_cache(&self) -> Result<Vec<ToolInfo>> {
        let managed_client = self
            .clients
            .get(CREWON_APPS_MCP_SERVER_NAME)
            .ok_or_else(|| anyhow!("unknown MCP server '{CREWON_APPS_MCP_SERVER_NAME}'"))?
            .client()
            .await
        ...
        .await
        .with_context(|| {
            format!("failed to refresh tools for MCP server '{CREWON_APPS_MCP_SERVER_NAME}'")
        })?;
        emit_duration(
        ...
        write_cached_crewon_apps_tools_if_needed(
            CREWON_APPS_MCP_SERVER_NAME,
            managed_client.crewon_apps_tools_cache_context.as_ref(),
