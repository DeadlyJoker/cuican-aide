...
//! Session orchestration stays in `crewon-core`.

use crewon_protocol::mcp::CallToolResult;
use serde::Serialize;

pub const MCP_TOOL_CREWON_APPS_META_KEY: &str = "_crewon_apps";
pub const LEGACY_MCP_TOOL_CODEX_APPS_META_KEY: &str = "_codex_apps";
...
pub struct CrewonAppsConnectorAuthFailure {
...
pub struct CrewonAppsAuthElicitation {
...
#[derive(Debug, Clone, PartialEq)]
pub struct CrewonAppsAuthElicitationPlan {
    pub auth_failure: CrewonAppsConnectorAuthFailure,
    pub elicitation: CrewonAppsAuthElicitation,
}

#[derive(Serialize)]
struct CrewonAppsConnectorAuthFailureMeta<'a> {
...
pub fn connector_auth_failure_from_tool_result(
...
) -> Option<CrewonAppsConnectorAuthFailure> {
    ...
    let auth_failure = meta
        .get(MCP_TOOL_CREWON_APPS_META_KEY)
        .or_else(|| meta.get(LEGACY_MCP_TOOL_CODEX_APPS_META_KEY))?
        .as_object()?
    ...
    Some(CrewonAppsConnectorAuthFailure {
...
pub fn build_auth_elicitation_plan(
    ...
    connector_name: Option<&str>,
    install_url: Option<String>,
) -> Option<CrewonAppsAuthElicitationPlan> {
    let auth_failure =
    ...
    Some(CrewonAppsAuthElicitationPlan {
        auth_failure,
        elicitation,
    })
}
...
pub fn build_auth_elicitation(
    call_id: &str,
    auth_failure: &CrewonAppsConnectorAuthFailure,
) -> CrewonAppsAuthElicitation {
    CrewonAppsAuthElicitation {
        meta: serde_json::json!({
            MCP_TOOL_CREWON_APPS_META_KEY: {
                CONNECTOR_AUTH_FAILURE_META_KEY: CrewonAppsConnectorAuthFailureMeta {
...
pub fn auth_elicitation_id(call_id: &str) -> String {
    format!("crewon_apps_auth_{call_id}")
}
...
fn auth_elicitation_message(auth_failure: &CrewonAppsConnectorAuthFailure) -> String {
    match auth_failure.auth_reason.as_deref() {
        ...
        Some("missing_link") => format!(
            "Sign in to {} on ChatGPT to use it in Crewon.",
            auth_failure.connector_name
        ),
        _ => format!(
...
mod tests {
    ...
    fn builds_url_elicitation_payload() {
        ...
        assert_eq!(
            ...
            CrewonAppsAuthElicitation {
                ...
                elicitation_id: "crewon_apps_auth_call_123".to_string(),
            }
        );
    }
    ...
    fn builds_auth_elicitation_plan() {
        ...
        assert_eq!(plan.elicitation.elicitation_id, "crewon_apps_auth_call_123");
    }
}
