use super::*;
use crate::crewon_apps::CREWON_APPS_TOOLS_CACHE_SCHEMA_VERSION;
use crate::crewon_apps::CrewonAppsToolsCacheContext;
use crate::crewon_apps::load_startup_cached_crewon_apps_server_info;
use crate::crewon_apps::load_startup_cached_crewon_apps_tools_snapshot;
use crate::crewon_apps::read_cached_crewon_apps_tools;
use crate::crewon_apps::write_cached_crewon_apps_tools;
use crate::crewon_apps::write_cached_crewon_apps_tools_if_needed;
...
use crewon_protocol::protocol::McpAuthStatus;
...
fn create_crewon_apps_tools_cache_context(
    crewon_home: PathBuf,
    account_id: Option<&str>,
    chatgpt_user_id: Option<&str>,
) -> CrewonAppsToolsCacheContext {
    CrewonAppsToolsCacheContext {
        crewon_home,
        user_key: CrewonAppsToolsCacheKey {
...
fn tool_with_model_visible_input_schema_masks_file_params() {
    let mut tool = create_test_tool(CREWON_APPS_MCP_SERVER_NAME, "upload").tool;
...
#[test]
fn crewon_apps_tools_cache_is_overwritten_by_last_write() {
    let crewon_home = tempdir().expect("tempdir");
    let cache_context = create_crewon_apps_tools_cache_context(
        crewon_home.path().to_path_buf(),
        Some("account-one"),
        Some("user-one"),
    );
    let tools_gateway_1 = vec![create_test_tool(CREWON_APPS_MCP_SERVER_NAME, "one")];
    let tools_gateway_2 = vec![create_test_tool(CREWON_APPS_MCP_SERVER_NAME, "two")];

    write_cached_crewon_apps_tools(&cache_context, &tools_gateway_1);
    let cached_gateway_1 =
        read_cached_crewon_apps_tools(&cache_context).expect("cache entry exists for first write");
    assert_eq!(cached_gateway_1[0].callable_name, "one");

    write_cached_crewon_apps_tools(&cache_context, &tools_gateway_2);
    let cached_gateway_2 =
        read_cached_crewon_apps_tools(&cache_context).expect("cache entry exists for second write");
    assert_eq!(cached_gateway_2[0].callable_name, "two");
}

#[test]
fn crewon_apps_tools_cache_is_scoped_per_user() {
    let crewon_home = tempdir().expect("tempdir");
    let cache_context_user_1 = create_crewon_apps_tools_cache_context(
        crewon_home.path().to_path_buf(),
        Some("account-one"),
        Some("user-one"),
    );
    let cache_context_user_2 = create_crewon_apps_tools_cache_context(
        crewon_home.path().to_path_buf(),
        Some("account-two"),
        Some("user-two"),
    );
    let tools_user_1 = vec![create_test_tool(CREWON_APPS_MCP_SERVER_NAME, "one")];
    let tools_user_2 = vec![create_test_tool(CREWON_APPS_MCP_SERVER_NAME, "two")];

    write_cached_crewon_apps_tools(&cache_context_user_1, &tools_user_1);
    write_cached_crewon_apps_tools(&cache_context_user_2, &tools_user_2);

    let read_user_1 =
        read_cached_crewon_apps_tools(&cache_context_user_1).expect("cache entry for user one");
    let read_user_2 =
        read_cached_crewon_apps_tools(&cache_context_user_2).expect("cache entry for user two");
...
#[test]
fn crewon_apps_tools_cache_filters_disallowed_connectors() {
    let crewon_home = tempdir().expect("tempdir");
    let cache_context = create_crewon_apps_tools_cache_context(
        crewon_home.path().to_path_buf(),
        Some("account-one"),
    ...
    let cached =
        read_cached_crewon_apps_tools(&cache_context).expect("cache entry exists for user");
...
#[test]
fn crewon_apps_tools_cache_is_ignored_when_schema_version_mismatches() {
    let crewon_home = tempdir().expect("tempdir");
    let cache_context = create_crewon_apps_tools_cache_context(
        crewon_home.path().to_path_buf(),
        Some("account-one"),
    ...
    let bytes = serde_json::to_vec_pretty(&serde_json::json!({
        "schema_version": CREWON_APPS_TOOLS_CACHE_SCHEMA_VERSION + 1,
        "tools": [create_test_tool(CREWON_APPS_MCP_SERVER_NAME, "one")],
    }))
    .expect("serialize");
    std::fs::write(cache_path, bytes).expect("write");

    assert!(read_cached_crewon_apps_tools(&cache_context).is_none());
}

#[test]
fn crewon_apps_tools_cache_is_ignored_when_json_is_invalid() {
    let crewon_home = tempdir().expect("tempdir");
    let cache_context = create_crewon_apps_tools_cache_context(
        crewon_home.path().to_path_buf(),
        Some("account-one"),
...
#[test]
fn startup_cached_crewon_apps_tools_loads_from_disk_cache() {
    let crewon_home = tempdir().expect("tempdir");
    let cache_context = create_crewon_apps_tools_cache_context(
        crewon_home.path().to_path_buf(),
        Some("account-one"),
    ...
    let server_info = create_test_server_info("Crewon Apps");
    write_cached_crewon_apps_tools_if_needed(
        CREWON_APPS_MCP_SERVER_NAME,
        Some(&cache_context),
        &server_info,
        &cached_tools,
    ...
    let startup_tools = load_startup_cached_crewon_apps_tools_snapshot(
        CREWON_APPS_MCP_SERVER_NAME,
        Some(&cache_context),
    )
    .expect("expected startup snapshot to load from cache");
    let cached_server_info = load_startup_cached_crewon_apps_server_info(
        CREWON_APPS_MCP_SERVER_NAME,
        Some(&cache_context),
    );

    assert_eq!(startup_tools.len(), 1);
    assert_eq!(startup_tools[0].server_name, CREWON_APPS_MCP_SERVER_NAME);
    assert_eq!(startup_tools[0].callable_name, "calendar_search");
    assert_eq!(cached_server_info, Some(server_info));
}

#[test]
fn startup_cached_crewon_apps_tools_loads_without_server_info_cache() {
    let crewon_home = tempdir().expect("tempdir");
    let cache_context = create_crewon_apps_tools_cache_context(
        crewon_home.path().to_path_buf(),
        Some("account-one"),
    ...
    let bytes = serde_json::to_vec_pretty(&serde_json::json!({
        ...
        "tools": [create_test_tool(CREWON_APPS_MCP_SERVER_NAME, "calendar_search")],
    }))
    .expect("serialize");
    std::fs::write(cache_path, bytes).expect("write");
...
#[test]
fn crewon_apps_server_info_cache_survives_legacy_tools_cache_write() {
    let crewon_home = tempdir().expect("tempdir");
    let cache_context = create_crewon_apps_tools_cache_context(
        crewon_home.path().to_path_buf(),
        Some("account-one"),
    ...
    assert_eq!(
        load_startup_cached_crewon_apps_server_info(
            CREWON_APPS_MCP_SERVER_NAME,
            Some(&cache_context),
        ),
        Some(server_info)
    ...
    assert!(
        load_startup_cached_crewon_apps_tools_snapshot(
            CREWON_APPS_MCP_SERVER_NAME,
            Some(&cache_context),
        )
        .is_none()
...
async fn list_all_tools_uses_cached_tool_info_snapshot_while_client_is_pending() {
    ...
    let tool = tools
        ...
        .find(|tool| {
            tool.canonical_tool_name()
                == ToolName::namespaced("mcp__crewon_apps", "calendar_create_event")
        })
        .expect("tool from startup cache");
    assert_eq!(tool.server_name, CREWON_APPS_MCP_SERVER_NAME);
...
async fn list_available_server_infos_uses_cache_while_client_is_pending() {
    ...
    assert_eq!(
        server_infos.get(CREWON_APPS_MCP_SERVER_NAME),
        Some(&server_info)
    );
}
...
async fn no_local_runtime_fails_local_stdio_but_keeps_local_http_server() {
    ...
    let manager = McpConnectionManager::new(
        ...
        /*host_owned_crewon_apps_enabled*/ false,
        /*prefix_mcp_tool_names*/ true,
...
fn mcp_init_error_display_prompts_for_login_when_auth_required() {
    ...
    let expected = format!(
        "The {server_name} MCP server is not logged in. Authenticate it through client settings or update its config.toml credentials."
    );
