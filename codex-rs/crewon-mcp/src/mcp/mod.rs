...
use crewon_protocol::mcp::Resource;
use crewon_protocol::mcp::ResourceTemplate;
use crewon_protocol::mcp::Tool;
...
use crate::crewon_apps::crewon_apps_tools_cache_key;
use crate::runtime::McpRuntimeContext;
use crate::server::EffectiveMcpServer;

pub const CREWON_APPS_MCP_SERVER_NAME: &str = "crewon_apps";
pub const LEGACY_CODEX_APPS_MCP_SERVER_NAME: &str = "codex_apps";
const MCP_TOOL_NAME_PREFIX: &str = "mcp";
const MCP_TOOL_NAME_DELIMITER: &str = "__";
const CREWON_CONNECTORS_TOKEN_ENV_VAR: &str = "CREWON_CONNECTORS_TOKEN";
const LEGACY_CODEX_CONNECTORS_TOKEN_ENV_VAR: &str = "CODEX_CONNECTORS_TOKEN";
...
/// MCP runtime settings derived from `crewon_core::config::Config`.
///
/// This struct should contain only long-lived configuration values that the
/// `crewon-mcp` crate needs to construct server transports, enforce MCP
...
pub struct McpConfig {
    ...
    /// Crewon home directory used for MCP OAuth state and app-tool cache files.
    ...
    /// Optional path to `crewon-linux-sandbox` for sandboxed MCP tool execution.
...
pub fn host_owned_crewon_apps_enabled(config: &McpConfig, auth: Option<&CrewonAuth>) -> bool {
    config.apps_enabled && auth.is_some_and(CrewonAuth::uses_crewon_backend)
}
...
pub fn effective_mcp_servers_from_configured(
    ...
    let mut servers = configured_servers
        ...
        .collect::<HashMap<_, _>>();
    if !host_owned_crewon_apps_enabled(config, auth) {
        servers.remove(CREWON_APPS_MCP_SERVER_NAME);
        servers.remove(LEGACY_CODEX_APPS_MCP_SERVER_NAME);
    }
    servers
}
...
pub async fn read_mcp_resource(
    ...
    let host_owned_crewon_apps_enabled = host_owned_crewon_apps_enabled(config, auth);
...
fn crewon_apps_mcp_bearer_token_env_var() -> Option<String> {
    match env::var(CREWON_CONNECTORS_TOKEN_ENV_VAR) {
        Ok(value) if !value.trim().is_empty() => Some(CREWON_CONNECTORS_TOKEN_ENV_VAR.to_string()),
        Ok(_) => None,
        Err(env::VarError::NotPresent) => match env::var(LEGACY_CODEX_CONNECTORS_TOKEN_ENV_VAR) {
            Ok(value) if !value.trim().is_empty() => {
                Some(LEGACY_CODEX_CONNECTORS_TOKEN_ENV_VAR.to_string())
            }
        ...
        Err(env::VarError::NotUnicode(_)) => Some(CREWON_CONNECTORS_TOKEN_ENV_VAR.to_string()),
    }
}

fn normalize_crewon_apps_base_url(base_url: &str) -> String {
...
fn crewon_apps_mcp_url_for_base_url(base_url: &str) -> String {
    let base_url = normalize_crewon_apps_base_url(base_url);
...
pub fn crewon_apps_mcp_server_config(
    chatgpt_base_url: &str,
    apps_mcp_product_sku: Option<&str>,
) -> McpServerConfig {
    mcp_server_config_for_url(
        crewon_apps_mcp_url_for_base_url(chatgpt_base_url),
...
pub fn hosted_plugin_runtime_mcp_server_config(
    ...
    let base_url = normalize_crewon_apps_base_url(chatgpt_base_url);
...
fn mcp_server_config_for_url(url: String, apps_mcp_product_sku: Option<&str>) -> McpServerConfig {
    ...
    McpServerConfig {
        transport: McpServerTransportConfig::StreamableHttp {
            url,
            bearer_token_env_var: crewon_apps_mcp_bearer_token_env_var(),
            http_headers,
