...
fn test_mcp_config(crewon_home: PathBuf) -> McpConfig {
    McpConfig {
        chatgpt_base_url: "https://chatgpt.com".to_string(),
        apps_mcp_product_sku: None,
        codex_home: crewon_home,
...
#[test]
fn crewon_apps_mcp_url_for_base_url_keeps_existing_paths() {
    assert_eq!(
        crewon_apps_mcp_url_for_base_url("https://chatgpt.com/backend-api"),
        "https://chatgpt.com/backend-api/wham/apps"
    );
    assert_eq!(
        crewon_apps_mcp_url_for_base_url("https://chat.openai.com"),
        "https://chat.openai.com/backend-api/wham/apps"
    );
    assert_eq!(
        crewon_apps_mcp_url_for_base_url("http://localhost:8080/api/codex"),
        "http://localhost:8080/api/codex/apps"
    );
    assert_eq!(
        crewon_apps_mcp_url_for_base_url("http://localhost:8080"),
        "http://localhost:8080/api/codex/apps"
    );
}

#[test]
fn crewon_apps_server_config_uses_legacy_crewon_apps_path() {
    let config =
        crewon_apps_mcp_server_config("https://chatgpt.com", /*apps_mcp_product_sku*/ None);
    let url = match &config.transport {
        McpServerTransportConfig::StreamableHttp { url, .. } => url,
        _ => panic!("expected streamable http transport for crewon apps"),
    };
...
#[test]
fn crewon_apps_server_config_forwards_configured_product_sku_header() {
    let config = crewon_apps_mcp_server_config("https://chatgpt.com", Some("tpp"));
...
async fn effective_mcp_servers_preserve_runtime_servers() {
    ...
    let mut config = test_mcp_config(crewon_home.path().to_path_buf());
    ...
    let crewon_apps = effective
        .get(CREWON_APPS_MCP_SERVER_NAME)
        .expect("crewon apps server should exist");
    ...
    let docs = docs
        .configured_config()
        .expect("configured server should retain transport");
    let crewon_apps = crewon_apps
        .configured_config()
        .expect("crewon apps should use configured transport");
