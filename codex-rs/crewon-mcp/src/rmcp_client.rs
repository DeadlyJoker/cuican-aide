...
use crate::crewon_apps::CachedCrewonAppsToolsLoad;
use crate::crewon_apps::CrewonAppsToolsCacheContext;
use crate::crewon_apps::filter_disallowed_crewon_apps_tools;
use crate::crewon_apps::load_cached_crewon_apps_tools;
...
use crate::crewon_apps::normalize_crewon_apps_callable_name;
use crate::crewon_apps::normalize_crewon_apps_callable_namespace;
use crate::crewon_apps::normalize_crewon_apps_tool_title;
...
use crewon_exec_server::ReqwestHttpClient;
use crewon_protocol::mcp::McpServerInfo;
use crewon_protocol::protocol::Event;
use crewon_rmcp_client::ExecutorStdioServerLauncher;
use crewon_rmcp_client::LocalStdioServerLauncher;
use crewon_rmcp_client::RmcpClient;
use crewon_rmcp_client::StdioServerLauncher;
...
/// MCP server capability indicating that Crewon should include [`SandboxState`]
/// in tool-call request `_meta` under this key.
pub const MCP_SANDBOX_STATE_META_CAPABILITY: &str = "crewon/sandbox-state-meta";
pub const LEGACY_MCP_SANDBOX_STATE_META_CAPABILITY: &str = "codex/sandbox-state-meta";

pub(crate) const MCP_TOOLS_LIST_DURATION_METRIC: &str = "crewon.mcp.tools.list.duration_ms";
pub(crate) const MCP_TOOLS_FETCH_UNCACHED_DURATION_METRIC: &str =
    "crewon.mcp.tools.fetch_uncached.duration_ms";
...
pub(crate) struct ManagedClient {
    ...
    pub(crate) crewon_apps_tools_cache_context: Option<CrewonAppsToolsCacheContext>,
}

impl ManagedClient {
    fn listed_tools(&self) -> Vec<ToolInfo> {
        let total_start = Instant::now();
        if let Some(cache_context) = self.crewon_apps_tools_cache_context.as_ref()
            && let CachedCrewonAppsToolsLoad::Hit(tools) =
                load_cached_crewon_apps_tools(cache_context)
        {
            emit_duration(
        ...
        if self.crewon_apps_tools_cache_context.is_some() {
            emit_duration(
                MCP_TOOLS_LIST_DURATION_METRIC,
                total_start.elapsed(),
                &[("cache", "miss")],
...
impl AsyncManagedClient {
    ...
    pub(crate) fn new(
        ...
        crewon_apps_tools_cache_context: Option<CrewonAppsToolsCacheContext>,
        ...
        let tool_filter = server
            ...
            .unwrap_or_default();
        let cached_tool_info_snapshot = load_startup_cached_crewon_apps_tools_snapshot(
            &server_name,
            crewon_apps_tools_cache_context.as_ref(),
        );
            ...
            let outcome = match async {
                ...
                start_server_task(
                    ...
                    StartServerTaskParams {
                        ...
                        crewon_apps_tools_cache_context,
...
pub(crate) async fn list_tools_for_client_uncached(
    ...
    let tools = resp
        ...
        .map(|tool| {
            ...
            let callable_name = normalize_crewon_apps_callable_name(
                server_name,
                &tool_def.name,
            ...
            let callable_namespace =
                normalize_crewon_apps_callable_namespace(server_name, connector_name.as_deref());
            if let Some(title) = tool_def.title.as_deref() {
                let normalized_title =
                    normalize_crewon_apps_tool_title(server_name, connector_name.as_deref(), title);
    ...
    if server_name == CREWON_APPS_MCP_SERVER_NAME {
        return Ok(filter_disallowed_crewon_apps_tools(tools));
    }
    Ok(tools)
}
...
async fn start_server_task(
    ...
    let params = InitializeRequestParams::new(
        capabilities,
        Implementation::new("crewon-mcp-client", env!("CARGO_PKG_VERSION")).with_title("Crewon"),
    )
    ...
    let server_supports_sandbox_state_meta_capability = initialize_result
        ...
        .as_ref()
        .is_some_and(|exp| {
            exp.get(MCP_SANDBOX_STATE_META_CAPABILITY)
                .or_else(|| exp.get(LEGACY_MCP_SANDBOX_STATE_META_CAPABILITY))
                .is_some()
        });
...
mod tests {
    ...
    #[test]
    fn crewon_apps_connector_metadata_is_preserved() {
        let mut tool = tool_with_connector_meta();
