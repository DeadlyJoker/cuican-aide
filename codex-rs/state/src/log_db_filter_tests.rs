...
async fn sqlite_sink_drops_low_level_opentelemetry_sdk_logs() {
    let codex_home =
        std::env::temp_dir().join(format!("crewon-state-log-db-filter-{}", Uuid::new_v4()));
    ...
    tracing::trace!(target: "crewon_state", "retained-trace");
    ...
    assert_eq!(
        ...
        vec![
            ...
            ("TRACE", "crewon_state", Some("retained-trace")),
        ]
    );
