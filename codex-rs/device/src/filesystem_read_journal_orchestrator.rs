...
impl NativeFilesystemReadOrchestrator {
        ...
        Observer: FnOnce(&DeviceFilesystemReadEvent),
    {
        ...
        let command =
            parse_device_filesystem_read_command(serde_json::from_slice(command_frame).map_err(
                |_| NativeDeviceAdmissionError::new("device_filesystem_read_command_invalid"),
            )?)
            .map_err(NativeDeviceAdmissionError::from_protocol)?;
        let prepared = self
            .journal
            .prepare_filesystem_read_with_admission(&command, || {
                let now = (self.now)();
                let verified = connection.verify_filesystem_read_command(command_frame, now)?;
                let admitted =
                    connection.admit_filesystem_read_metadata(verified, now, registry)?;
                if admitted != command {
        ...
        let prepared = match prepared {
            ...
            Err(PrepareFilesystemReadWithAdmissionError::Journal(error)) => {
                return Err(from_journal(error));
            }
        };
        match prepared {
            PrepareFilesystemReadWithAdmissionOutcome::AcceptedReplay(execution) => {
                self.replay_or_recover(execution).await
            }
            PrepareFilesystemReadWithAdmissionOutcome::TerminalReplay(execution) => {
            ...
            PrepareFilesystemReadWithAdmissionOutcome::New {
                ...
                let terminal = match lease {
                    ...
                    Ok(lease) => match lease.read(cancellation) {
                        Ok(result)
                            if connection
                                .validate_filesystem_read_continuation(&command, (self.now)())
                                .is_ok() =>
                        {
                        ...
                        Err(error) => failed_terminal(
                            &execution,
                            &NativeDeviceAdmissionError::from_workspace(error),
                            (self.now)(),
                        )?,
                    },
                };
    ...
    ) -> Result<AcknowledgeFilesystemReadOutcome, NativeDeviceAdmissionError> {
        self.journal
    ...
    ) -> Result<NativeFilesystemReadReconnectPage, NativeDeviceAdmissionError> {
        let page = self
            .journal
            .list_unacknowledged_filesystem_reads(query)
            .await
            .map_err(from_journal)?;
        let items = page.executions.into_iter().map(project).collect();
        Ok(NativeFilesystemReadReconnectPage {
            items,
            next_cursor: page.next_cursor,
        })
    }

    async fn replay_or_recover(
        &self,
        ...
        if running()
            ...
            .contains(&execution.command.command.execution_id)
        {
            return Ok(NativeFilesystemReadDispatchOutcome::AcceptedInFlight {
        ...
        let resolved = record_terminal(&self.journal, &terminal).await?;
        Ok(
            NativeFilesystemReadDispatchOutcome::RecoveredUnknownOutcome {
                accepted: resolved.accepted,
                terminal: required_terminal(resolved.terminal)?,
            },
        )
...
async fn record_terminal(
    journal: &DeviceWorkspaceJournal,
    terminal: &DeviceFilesystemReadEvent,
) -> Result<FilesystemReadJournalExecution, NativeDeviceAdmissionError> {
    match journal
        .record_filesystem_read_terminal(terminal)
        .await
        .map_err(from_journal)?
    {
        RecordFilesystemReadTerminalOutcome::Committed(execution)
        | RecordFilesystemReadTerminalOutcome::Replayed(execution) => Ok(execution),
    }
}
...
fn project(execution: FilesystemReadJournalExecution) -> NativeFilesystemReadReconnectItem {
    ...
    NativeFilesystemReadReconnectItem {
        execution_id: execution.command.command.execution_id,
        events,
...
fn required_terminal(
    terminal: Option<DeviceFilesystemReadEvent>,
...
impl Running {
    fn reserve(execution_id: &str) -> Result<Self, NativeDeviceAdmissionError> {
        let mut guard = running().lock().map_err(|_| {
            NativeDeviceAdmissionError::new("device_workspace_single_flight_poisoned")
        })?;
        if !guard.insert(execution_id.to_string()) {
            return Err(NativeDeviceAdmissionError::new(
                "device_workspace_execution_in_flight",
            ));
        }
        Ok(Self {
...
impl Drop for Running {
    fn drop(&mut self) {
        if let Ok(mut guard) = running().lock() {
            guard.remove(&self.execution_id);
        }
    }
}
