...
impl CrewonDomainRequestProcessor {
    ...
    pub(crate) async fn office_auto_dispatch_is_relevant(
        &self,
        cwd: &str,
        source_thread_id: &str,
        source_turn_id: &str,
    ) -> Result<bool, JSONRPCErrorError> {
        office_migration_targets::configs_referencing_turn(cwd, source_thread_id, source_turn_id)
            .await
    ...
    pub(crate) async fn sync_office_run_updates_for_thread_turn(
        ...
        if !self
            .office_auto_dispatch_is_relevant(cwd, thread_id, &turn.id)
            .await?
        {
            return Ok(Vec::new());
        }
