...
`crewon-tools` is the shared support crate for building, adapting, and executing
model-visible tools outside the core runtime.
...
compatibility-sensitive orchestration in the core runtime until the surrounding
...
- code-mode compatibility shims that do not depend on the core runtime
- other narrowly scoped host utilities that multiple crates need

The corresponding non-goals are just as important:

- do not move core orchestration here prematurely
...
1. Keep extension-owned executable-tool authoring in the extension API crate.
2. Move host-side planning/adaptation helpers here when they no longer need to
   stay coupled to the core runtime.
3. Leave compatibility-sensitive adapters in the core runtime while downstream
...
the core runtime, that is a sign to revisit the extraction boundary before adding
more here.
