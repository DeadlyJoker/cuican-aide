...
use crewon_code_mode::ToolDefinition as CodeModeToolDefinition;
            ...
            for tool in &mut namespace.tools {
                match tool {
                    ResponsesApiNamespaceTool::Function(tool) => {
                        ...
                        tool.description =
                            crewon_code_mode::augment_tool_definition(definition).description;
                    }
                }
            }
...
pub fn tool_spec_to_code_mode_tool_definition(spec: &ToolSpec) -> Option<CodeModeToolDefinition> {
    ...
    crewon_code_mode::is_code_mode_nested_tool(&definition.name)
        .then(|| crewon_code_mode::augment_tool_definition(definition))
}
...
pub fn collect_code_mode_tool_definitions<'a>(
    ...
    let mut tool_definitions = specs
        ...
        .filter(|definition| crewon_code_mode::is_code_mode_nested_tool(&definition.name))
        .map(crewon_code_mode::augment_tool_definition)
        .collect::<Vec<_>>();
