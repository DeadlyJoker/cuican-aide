...
fn auth_identity(auth: &CrewonAuth) -> (Option<String>, Option<String>) {
    (auth.get_chatgpt_user_id(), auth.get_account_id())
}

fn cloud_config_eligible_auth(auth: &CrewonAuth) -> bool {
...
impl<C> Clone for CloudConfigBundleService<C> {
    fn clone(&self) -> Self {
        Self {
            ...
            crewon_home: self.crewon_home.clone(),
    ...
    C: BundleClient + 'static,
{
    pub(crate) fn new(
        ...
        let crewon_home = AbsolutePathBuf::resolve_path_against_base(crewon_home, "/");
        Self {
            auth_manager,
            client,
            cache: CloudConfigBundleCache::new(crewon_home.clone()),
            crewon_home,
            timeout,
        }
    }
    ...
    pub(crate) async fn load_startup_bundle_with_timeout(
        ...
        let _timer =
            crewon_otel::start_global_timer("crewon.cloud_config_bundle.fetch.duration_ms", &[]);
    ...
    async fn load_valid_cached_bundle(
        ...
        match self.cache.load(chatgpt_user_id, account_id).await {
            Ok(signed_payload) => {
                if let Err(err) = validate_bundle(&signed_payload.bundle, &self.crewon_home) {
                    tracing::warn!(
    ...
    async fn validate_and_cache_remote_bundle(
        ...
        if let Err(err) = validate_bundle(&bundle, &self.crewon_home) {
            emit_fetch_final_metric(
                trigger,
                "error",
                "invalid_bundle",
