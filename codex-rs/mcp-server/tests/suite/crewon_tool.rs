...
use crewon_mcp_server::CrewonToolCallParam;
use crewon_mcp_server::ExecApprovalElicitRequestParams;
use crewon_mcp_server::ExecApprovalResponse;
use crewon_mcp_server::PatchApprovalElicitRequestParams;
use crewon_mcp_server::PatchApprovalResponse;
use crewon_protocol::protocol::FileChange;
use crewon_protocol::protocol::ReviewDecision;
use crewon_shell_command::parse_command;
...
async fn shell_command_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // Send a "crewon" tool request, which should hit the responses endpoint.
    // In turn, it should reply with a tool call, which the MCP should forward
    // as an elicitation.
    let crewon_request_id = mcp_process
        .send_crewon_tool_call(CrewonToolCallParam {
    ...
    assert_eq!(
        ...
        Some(create_expected_elicitation_request_params(
            ...
            crewon_request_id.to_string(),
            params.crewon_event_id.clone(),
            params.thread_id,
        )?)
    );
    ...
    // Verify the original `crewon` tool call completes and that the file was created.
    let crewon_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_response_message(RequestId::Number(crewon_request_id)),
    )
    .await??;
    assert_eq!(
        JsonRpcResponse {
            jsonrpc: JsonRpcVersion2_0,
            id: RequestId::Number(crewon_request_id),
            result: json!({
                "content": [
                    {
                        "text": "File created!",
...
fn create_expected_elicitation_request_params(
    command: Vec<String>,
    workdir: &Path,
    crewon_mcp_tool_call_id: String,
    crewon_event_id: String,
    thread_id: crewon_protocol::ThreadId,
) -> anyhow::Result<serde_json::Value> {
    let expected_message = format!(
        "Allow Crewon to run `{}` in `{}`?",
        shlex::try_join(command.iter().map(std::convert::AsRef::as_ref))?,
        workdir.to_string_lossy()
    );
    let crewon_parsed_cmd = parse_command::parse_command(&command);
    let params_json = serde_json::to_value(ExecApprovalElicitRequestParams {
        message: expected_message,
        requested_schema: json!({"type":"object","properties":{}}),
        thread_id,
        crewon_elicitation: "exec-approval".to_string(),
        crewon_mcp_tool_call_id,
        crewon_event_id,
        crewon_command: command,
        crewon_cwd: workdir.to_path_buf(),
        crewon_call_id: "call1234".to_string(),
        crewon_parsed_cmd,
    })?;
    Ok(params_json)
}
...
async fn patch_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // Send a "crewon" tool request that will trigger the apply_patch command
    let crewon_request_id = mcp_process
        .send_crewon_tool_call(CrewonToolCallParam {
    ...
    // Verify the original `crewon` tool call completes
    let crewon_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_response_message(RequestId::Number(crewon_request_id)),
    )
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_crewon_tool_passes_base_instructions() {
    skip_if_no_network!();

    // Apparently `#[tokio::test]` must return `()`, so we create a helper
    // function that returns `Result` so we can use `?` in favor of `unwrap`.
    if let Err(err) = crewon_tool_passes_base_instructions().await {
        panic!("failure: {err}");
    }
}

async fn crewon_tool_passes_base_instructions() -> anyhow::Result<()> {
    ...
    // Run the MCP server with a specific config.toml.
    ...
    .await??;
    assert_eq!(crewon_response.jsonrpc, JsonRpcVersion2_0);
    assert_eq!(crewon_response.id, RequestId::Number(crewon_request_id));
    assert_eq!(
        crewon_response.result,
        json!({
            "content": [
                {
                    "text": "Enjoy!",
            ...
            "structuredContent": {
                "threadId": crewon_response
                    .result
                    .get("structuredContent")
                    .and_then(|v| v.get("threadId"))
                    .and_then(serde_json::Value::as_str)
                    .expect("crewon tool response should include structuredContent.threadId"),
                "content": "Enjoy!"
            }
        })
    );
...
/// Create a Crewon config that uses the mock server as the model provider.
