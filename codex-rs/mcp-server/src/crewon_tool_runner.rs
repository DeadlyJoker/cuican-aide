//! Asynchronous worker that executes a **Crewon** tool-call inside a spawned
...
use crewon_core::config::Config as CrewonConfig;
...
/// To adhere to MCP `tools/call` response format, include the Crewon
...
/// Run a complete Crewon session and stream events back to the client.
///
/// On completion (success or error) the function sends the appropriate
/// `tools/call` response so the LLM can continue the conversation.
pub async fn run_crewon_tool_session(
    id: RequestId,
    ...
    let NewThread {
        ...
        Err(e) => {
            let result = CallToolResult::error(vec![Content::text(format!(
                "Failed to start Crewon session: {e}"
            ))]);
    ...
    // Use the original MCP request ID as the `sub_id` for the Crewon submission so that
    ...
    if let Err(e) = thread.submit_with_id(submission).await {
        ...
        running_requests_id_to_thread_id.lock().await.remove(&id);
        return;
    }

    run_crewon_tool_session_inner(
        thread_id,
        thread,
        outgoing,
        id,
...
pub async fn run_crewon_tool_session_reply(
    thread_id: ThreadId,
...
async fn run_crewon_tool_session_inner(
    thread_id: ThreadId,
    thread: Arc<CrewonThread>,
    outgoing: Arc<OutgoingMessageSender>,
    request_id: RequestId,
