//! Local OAuth callback server for interactive login.
//!
...
//! Returned `io::Error` values still carry the detail needed by client/browser callers, while
...
// Keep in sync with the Crewon Hydra redirect URI allow-list.
                ...
                Ok(tokens) => {
                    ...
                    match tiny_http::Header::from_bytes(&b"Location"[..], success_url.as_bytes()) {
                        ...
                        Err(_) => login_error_response(
                            "Sign-in completed but redirecting back to Crewon failed.",
                            io::ErrorKind::Other,
...
fn build_authorize_url(
    ...
    let mut query = vec![
        ...
        ("crewon_simplified_flow".to_string(), "true".to_string()),
        // Keep the legacy parameter until the auth backend no longer requires it.
...
///
/// The returned error remains suitable for user-facing client/browser surfaces, so backend-provided
...
/// Returns true when the OAuth callback represents a missing Crewon entitlement.
...
fn oauth_callback_error_message(error_code: &str, error_description: Option<&str>) -> String {
    if is_missing_codex_entitlement_error(error_code, error_description) {
        return "Crewon is not enabled for your workspace. Contact your workspace administrator to request access to Crewon.".to_string();
    }
...
/// preserved only for the returned error path so the client/browser can still surface the backend
...
fn parse_token_endpoint_error(body: &str) -> TokenEndpointErrorDetail {
    ...
    // Preserve non-JSON token-endpoint bodies for the returned error so client/browser flows still
...
fn render_login_error_page(
    ...
    let (title, display_message, display_description, help_text) =
        if is_missing_codex_entitlement_error(code, error_description) {
            (
                "You do not have access to Crewon".to_string(),
                "This account is not currently authorized to use Crewon in this workspace."
                    .to_string(),
                "Contact your workspace administrator to request access to Crewon.".to_string(),
                "Contact your workspace administrator to get access to Crewon, then return to Crewon and try again."
                    .to_string(),
            )
        } else {
            (
                ...
                "Return to Crewon to retry, switch accounts, or contact your workspace admin if access is restricted."
                    .to_string(),
            )
        };
...
mod tests {
    ...
    #[test]
    fn build_authorize_url_sends_crewon_and_legacy_simplified_flow_flags() {
        let pkce = PkceCodes {
        ...
        assert_eq!(
            query.get("crewon_simplified_flow").map(String::as_str),
            Some("true")
        );
        assert_eq!(
    ...
    fn render_login_error_page_uses_entitlement_copy() {
        ...
        assert!(body.contains("You do not have access to Crewon"));
