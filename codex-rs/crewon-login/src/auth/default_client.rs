//! Default Crewon HTTP client: shared `User-Agent`, `originator`, optional residency header, and
//! reqwest/`CrewonHttpClient` construction.
//!
//! Use [`crate::default_client`] or [`crewon_login::default_client`] from other crates in this
//! workspace.

use crewon_client::BuildCustomCaTransportError;
use crewon_client::CrewonHttpClient;
pub use crewon_client::CrewonRequestBuilder;
...
/// Keep this centralized so every outbound client receives the same process-level suffix.
...
/// Parenthesis will be added by Crewon. This should only specify what goes inside of the parenthesis.
pub static USER_AGENT_SUFFIX: LazyLock<Mutex<Option<String>>> = LazyLock::new(|| Mutex::new(None));
pub const DEFAULT_ORIGINATOR: &str = "crewon_rs";
pub const CREWON_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR: &str = "CREWON_INTERNAL_ORIGINATOR_OVERRIDE";
pub const RESIDENCY_HEADER_NAME: &str = "x-crewon-residency";

pub use crewon_config::ResidencyRequirement;
...
fn get_originator_value(provided: Option<String>) -> Originator {
    let value = std::env::var(CREWON_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR)
        .ok()
        .or(provided)
...
pub fn originator() -> Originator {
    ...
    if std::env::var(CREWON_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR).is_ok() {
...
pub fn is_first_party_originator(originator_value: &str) -> bool {
    originator_value == DEFAULT_ORIGINATOR
        || originator_value == "crewon-web"
        || originator_value == "crewon_vscode"
        || originator_value.starts_with("Crewon ")
}

pub fn is_first_party_chat_originator(originator_value: &str) -> bool {
    originator_value == "crewon_atlas" || originator_value == "crewon_chatgpt_desktop"
}

pub fn get_crewon_user_agent() -> String {
...
fn sanitize_user_agent(candidate: String, fallback: &str) -> String {
    ...
    if !sanitized.is_empty() && HeaderValue::from_str(sanitized.as_str()).is_ok() {
        tracing::warn!(
            "Sanitized Crewon user agent because provided suffix contained invalid header characters"
        );
        sanitized
    } else if HeaderValue::from_str(fallback).is_ok() {
        tracing::warn!(
            "Falling back to base Crewon user agent because provided suffix could not be sanitized"
        );
        fallback.to_string()
    } else {
        tracing::warn!(
            "Falling back to default Crewon originator because base user agent string is invalid"
        );
        originator().value
    }
}
...
pub fn create_client() -> CrewonHttpClient {
    let inner = build_reqwest_client();
    CrewonHttpClient::new(inner)
}

/// Builds the default reqwest client used for ordinary Crewon HTTP traffic.
///
/// This starts from the standard Crewon user agent, default headers, and sandbox-specific proxy
/// policy, then layers in shared custom CA handling from `CREWON_CA_CERTIFICATE` /
...
/// Tries to build the default reqwest client used for ordinary Crewon HTTP traffic.
///
...
pub fn default_headers() -> HeaderMap {
    ...
    if let Ok(user_agent) = HeaderValue::from_str(&get_crewon_user_agent()) {
