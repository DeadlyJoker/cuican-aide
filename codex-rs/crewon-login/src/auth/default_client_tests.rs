...
fn is_first_party_originator_matches_known_values() {
    ...
    assert_eq!(is_first_party_originator("crewon-web"), true);
    assert_eq!(is_first_party_originator("crewon_vscode"), true);
    assert_eq!(is_first_party_originator("Crewon Something Else"), true);
    assert_eq!(is_first_party_originator("legacy_cli"), false);
    assert_eq!(is_first_party_originator("Other"), false);
}

#[test]
fn is_first_party_chat_originator_matches_known_values() {
    assert_eq!(is_first_party_chat_originator("crewon_atlas"), true);
    assert_eq!(
        is_first_party_chat_originator("crewon_chatgpt_desktop"),
        true
    );
    assert_eq!(is_first_party_chat_originator(DEFAULT_ORIGINATOR), false);
    assert_eq!(is_first_party_chat_originator("crewon_vscode"), false);
}
...
async fn test_create_client_sets_default_headers() {
    ...
    // User-Agent matches the computed Crewon UA for that originator
    let expected_ua = get_crewon_user_agent();
...
fn test_invalid_suffix_is_sanitized() {
    let prefix = "crewon_rs/0.0.0";
    let suffix = "bad\rsuffix";

    assert_eq!(
        sanitize_user_agent(format!("{prefix} ({suffix})"), prefix),
        "crewon_rs/0.0.0 (bad_suffix)"
    );
}
