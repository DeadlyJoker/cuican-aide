...
use crewon_agent_identity::decode_agent_identity_jwt;
use crewon_agent_identity::fetch_agent_identity_jwks;
use crewon_app_server_protocol::AuthMode;
use crewon_app_server_protocol::AuthMode as ApiAuthMode;
use crewon_protocol::config_types::ForcedLoginMethod;
use crewon_protocol::config_types::ModelProviderAuthInfo;

use super::access_token::CrewonAccessToken;
use super::access_token::classify_crewon_access_token;
...
use crewon_client::CrewonHttpClient;
use crewon_config::types::AuthCredentialsStoreMode;
use crewon_protocol::account::PlanType as AccountPlanType;
use crewon_protocol::auth::PlanType as InternalPlanType;
use crewon_protocol::auth::RefreshTokenFailedError;
use crewon_protocol::auth::RefreshTokenFailedReason;
...
impl CrewonAuth {
    ...
    pub fn uses_crewon_backend(&self) -> bool {
        matches!(
            self,
            Self::Chatgpt(_)
    ...
    pub fn get_token(&self) -> Result<String, std::io::Error> {
        match self {
            ...
            Self::BedrockApiKey(_) => Err(std::io::Error::other(
                "Bedrock API key auth does not expose a Crewon bearer token",
            )),
        }
    }

    /// Returns `None` if Crewon backend auth does not expose an account id.
    ...
    /// Returns false if Crewon backend auth omits the FedRAMP claim.
    ...
    /// Returns `None` if Crewon backend auth does not expose an account email.
    ...
    /// Returns `None` if Crewon backend auth does not expose a ChatGPT user id.
...
impl ChatgptAuth {
    ...
    fn client(&self) -> &CrewonHttpClient {
        &self.state.client
    }
}
...
pub async fn login_with_access_token(
    ...
    let auth_dot_json = match classify_crewon_access_token(access_token) {
        CrewonAccessToken::PersonalAccessToken(access_token) => {
            PersonalAccessTokenAuth::load(access_token).await?;
            AuthDotJson {
                // Infer PAT auth from the credential field so older Crewon builds can still
        ...
        CrewonAccessToken::AgentIdentityJwt(jwt) => {
            let base_url = chatgpt_base_url
                .unwrap_or(DEFAULT_CHATGPT_BACKEND_BASE_URL)
                .trim_end_matches('/')
                .to_string();
...
pub async fn enforce_login_restrictions(config: &AuthConfig) -> std::io::Result<()> {
    ...
    if let Some(expected_account_ids) = config.forced_chatgpt_workspace_id.as_deref() {
        let chatgpt_account_id = match &auth {
            ...
            | CrewonAuth::BedrockApiKey(_) => return Ok(()),
            CrewonAuth::AgentIdentity(_) => auth.get_account_id(),
            CrewonAuth::Chatgpt(_) | CrewonAuth::ChatgptAuthTokens(_) => {
...
async fn load_auth(
    ...
    auth_credentials_store_mode: AuthCredentialsStoreMode,
    chatgpt_base_url: Option<&str>,
) -> std::io::Result<Option<CrewonAuth>> {
    // API key via env var takes precedence over any other auth method.
    if enable_codex_api_key_env && let Some(api_key) = read_codex_api_key_from_env() {
        return Ok(Some(CrewonAuth::from_api_key(api_key.as_str())));
    }
    ...
    if let Some(auth_dot_json) = ephemeral_storage.load()? {
        let auth = CrewonAuth::from_auth_dot_json(
            codex_home,
            auth_dot_json,
            AuthCredentialsStoreMode::Ephemeral,
            chatgpt_base_url,
    ...
    if let Some(access_token) = read_codex_access_token_from_env() {
        return match classify_crewon_access_token(&access_token) {
            CrewonAccessToken::PersonalAccessToken(access_token) => {
                CrewonAuth::from_personal_access_token(access_token)
                    .await
                    .map(Some)
            }
            CrewonAccessToken::AgentIdentityJwt(jwt) => {
                CrewonAuth::from_agent_identity_jwt(jwt, chatgpt_base_url)
                    .await
                    .map(Some)
            }
        };
...
impl Debug for CachedAuth {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("CachedAuth")
            .field(
                "auth_mode",
                &self.auth.as_ref().map(CrewonAuth::api_auth_mode),
            )
            .field(
...
impl UnauthorizedRecovery {
    fn new(manager: Arc<AuthManager>) -> Self {
        ...
        let expected_account_id = cached_auth.as_ref().and_then(CrewonAuth::get_account_id);
        let mode = if manager.has_external_api_key_auth()
            || cached_auth
                .as_ref()
                .is_some_and(CrewonAuth::is_external_chatgpt_tokens)
        {
    ...
    pub fn has_next(&self) -> bool {
        ...
        if !self
            ...
            .auth_cached()
            .as_ref()
            .is_some_and(CrewonAuth::supports_unauthorized_recovery)
        {
            return false;
        }
    ...
    pub fn unavailable_reason(&self) -> &'static str {
        ...
        if self
            ...
            .auth_cached()
            .as_ref()
            .is_some_and(CrewonAuth::is_personal_access_token_auth)
        {
...
/// hands out cloned `CrewonAuth` values so the rest of the program has a
/// consistent snapshot.
///
...
/// the core config crate, but this trait keeps `crewon-login` independent
/// from the core runtime.
pub trait AuthManagerConfig {
    /// Returns the Crewon home directory used for auth storage.
    fn codex_home(&self) -> PathBuf;

    /// Returns the auth credential storage mode for auth loading.
...
impl AuthManager {
    ...
    /// Create an AuthManager with a specific CrewonAuth, for testing only.
    pub fn from_auth_for_testing(auth: CrewonAuth) -> Arc<Self> {
    ...
    /// Create an AuthManager with a specific CrewonAuth and Crewon home, for testing only.
    pub fn from_auth_for_testing_with_home(auth: CrewonAuth, codex_home: PathBuf) -> Arc<Self> {
        let cached = CachedAuth {
            auth: Some(auth),
            permanent_refresh_failure: None,
        };
    ...
    pub fn auth_cached(&self) -> Option<CrewonAuth> {
    ...
    pub fn refresh_failure_for_auth(&self, auth: &CrewonAuth) -> Option<RefreshTokenFailedError> {
    ...
    pub async fn auth(&self) -> Option<CrewonAuth> {
    ...
    async fn reload_if_account_id_matches(
        ...
        let new_auth = self.load_auth_from_storage().await;
        let new_account_id = new_auth.as_ref().and_then(CrewonAuth::get_account_id);
    ...
    fn auths_equal_for_refresh(a: Option<&CrewonAuth>, b: Option<&CrewonAuth>) -> bool {
        match (a, b) {
            (None, None) => true,
            (Some(a), Some(b)) => match (a.api_auth_mode(), b.api_auth_mode()) {
                ...
                (ApiAuthMode::AgentIdentity, ApiAuthMode::AgentIdentity) => match (a, b) {
                    (CrewonAuth::AgentIdentity(a), CrewonAuth::AgentIdentity(b)) => {
                        a.record() == b.record()
                    }
                    _ => false,
                },
    ...
    fn auths_equal(a: Option<&CrewonAuth>, b: Option<&CrewonAuth>) -> bool {
        match (a, b) {
            (None, None) => true,
            (Some(a), Some(b)) => a == b,
            _ => false,
    ...
    async fn load_auth_from_storage(&self) -> Option<CrewonAuth> {
        load_auth(
            &self.codex_home,
    ...
    fn set_cached_auth(&self, new_auth: Option<CrewonAuth>) -> bool {
        if let Ok(mut guard) = self.inner.write() {
    ...
    async fn resolve_external_api_key_auth(&self) -> Option<CrewonAuth> {
        ...
        match external_auth.resolve().await {
            Ok(Some(tokens)) => Some(CrewonAuth::from_api_key(&tokens.access_token)),
            Ok(None) => None,
            Err(err) => {
    ...
    pub async fn refresh_token(&self) -> Result<(), RefreshTokenError> {
        ...
        let expected_account_id = auth_before_reload
            .as_ref()
            .and_then(CrewonAuth::get_account_id);
    ...
    async fn refresh_token_from_authority_impl(&self) -> Result<(), RefreshTokenError> {
        ...
        let result = match auth {
            ...
            CrewonAuth::Chatgpt(chatgpt_auth) => {
    ...
    pub fn get_api_auth_mode(&self) -> Option<ApiAuthMode> {
        ...
        self.auth_cached().as_ref().map(CrewonAuth::api_auth_mode)
    }
    ...
    pub fn auth_mode(&self) -> Option<AuthMode> {
        ...
        self.auth_cached().as_ref().map(CrewonAuth::auth_mode)
    }

    pub fn current_auth_uses_crewon_backend(&self) -> bool {
        matches!(
            self.auth_mode(),
            Some(
                AuthMode::Chatgpt
    ...
    fn should_refresh_proactively(auth: &CrewonAuth) -> bool {
        let chatgpt_auth = match auth {
            CrewonAuth::Chatgpt(chatgpt_auth) => chatgpt_auth,
            _ => return false,
        };
