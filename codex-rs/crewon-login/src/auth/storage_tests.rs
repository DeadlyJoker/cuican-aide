...
fn keyring_auth_storage_compute_store_key_for_home_directory() -> anyhow::Result<()> {
    let codex_home = PathBuf::from("~/.crewon");

    let key = compute_store_key(codex_home.as_path())?;

    assert_eq!(key, "crewon|245d3be8196a3774");
    Ok(())
}

#[test]
fn keyring_auth_storage_loads_legacy_cli_key() -> anyhow::Result<()> {
    let codex_home = tempdir()?;
    let mock_keyring = MockKeyringStore::default();
    let storage = KeyringAuthStorage::new(
        codex_home.path().to_path_buf(),
    ...
    seed_keyring_with_auth(
        &mock_keyring,
        || compute_legacy_store_key(codex_home.path()),
        &expected,
    )?;
...
#[test]
fn keyring_auth_storage_save_removes_legacy_cli_key() -> anyhow::Result<()> {
    let codex_home = tempdir()?;
    let mock_keyring = MockKeyringStore::default();
    let storage = KeyringAuthStorage::new(
        codex_home.path().to_path_buf(),
    ...
    let legacy_key = compute_legacy_store_key(codex_home.path())?;
    mock_keyring.save(KEYRING_SERVICE, &legacy_key, "{}")?;

    storage.save(&auth_with_prefix("saved"))?;

    assert!(
        !mock_keyring.contains(&legacy_key),
        "legacy keyring entry should be removed after saving to the current key"
    );
    Ok(())
}
...
#[test]
fn keyring_auth_storage_delete_removes_legacy_cli_key() -> anyhow::Result<()> {
    let codex_home = tempdir()?;
    let mock_keyring = MockKeyringStore::default();
    let storage = KeyringAuthStorage::new(
        codex_home.path().to_path_buf(),
    ...
    assert!(removed, "delete should report legacy removal");
    assert!(
        !mock_keyring.contains(&legacy_key),
        "legacy keyring entry should be removed"
    );
    Ok(())
}
