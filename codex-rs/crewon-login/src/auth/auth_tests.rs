...
use crewon_protocol::auth::KnownPlan as InternalKnownPlan;
...
async fn missing_auth_json_returns_none() {
    ...
    let auth = CrewonAuth::from_auth_storage(
        dir.path(),
        AuthCredentialsStoreMode::File,
        /*chatgpt_base_url*/ None,
    )
...
async fn refresh_failure_is_scoped_to_the_matching_auth_snapshot() {
    ...
    let updated_auth = CrewonAuth::from_auth_dot_json(
        codex_home.path(),
...
async fn load_auth_reads_access_token_from_env() {
    ...
    let CrewonAuth::AgentIdentity(agent_identity) = auth else {
...
fn agent_identity_record(account_id: &str) -> AgentIdentityAuthRecord {
    let key_material =
        crewon_agent_identity::generate_agent_key_material().expect("generate agent key material");
...
async fn assert_agent_identity_plan_alias(
    ...
    let _authapi_guard =
        EnvVarGuard::set("CODEX_AGENT_IDENTITY_AUTHAPI_BASE_URL", &chatgpt_base_url);
    let auth = CrewonAuth::from_agent_identity_jwt(&jwt, Some(&chatgpt_base_url))
        .await
