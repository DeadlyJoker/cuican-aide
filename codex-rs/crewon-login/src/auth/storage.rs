...
use crewon_agent_identity::AgentIdentityJwtClaims;
...
const KEYRING_SERVICE: &str = "Crewon Auth";

// turns codex_home path into a stable, short key string
fn compute_store_key(codex_home: &Path) -> std::io::Result<String> {
    compute_store_key_with_prefix(codex_home, "crewon")
}

fn compute_legacy_store_key(codex_home: &Path) -> std::io::Result<String> {
    compute_store_key_with_prefix(codex_home, "cli")
}

fn compute_store_key_with_prefix(codex_home: &Path, prefix: &str) -> std::io::Result<String> {
        ...
        match self.keyring_store.load(KEYRING_SERVICE, key) {
            Ok(Some(serialized)) => serde_json::from_str(&serialized).map(Some).map_err(|err| {
                std::io::Error::other(format!(
                    "failed to deserialize Crewon auth from keyring: {err}"
                ))
            }),
            Ok(None) => Ok(None),
            Err(error) => Err(std::io::Error::other(format!(
                "failed to load Crewon auth from keyring: {}",
                error.message()
            ))),
        }
    }
...
impl AuthStorageBackend for KeyringAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        ...
        match self.load_from_keyring(&key)? {
            ...
            None => {
                let legacy_key = compute_legacy_store_key(&self.codex_home)?;
                self.load_from_keyring(&legacy_key)
            }
        }
    }
    ...
    fn save(&self, auth: &AuthDotJson) -> std::io::Result<()> {
        ...
        if let Err(err) = delete_file_if_exists(&self.codex_home) {
            warn!("failed to remove Crewon auth fallback file: {err}");
        }
        if let Ok(legacy_key) = compute_legacy_store_key(&self.codex_home)
            && legacy_key != key
        {
            let _ = self.keyring_store.delete(KEYRING_SERVICE, &legacy_key);
        }
        Ok(())
    }
    ...
    fn delete(&self) -> std::io::Result<bool> {
        ...
        let legacy_key = compute_legacy_store_key(&self.codex_home)?;
        let legacy_keyring_removed = if legacy_key == key {
            false
        } else {
            self.keyring_store
                .delete(KEYRING_SERVICE, &legacy_key)
                .map_err(|err| {
                    std::io::Error::other(format!(
                        "failed to delete legacy auth from keyring: {err}"
                    ))
                })?
        };
        let file_removed = delete_file_if_exists(&self.codex_home)?;
        Ok(keyring_removed || legacy_keyring_removed || file_removed)
    }
}
...
impl AuthStorageBackend for AutoAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        match self.keyring_storage.load() {
            ...
            Err(err) => {
                warn!(
                    "failed to load Crewon auth from keyring, falling back to file storage: {err}"
                );
