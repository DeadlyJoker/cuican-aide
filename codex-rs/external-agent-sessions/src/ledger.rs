...
pub fn has_current_session_been_imported(
    ...
    load_import_ledger(crewon_home)?.contains_current_source(source_path)
}
...
pub fn record_completed_session_imports(
    ...
    let mut ledger = load_import_ledger(crewon_home)?;
    ...
    save_import_ledger(crewon_home, &ledger)
}
...
pub(super) fn load_import_ledger(
    ...
    let path = import_ledger_path(crewon_home);
...
pub(super) fn save_import_ledger(
    ...
    fs::create_dir_all(crewon_home)?;
    let path = import_ledger_path(crewon_home);
    let raw = serde_json::to_vec_pretty(ledger).map_err(io::Error::other)?;
    fs::write(path, raw)
}

fn import_ledger_path(crewon_home: &Path) -> PathBuf {
    crewon_home.join(SESSION_IMPORT_LEDGER_FILE)
}
