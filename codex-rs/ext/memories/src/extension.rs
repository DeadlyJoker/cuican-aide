...
/// Contributes Crewon memory read-path prompt context and memory read tools.
...
pub(crate) struct MemoriesExtensionConfig {
    ...
    pub(crate) crewon_home: AbsolutePathBuf,
}
...
impl MemoriesExtensionConfig {
    fn from_config(config: &Config) -> Self {
        Self {
            ...
            crewon_home: config.codex_home.clone(),
        }
    }
}
...
impl ContextContributor for MemoriesExtension {
    fn contribute<'a>(
        ...
        Box::pin(async move {
            ...
            build_memory_tool_developer_instructions(&config.crewon_home)
                .await
...
impl ToolContributor for MemoriesExtension {
    fn tools(
        ...
        tools::memory_tools(
            LocalMemoriesBackend::from_crewon_home(&config.crewon_home),
