...
use crewon_protocol::protocol::validate_thread_goal_objective;
...
pub(crate) struct GoalToolExecutor {
    kind: GoalToolKind,
    thread_id: ThreadId,
    state_db: Arc<crewon_state::StateRuntime>,
...
impl ToolExecutor<ToolCall> for GoalToolExecutor {
    ...
    fn handle(&self, invocation: ToolCall) -> crewon_extension_api::ToolExecutorFuture<'_> {
        Box::pin(async move {
            match self.kind {
...
impl GoalToolExecutor {
    ...
    async fn handle_update(
        ...
        self.account_active_goal_progress(
            match args.status {
                ThreadGoalStatus::Complete => crewon_state::GoalAccountingMode::ActiveOrComplete,
                ThreadGoalStatus::Blocked => crewon_state::GoalAccountingMode::ActiveOrStopped,
    ...
    async fn account_active_goal_progress(
        &self,
        mode: crewon_state::GoalAccountingMode,
        event_id: &str,
        ...
        Ok(match outcome {
            crewon_state::GoalAccountingOutcome::Updated(goal) => {
                self.metrics
                    .record_terminal_if_status_changed(previous_status, &goal);
                self.analytics
            ...
            crewon_state::GoalAccountingOutcome::Unchanged(_) => None,
        })
    }
    ...
    ) -> Result<Option<crewon_state::ThreadGoalStatus>, FunctionCallError> {
        let goal = self
            .state_db
            .thread_goals()
            .get_thread_goal(self.thread_id)
...
pub(crate) async fn fill_empty_thread_preview_if_possible(
    state_db: &crewon_state::StateRuntime,
    thread_id: ThreadId,
    goal: &crewon_state::ThreadGoal,
) {
...
pub(crate) fn protocol_goal_from_state(goal: crewon_state::ThreadGoal) -> ThreadGoal {
    ThreadGoal {
...
fn protocol_status_from_state(status: crewon_state::ThreadGoalStatus) -> ThreadGoalStatus {
    match status {
...
pub(crate) fn state_status_from_protocol(
    status: ThreadGoalStatus,
) -> crewon_state::ThreadGoalStatus {
    match status {
        ThreadGoalStatus::Active => crewon_state::ThreadGoalStatus::Active,
        ThreadGoalStatus::Paused => crewon_state::ThreadGoalStatus::Paused,
        ThreadGoalStatus::Blocked => crewon_state::ThreadGoalStatus::Blocked,
        ThreadGoalStatus::UsageLimited => crewon_state::ThreadGoalStatus::UsageLimited,
        ThreadGoalStatus::BudgetLimited => crewon_state::ThreadGoalStatus::BudgetLimited,
        ThreadGoalStatus::Complete => crewon_state::ThreadGoalStatus::Complete,
    }
}
