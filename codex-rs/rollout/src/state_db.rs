...
pub type StateDbHandle = Arc<crewon_state::StateRuntime>;
...
async fn try_init_with_roots_inner(
    ...
    let codex_home = normalize_home_for_state_db(codex_home);
    let sqlite_home = normalize_home_for_state_db(sqlite_home);
    let runtime =
        crewon_state::StateRuntime::init(sqlite_home.clone(), default_model_provider_id.clone())
            .await
            .with_context(|| {
                format!(
    ...
    .await;
    crewon_state::record_backfill_gate(
        /*telemetry*/ None,
...
async fn wait_for_backfill_gate(
    ...
    loop {
        ...
        if backfill_state.status == crewon_state::BackfillStatus::Complete {
            return Ok(());
        }
...
pub async fn get_state_db(config: &impl RolloutConfigView) -> Option<StateDbHandle> {
    let sqlite_home = normalize_home_for_state_db(config.sqlite_home().to_path_buf());
    let state_path = crewon_state::state_db_path(sqlite_home.as_path());
    ...
    let runtime = match crewon_state::StateRuntime::init(
        sqlite_home.clone(),
    ...
    require_backfill_complete(runtime, sqlite_home.as_path()).await
}

fn normalize_home_for_state_db(path: PathBuf) -> PathBuf {
    normalize_for_path_comparison(path.as_path()).unwrap_or(path)
}

/// Build a SQLite telemetry recorder backed by an OTEL metrics client.
pub fn sqlite_telemetry_recorder(
    metrics: crewon_otel::MetricsClient,
    originator: &str,
) -> crewon_state::DbTelemetryHandle {
...
async fn require_backfill_complete(
    ...
    match runtime.get_backfill_state().await {
        Ok(state) if state.status == crewon_state::BackfillStatus::Complete => Some(runtime),
        Ok(state) => {
            warn!(
...
fn cursor_to_anchor(cursor: Option<&Cursor>) -> Option<crewon_state::Anchor> {
    let cursor = cursor?;
...
pub async fn list_thread_ids_db(
    context: Option<&crewon_state::StateRuntime>,
    codex_home: &Path,
    page_size: usize,
    ...
    match ctx
        .list_thread_ids(
            ...
            anchor.as_ref(),
            match sort_key {
                ThreadSortKey::CreatedAt => crewon_state::SortKey::CreatedAt,
                ThreadSortKey::UpdatedAt => crewon_state::SortKey::UpdatedAt,
            },
...
pub async fn list_threads_db(
...
) -> Option<crewon_state::ThreadsPage> {
    let ctx = context?;
    if ctx.codex_home() != codex_home {
        warn!(
            "state db codex_home mismatch: expected {}, got {}",
    ...
    match ctx
        .list_threads(
            page_size,
            crewon_state::ThreadFilterOptions {
                ...
                sort_direction: match sort_direction {
                    SortDirection::Asc => crewon_state::SortDirection::Asc,
                    SortDirection::Desc => crewon_state::SortDirection::Desc,
                },
                search_term,
            },
        )
