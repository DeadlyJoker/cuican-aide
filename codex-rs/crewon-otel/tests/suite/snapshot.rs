...
fn snapshot_collects_metrics_without_shutdown() -> Result<()> {
    ...
    let metric = find_metric(&snapshot, "crewon.tool.call").expect("counter metric missing");
...
fn manager_snapshot_metrics_collects_without_shutdown() -> Result<()> {
    let exporter = InMemoryMetricExporter::default();
    let config = MetricsConfig::in_memory("test", "crewon", env!("CARGO_PKG_VERSION"), exporter)
        .with_tag("service", "crewon")?
        .with_runtime_reader();
    let metrics = MetricsClient::new(config)?;
