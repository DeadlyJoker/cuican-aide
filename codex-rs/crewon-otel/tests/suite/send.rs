...
fn send_builds_payload_with_tags_and_histograms() -> Result<()> {
    let (metrics, exporter) =
        build_metrics_with_defaults(&[("service", "crewon"), ("env", "prod")])?;

    metrics.counter_with_description(
        "crewon.turns",
        "Total number of Crewon turns.",
        /*inc*/ 1,
    ...
    metrics.gauge_with_description(
        "crewon.active",
        "Number of active Crewon operations.",
        /*value*/ 2,
        &[("component", "test")],
    )?;
    metrics.shutdown()?;
    ...
    let counter = find_metric(&resource_metrics, "crewon.turns").expect("counter metric missing");
    assert_eq!(counter.description(), "Total number of Crewon turns.");
    ...
    let expected_counter_attributes = BTreeMap::from([
        ("service".to_string(), "crewon".to_string()),
    ...
    let (bounds, bucket_counts, sum, count) =
        histogram_data(&resource_metrics, "crewon.tool_latency");
    ...
    let histogram_attrs = attributes_to_map(
        match find_metric(&resource_metrics, "crewon.tool_latency").and_then(|metric| match metric
            .data()
        {
    ...
    let gauge = find_metric(&resource_metrics, "crewon.active").expect("gauge metric missing");
    assert_eq!(gauge.description(), "Number of active Crewon operations.");
...
fn send_merges_default_tags_per_line() -> Result<()> {
    let (metrics, exporter) =
        build_metrics_with_defaults(&[("service", "crewon"), ("env", "prod"), ("region", "us")])?;
    ...
    let resource_metrics = latest_metrics(&exporter);
    let alpha_metric =
        find_metric(&resource_metrics, "crewon.alpha").expect("crewon.alpha metric missing");
    ...
    let beta_metric =
        find_metric(&resource_metrics, "crewon.beta").expect("crewon.beta metric missing");
...
fn client_sends_enqueued_metric() -> Result<()> {
    ...
    metrics.counter("crewon.turns", /*inc*/ 1, &[("model", "gpt-5.1")])?;
    metrics.shutdown()?;
...
fn shutdown_flushes_in_memory_exporter() -> Result<()> {
    ...
    metrics.counter("crewon.turns", /*inc*/ 1, &[])?;
    metrics.shutdown()?;
