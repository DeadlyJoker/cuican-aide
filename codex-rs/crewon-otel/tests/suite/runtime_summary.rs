...
use crewon_otel::RuntimeMetricTotals;
use crewon_otel::RuntimeMetricsSummary;
...
fn runtime_metrics_summary_collects_tool_api_and_streaming_metrics() -> Result<()> {
    ...
    let metrics = MetricsClient::new(
        MetricsConfig::in_memory("test", "crewon", env!("CARGO_PKG_VERSION"), exporter)
            .with_runtime_reader(),
    )?;
    ...
    manager.record_duration(
        "crewon.turn.ttft.duration_ms",
        Duration::from_millis(95),
        &[],
    );
    manager.record_duration(
        "crewon.turn.ttfm.duration_ms",
