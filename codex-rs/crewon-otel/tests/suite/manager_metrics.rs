...
use crewon_otel::PLUGIN_INSTALL_ELICITATION_SENT_METRIC;
use crewon_otel::PLUGIN_INSTALL_SUGGESTION_METRIC;
...
fn manager_attaches_metadata_tags_to_metrics() -> Result<()> {
    let (metrics, exporter) = build_metrics_with_defaults(&[("service", "crewon")])?;
    let manager = SessionTelemetry::new(
        ...
        "web".to_string(),
        SessionSource::Custom("web".to_string()),
    )
    .with_metrics(metrics);

    manager.counter(
        "crewon.session_started",
        /*inc*/ 1,
        &[("source", "web")],
    );
    ...
    let metric =
        find_metric(&resource_metrics, "crewon.session_started").expect("counter metric missing");
    ...
    let expected = BTreeMap::from([
        ...
        ("session_source".to_string(), "web".to_string()),
...
fn manager_allows_disabling_metadata_tags() -> Result<()> {
    ...
    let expected = BTreeMap::from([("source".to_string(), "web".to_string())]);
    assert_eq!(attrs, expected);
...
fn manager_attaches_optional_service_name_tag() -> Result<()> {
    ...
    manager.counter("crewon.session_started", /*inc*/ 1, &[]);
    manager.shutdown_metrics()?;
