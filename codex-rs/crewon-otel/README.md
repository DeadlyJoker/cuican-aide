...
`crewon-otel` is the OpenTelemetry integration crate for Crewon. It provides:

- Provider wiring for log/trace/metric exporters (`crewon_otel::OtelProvider`
  and `crewon_otel::provider`).
- Session-scoped business event emission via `crewon_otel::SessionTelemetry`.
- Low-level metrics APIs via `crewon_otel::metrics`.
- Trace-context helpers via `crewon_otel::trace_context` and crate-root re-exports.
...
```rust
use crewon_otel::config::OtelExporter;
use crewon_otel::config::OtelHttpProtocol;
use crewon_otel::config::OtelSettings;
use crewon_otel::OtelProvider;
use tracing_subscriber::prelude::*;

let settings = OtelSettings {
    environment: "dev".to_string(),
    service_name: "crewon".to_string(),
...
that member. If propagated trace context already has the named member, Crewon
...
Crewon-specific session events. Rich session/business events should go through
...
`crewon-otel` also provides `OtelExporter::Statsig`, a shorthand for exporting OTLP/HTTP JSON metrics
to Statsig using Crewon-internal defaults.

Statsig ingestion (OTLP/HTTP JSON) example:

```rust
use crewon_otel::config::{OtelExporter, OtelHttpProtocol};
...
metrics.counter("crewon.session_started", 1, &[("source", "web")])?;
metrics.histogram("crewon.request_latency", 83, &[("route", "chat")])?;
```
...
metrics.counter("crewon.turns", 1, &[("model", "gpt-5.1")])?;
