...
pub use crewon_utils_string::sanitize_metric_tag_value;
...
/// Maps to API/auth `AuthMode` to avoid a circular dependency on the core runtime.
...
impl From<crewon_app_server_protocol::AuthMode> for TelemetryAuthMode {
    fn from(mode: crewon_app_server_protocol::AuthMode) -> Self {
        match mode {
            crewon_app_server_protocol::AuthMode::ApiKey
            | crewon_app_server_protocol::AuthMode::BedrockApiKey => Self::ApiKey,
            crewon_app_server_protocol::AuthMode::Chatgpt
            | crewon_app_server_protocol::AuthMode::ChatgptAuthTokens
            | crewon_app_server_protocol::AuthMode::AgentIdentity
            | crewon_app_server_protocol::AuthMode::PersonalAccessToken => Self::Chatgpt,
        }
    }
}
