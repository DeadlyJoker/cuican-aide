...
async fn append_entry_trims_history_when_beyond_max_bytes() {
    let crewon_home = TempDir::new().expect("create temp dir");
    let mut history = History::default();
    let mut config = HistoryConfig::new(crewon_home.path(), &history);
    let conversation_id = "conversation-id";

    let entry_one = "a".repeat(200);
    let entry_two = "b".repeat(200);

    let history_path = crewon_home.path().join("history.jsonl");
    ...
    config = HistoryConfig::new(crewon_home.path(), &history);
