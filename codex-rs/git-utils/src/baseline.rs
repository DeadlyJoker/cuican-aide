...
const BASELINE_COMMIT_MESSAGE: &str =
    "Initialize Crewon git baseline\n\nCo-authored-by: Crewon <noreply@crewon.local>";
...
fn commit_current_tree(repo: &gix::Repository, message: &str) -> anyhow::Result<()> {
    ...
    let signature = crewon_signature();
...
fn crewon_signature() -> gix::actor::Signature {
    gix::actor::Signature {
        name: "Crewon".into(),
        email: "noreply@crewon.local".into(),
        time: gix::date::Time {
