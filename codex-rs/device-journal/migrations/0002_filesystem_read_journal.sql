UPDATE device_journal_schema SET version = 2 WHERE singleton = 1 AND version = 1;
...
CREATE TABLE filesystem_read_events (
    ...
    event_type TEXT NOT NULL CHECK (
        (sequence = 1 AND event_type = 'workspace_read.accepted') OR
        (sequence = 2 AND event_type IN ('workspace_read.completed', 'workspace_read.failed',
            'workspace_read.canceled', 'workspace_read.unknown_outcome'))
    ),
...
CREATE UNIQUE INDEX filesystem_read_events_accepted_receipt_idx
    ON filesystem_read_events (receipt_id) WHERE sequence = 1;
CREATE INDEX filesystem_read_executions_unacknowledged_idx
    ON filesystem_read_executions (execution_id, acknowledged_through);
