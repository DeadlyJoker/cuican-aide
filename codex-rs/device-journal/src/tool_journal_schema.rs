...
pub(super) async fn assert_authority(pool: &SqlitePool) -> Result<(), DeviceJournalError> {
    ...
    assert_sql(
        ...
        &[
            ...
            "references tool_executions(execution_id) on delete restrict",
            ...
            "sequence = 1 and event_type = 'execution.accepted'",
    ...
    assert_index(
        pool,
        "tool_executions_unacknowledged_idx",
        "tool_executions",
    ...
    assert_index(
        pool,
        "tool_events_accepted_receipt_idx",
        "tool_events",
...
async fn assert_foreign_key(
    ...
    let rows = match table {
        "tool_events" => sqlx::query("PRAGMA foreign_key_list(tool_events)"),
        "tool_acks" => sqlx::query("PRAGMA foreign_key_list(tool_acks)"),
        _ => return Err(authority("device_journal_schema_corrupt")),
    }
    .fetch_all(pool)
    .await?;
