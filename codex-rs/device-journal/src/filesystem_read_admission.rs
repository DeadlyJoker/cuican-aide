...
impl DeviceWorkspaceJournal {
    pub async fn prepare_filesystem_read_with_admission<T, E>(
        ...
        PrepareFilesystemReadWithAdmissionOutcome<T>,
        PrepareFilesystemReadWithAdmissionError<E>,
    > {
        let command_json =
            encode_command(command).map_err(PrepareFilesystemReadWithAdmissionError::Journal)?;
        ...
        let mut tx = self
            ...
            .map_err(PrepareFilesystemReadWithAdmissionError::Journal)?;
        if let Some(existing) = load(&mut tx, &command.command.execution_id)
            .await
            .map_err(PrepareFilesystemReadWithAdmissionError::Journal)?
        {
            if existing.command != *command {
                return Err(PrepareFilesystemReadWithAdmissionError::Journal(authority(
        ...
        let (accepted, admitted) =
            admit().map_err(PrepareFilesystemReadWithAdmissionError::Admission)?;
        ...
        validate_acceptance(command, &accepted, accepted_envelope, accepted_type)
            .map_err(PrepareFilesystemReadWithAdmissionError::Journal)?;
        let accepted_json =
            encode_event(&accepted).map_err(PrepareFilesystemReadWithAdmissionError::Journal)?;
        ...
        if owner.is_some() {
            return Err(PrepareFilesystemReadWithAdmissionError::Journal(authority(
                "device_journal_filesystem_read_receipt_conflict",
            )));
        }
        sqlx::query(
...
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"#,
        ...
        .bind(i64::try_from(command.command.lease_epoch).map_err(|_| {
            PrepareFilesystemReadWithAdmissionError::Journal(authority(
                "device_journal_filesystem_read_invalid",
            ))
        })?)
        ...
        .map_err(DeviceJournalError::from)
        .map_err(PrepareFilesystemReadWithAdmissionError::Journal)?;
        insert_event(&mut tx, accepted_envelope, accepted_type, accepted_json)
            .await
            .map_err(PrepareFilesystemReadWithAdmissionError::Journal)?;
        ...
        tx.commit()
            ...
            .map_err(DeviceJournalError::from)
            .map_err(PrepareFilesystemReadWithAdmissionError::Journal)?;
        Ok(PrepareFilesystemReadWithAdmissionOutcome::New {
            execution,
            admitted,
        })
    }
