...
test("freezes redacted scheduled Automation commands and pagination", () => {
  const create = {
    threadId: "thread-1",
  ...
  const publicContract = JSON.stringify({
    paths: {
      collection: openApi.paths["/api/v1/automations"],
      item: openApi.paths["/api/v1/automations/{automationId}"],
      runNow: openApi.paths["/api/v1/automations/{automationId}:run-now"],
    },
    view: openApi.components.schemas.AutomationView,
    response: openApi.components.schemas.RunAutomationNowResponse,
  });
  const automationView = openApi.components.schemas.AutomationView as {
    properties: {
      executionMode: { enum: readonly string[] };
      automaticScheduling: { type: string };
      schedule: unknown;
    };
  };
  assert.deepEqual(automationView.properties.executionMode.enum, [
    "manualOnly",
    "scheduled",
  ]);
  assert.equal(automationView.properties.automaticScheduling.type, "boolean");
  assert.equal(Object.hasOwn(automationView.properties, "schedule"), true);
  const invocationView = openApi.components.schemas
    .AutomationInvocationView as unknown as {
    required: readonly string[];
    properties: Record<string, unknown>;
  };
  assert.deepEqual(invocationView.required, ["automationId", "runId"]);
  assert.deepEqual(Object.keys(invocationView.properties), [
    "automationId",
    "runId",
  ]);
  for (const forbidden of [
...
test("freezes a non-secret Provider snapshot and active-only bounded probe", () => {
  assert.deepEqual(parseProbeModelProviderRequest({}), {});
  for (const input of [null, { providerId: "attacker" }, { endpoint: "x" }]) {
    assert.throws(() => parseProbeModelProviderRequest(input), isContractError);
  }
  const publicSchemas = JSON.stringify({
    snapshot: openApi.components.schemas.ModelProviderSettingsSnapshot,
    result: openApi.components.schemas.ProbeModelProviderResponse,
  });
  for (const forbidden of [
    "runtimeBindingId",
    "coordinatorBinding",
    ...
    assert.equal(publicSchemas.includes(forbidden), false, forbidden);
  }
  assert.equal(
    openApi.paths["/api/v1/model-provider-settings/probe"].post.parameters
      .length,
    2,
  );
});
...
test("freezes strict Thread and text Message authority without client scope fields", () => {
  ...
  const getThreadResponseSchema = openApi.components.schemas
    .GetThreadResponse as {
    required: readonly string[];
    properties: { eventSequence: { minimum: number } };
  };
  const threadViewSchema = openApi.components.schemas.ThreadView as unknown as {
    properties: Record<string, unknown>;
  };
  assert.deepEqual(getThreadResponseSchema.required, [
    "thread",
    "eventSequence",
  ]);
  assert.equal(
    Object.hasOwn(threadViewSchema.properties, "eventSequence"),
    false,
  );
  assert.equal(getThreadResponseSchema.properties.eventSequence.minimum, 0);
  assert.deepEqual(
    parseForkThreadRequest({
      expectedRevision: 3,
...
type OpenApiDocument = Readonly<{
  ...
  paths: Record<string, unknown> & {
    ...
    "/api/v1/model-provider-settings": unknown;
    "/api/v1/model-provider-settings/probe": {
