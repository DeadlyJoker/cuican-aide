...
export type ModelProviderSettingView =
  components["schemas"]["ModelProviderSettingView"];
...
export type AutomationSchedule = components["schemas"]["AutomationSchedule"];
...
export function parseCreateAutomationRequest(
  ...
  return {
    ...
    schedule:
      input.schedule === null ? null : parseAutomationSchedule(input.schedule),
  };
}

function parseAutomationSchedule(input: unknown): AutomationSchedule {
  if (
    !hasExactKeys(input, [
      "intervalSeconds",
      "nextRunAt",
    ...
    Number(input.intervalSeconds) > 366 * 24 * 60 * 60 ||
    (input.scheduleType === "interval" &&
      Number(input.intervalSeconds) < 5 * 60) ||
...
export function parseStartTurnRequest(input: unknown): StartTurnRequest {
  ...
  if (
    ...
    input.executionIntent !== "resumeGoal"
  ) {
