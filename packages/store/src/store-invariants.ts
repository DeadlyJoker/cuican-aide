...
export function validateTurnStartInput(input: CommitTurnStartInput): {
  ...
  if (
    ...
    (runEvent.data.goalContinuationMode === "deferred" &&
      input.goal.kind !== "set") ||
    (input.goal.kind === "set" &&
      runEvent.data.goalContinuationMode !== "deferred" &&
      input.goal.goal.objective !== message.content)
  ) {
...
export function reduceGoalContinuationRun(
  ...
  if (
    goal?.status !== "active" ||
    previousRun.goalContinuationMode === "deferred"
  ) {
    if (input !== null) {
...
export function validateThreadScopedLocator(
  locator: import("@crewon/application").ThreadScopedLocator,
): void {
...
export function validateRunScopedLocator(
  locator: import("@crewon/application").RunScopedLocator,
): void {
  validateRunLocator(locator);
  requireNonEmpty(locator.spaceId, "space_id_invalid");
}
