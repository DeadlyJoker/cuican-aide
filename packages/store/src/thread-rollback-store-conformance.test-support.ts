...
export function registerThreadRollbackStoreConformance(
  ...
  describe(name, () => {
    test("atomically tombstones a turn, replays first, and filters before LIMIT", async (context) => {
      ...
      assert.deepEqual(audit[0]?.origin, seeded.messages[2]?.origin);
    ...
    const origin =
      sequence === 3
        ? {
            ...
            binding: {
              automationId: "rollback-automation",
              automationRevision: 1 as const,
              definitionDigest: digest(10),
              instructionDigest: digest(sequence),
              invocationId: "rollback-invocation",
              runId: "rollback-automation-run",
              routeDigest: digest(11),
            },
          }
        : null;
    return {
  ...
  const history: ModelHistoryItem[] = messages.map((message) =>
    message.origin === null
      ...
      : {
          ...
          threadId: message.threadId,
          sequence: message.sequence,
          runId: message.origin.binding.runId,
          segmentId: null,
          createdAt: message.createdAt,
          type: "message",
          role: "user",
