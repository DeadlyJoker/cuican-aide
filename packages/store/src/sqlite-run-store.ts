...
import {
  ...
  type ToolApprovalScopedLocator,
...
export class SqliteRunStore implements DomainStore {
  ...
  async listScheduledAutomations(
    ...
    return this.#automationAuthority.listScheduled(query);
  }
  ...
  async loadThreadInSpace(
    locator: ThreadScopedLocator,
  ): Promise<ThreadState | null> {
    this.#assertOpen();
    validateThreadScopedLocator(locator);
    try {
      return this.#loadThreadInSpace(locator);
    } catch (error) {
      throw normalizeSqliteError(error);
    }
  }
  ...
  async loadRunInSpace(locator: RunScopedLocator): Promise<RunState | null> {
    this.#assertOpen();
    validateRunScopedLocator(locator);
    try {
      const row = this.#database
        .prepare(
          `SELECT snapshots.tenant_id, snapshots.space_id, snapshots.run_id,
        ...
        .get(locator.tenantId, locator.spaceId, locator.runId) as
        | SnapshotRow
        | undefined;
      return row === undefined ? null : decodeStoredRunState(row, locator);
    } catch (error) {
      throw normalizeSqliteError(error);
    }
  }
  ...
  #loadThreadInSpace(locator: ThreadScopedLocator): ThreadState | null {
    const row = this.#database
      .prepare(
        `SELECT
           tenant_id,
           ...
           deleted_by_actor_id
         FROM threads
         WHERE tenant_id = ? AND space_id = ? AND thread_id = ?`,
      )
      .get(locator.tenantId, locator.spaceId, locator.threadId) as
      | ThreadSnapshotRow
      | undefined;
    if (row === undefined) {
      return null;
