...
export function migrateSqliteOffices(database: DatabaseSync) {
  database.exec(`
    ...
    CREATE INDEX IF NOT EXISTS office_definitions_list
      ON office_definitions (
        tenant_id,
        space_id,
        created_at DESC,
        office_version_id DESC
      );
...
export class SqliteOfficeStore implements OfficeDefinitionStore {
  ...
  async commitOfficeDefinition(
    ...
    try {
      const receipt = this.#database
        .prepare(
          `SELECT request_digest, office_version_id
           FROM office_create_receipts
      ...
      const latest = this.#database
        .prepare(
          `SELECT revision
           FROM office_definitions
           WHERE tenant_id = ? AND space_id = ? AND office_id = ?
      ...
      this.#database
        .prepare(
          `INSERT INTO office_definitions (
             tenant_id, space_id, office_id, office_version_id, revision,
             created_at, definition_json
           ) VALUES (?, ?, ?, ?, ?, ?, ?)`,
        )
        .run(
          value.tenantId,
      ...
      this.#database
        .prepare(
          `INSERT INTO office_create_receipts (
             tenant_id, space_id, actor_id, idempotency_key, request_digest,
             office_version_id
           ) VALUES (?, ?, ?, ?, ?, ?)`,
        )
        .run(
  ...
  async listOfficeDefinitions(input: {
    ...
    const values: Array<string | number> = [input.tenantId, input.spaceId];
    let cursor = "";
    ...
    const rows = this.#database
      .prepare(
        `SELECT definition_json
         FROM office_definitions
         WHERE tenant_id = ? AND space_id = ?${cursor}
         ORDER BY created_at DESC, office_version_id DESC
         LIMIT ?`,
      )
      .all(...values) as Array<{ definition_json: string }>;
    return rows.map((row) =>
      parseOfficeDefinition(JSON.parse(row.definition_json)),
    );
  }

  #load(tenantId: string, spaceId: string, officeVersionId: string) {
