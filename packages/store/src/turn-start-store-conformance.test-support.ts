...
export function registerTurnStartStoreConformance(
  ...
  describe(name, () => {
    ...
    test("atomically resumes a paused Goal without rewriting its objective", async (context) => {
      const store = await managedStore(context, createStore);
      await seedThread(store);
      const pausedGoal = { ...goalFixture(), status: "paused" as const };
      await store.commitTurnStart(
        turnStartCommit({
          goal: { kind: "set", expectedRevision: null, goal: pausedGoal },
          goalBinding: null,
        }),
      );
      await store.commitRun({
        tenantId: "tenant-1",
        idempotency: {
          scope: "turn-start-resume-setup",
          key: "turn-start-resume-terminal",
          requestFingerprint: "turn-start-resume-terminal-fingerprint",
        },
        expectedRevision: 1,
        events: [
          {
            ...
            eventId: "run-turn-resume-setup-terminal",
            sequence: 2,
            occurredAt: "2026-08-08T00:00:03Z",
            type: "run.failed",
      ...
      const committed = await store.commitTurnStart(
        turnStartCommit({
          key: "turn-start-resume",
          fingerprint: "turn-start-resume-fingerprint",
          runId: "run-turn-resume",
          expectedThreadRevision: 2,
          threadEventSequence: 3,
          messageSequence: 2,
          historySequence: 2,
          ...
          goal: {
            kind: "set",
            expectedRevision: pausedGoal.revision,
            goal: resumedGoal,
          },
          goalBinding: binding(resumedGoal),
          goalContinuationMode: "deferred",
        }),
      );

      assert.deepEqual(committed.goalState, resumedGoal);
      assert.equal(committed.runState.goalContinuationMode, "deferred");
      assert.equal(committed.messages[0]?.content, "question-2");
      assert.equal(committed.goalState?.objective, "question-1");
    });
      ...
      events: [
        {
          ...
          data: {
            ...
            ...(overrides.goalContinuationMode === undefined
              ? {}
              : { goalContinuationMode: overrides.goalContinuationMode }),
          },
        },
      ],
      outbox: [
