...
export class PostgresOfficeStore implements OfficeDefinitionStore {
    ...
    try {
      ...
      const receipt = await client.query<{
        ...
        `SELECT request_digest, office_version_id
         FROM ${this.#schema}.office_create_receipts
         WHERE tenant_id = $1 AND space_id = $2 AND actor_id = $3
      ...
      if (receipt.rows[0] !== undefined) {
        ...
        const found = await client.query<{
          ...
          `SELECT definition_json
           FROM ${this.#schema}.office_definitions
      ...
      const latest = await client.query<{ revision: number }>(
        `SELECT revision
         FROM ${this.#schema}.office_definitions
         WHERE tenant_id = $1 AND space_id = $2 AND office_id = $3
      ...
      await client.query(
        `INSERT INTO ${this.#schema}.office_definitions (
      ...
      await client.query(
        `INSERT INTO ${this.#schema}.office_create_receipts (
  ...
  async listOfficeDefinitions(input: {
    ...
    const result = await this.#pool.query<{
      ...
      `SELECT definition_json
       FROM ${this.#schema}.office_definitions
       WHERE tenant_id = $1 AND space_id = $2${cursor}
