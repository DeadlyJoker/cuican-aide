...
test("SQLite Workflow admission rejects a release switch after route resolution without writes", async (context) => {
  ...
  await assert.rejects(
    ...
    (error) =>
      ...
      [
        "workflow_run_route_mismatch",
        "workflow_agent_deployment_mismatch",
      ].includes(error.code),
  );
...
test("SQLite Workflow admission concurrent loser rechecks the receipt after resolving", async (context) => {
  ...
  const loser = first.commitWorkflowRunStart(
    admissionInput({
      resolveCandidateRoute: async () => {
        await paused;
        return route("default-v1");
...
test("SQLite Office delegation is receipt-first and bypasses both callbacks on replay", async (context) => {
  ...
  const progressed = await store.commitRun({
    tenantId: "tenant-1",
  ...
  assert.equal(replay.run.state.status, "queued");
  assert.equal(progressed.state.status, "running");
  assert.equal(
    (
      await store.loadRun({
        tenantId: "tenant-1",
        runId: fresh.run.state.runId,
      })
    )?.status,
    "running",
  );
...
async function seedOfficeAuthority(store: SqliteRunStore, path: string) {
  ...
  await store.commitOfficeDefinition({
    ...
    receipt: {
      actorId: "actor",
      idempotencyKey: "office-1",
      requestDigest: "office-fp-1",
    },
    definition: {
...
async function temporaryPath(
  ...
  const directory = await mkdtemp(
    join(tmpdir(), `workflow-admission-${label}-`),
  );
  context.after(() => rm(directory, { recursive: true, force: true }));
...
async function activateRelease(
  ...
  const bundle = {
    ...
    defaultAgentVersionId: defaultId,
    deployments,
  };
  await store.activateAgentVersionRelease({
    bundle,
    expectedActiveReleaseId: previousReleaseId,
    activation: {
      schemaVersion: "crewon.agent-version-release-activation.v0",
      tenantId: "tenant-1",
      releaseId,
      activationId: `activate-${releaseId}`,
      previousReleaseId,
      operator: {
...
function admissionInput(
  ...
  return {
    ...
    resolveCandidateRoute:
      options.resolveCandidateRoute ?? (async () => route("default-v1")),
    prepare: (authority) => {
      options.onPrepare?.();
      ...
      const event = {
        ...
        identity: { runId },
        eventId: `workflow-created-${suffix}`,
        sequence: 1,
        occurredAt,
      ...
      return {
        workflowInputValue: {
          ...
          valueId: `root-value-${suffix}`,
          value: rootValue,
          valueDigest: rootDigest,
        },
        commit: {
          tenantId: "tenant-1",
          expectedRevision: 0,
          idempotency: {
            scope: "run-create",
            key: `run-${suffix}`,
            requestFingerprint: `run-fp-${suffix}`,
          },
          events: [event],
          outbox: [
            {
          ...
          workItems: [
            {
              ...
              payload: {
                ...
                schedulerOperationId: `scheduler-${suffix}`,
...
function officeAdmissionInput(
  ...
  return {
    ...
    idempotency: {
      scope: "office-start",
      key: `office-${suffix}`,
      requestFingerprint: `office-fp-${suffix}`,
    },
    resolveCandidateRoute:
      options.resolveCandidateRoute ?? (async () => route("default-v1")),
    prepare: (authority) => {
