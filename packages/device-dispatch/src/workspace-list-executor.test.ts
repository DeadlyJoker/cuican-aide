...
test("omits links without following them and keeps them in the scan budget", async () => {
  const result = await executorFor(
    fakeCapability([
      entry("bazel-bin", "symlink"),
      entry("junction", "reparsePoint"),
      entry("AGENTS.md", "file"),
    ]),
  ).execute(
    command({ limits: limits({ maxEntries: 3, maxScannedEntries: 3 }) }),
    new AbortController().signal,
  );

  assert.deepEqual(result.entries, [{ name: "AGENTS.md", kind: "file" }]);
});

test("fails closed on unsupported entry types", async () => {
  await assert.rejects(
    executorFor(fakeCapability([entry("escape", "other")])).execute(
      command(),
      new AbortController().signal,
    ),
    hasCode("workspace_list_entry_type_unsupported"),
  );
});
