...
export async function collectEntries(
  ...
  while (true) {
    const remaining = limits.maxScannedEntries - scannedEntries;
    if (remaining < 1) {
    ...
    for (const candidate of page.entries) {
      scannedEntries += 1;
      if (
        hasExactKeys(candidate, ["kind", "nameBytes"]) &&
        (candidate.kind === "symlink" || candidate.kind === "reparsePoint")
      ) {
        if (
          !(candidate.nameBytes instanceof Uint8Array) ||
          candidate.nameBytes.byteLength < 1 ||
          candidate.nameBytes.byteLength > limits.maxNameBytes
        ) {
          throw new WorkspaceListExecutionError(
            "workspace_list_entry_name_invalid",
          );
        }
        scannedNameBytes += candidate.nameBytes.byteLength;
    ...
    if (
      ...
      scannedEntries >= limits.maxScannedEntries
    ) {
      throw new WorkspaceListExecutionError(
        "workspace_list_scan_limit_exceeded",
      );
