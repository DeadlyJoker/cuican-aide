...
export type {
  ResponsesIdentityPolicy,
  ResponsesSequencePolicy,
} from "./responses-protocol.ts";
...
export class DirectResponsesTransport implements ModelTransportPort {
  ...
  readonly #identityPolicy: ResponsesIdentityPolicy;
  readonly #outputDeltaFlushBytes: number | undefined;
  ...
  constructor(
    ...
    this.#identityPolicy = config.identityPolicy ?? "strict";
    if (
      this.#identityPolicy !== "strict" &&
      this.#identityPolicy !== "terminalAuthoritative"
    ) {
      throw protocolError("responses_identity_policy_invalid");
    }
    this.#outputDeltaFlushBytes =
      config.outputDeltaFlushBytes === undefined
        ? undefined
        : positiveInteger(
            config.outputDeltaFlushBytes,
            "responses_output_delta_flush_bytes_invalid",
          );
  ...
  async *stream(
    ...
    try {
      ...
      const events = responsesProtocolEvents(response.body, {
        ...
        finalOutputPolicy:
          this.#requestProfile === "responsesLite"
            ? "streamAuthoritativeWhenEmpty"
            : "strict",
        onActivity: () => idle.touch(),
        ...(this.#outputDeltaFlushBytes === undefined
          ? {}
          : { outputDeltaFlushBytes: this.#outputDeltaFlushBytes }),
