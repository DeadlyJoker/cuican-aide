...
test("accepts a bounded terminal identity from a portable Responses gateway", async () => {
  const transport = new DirectResponsesTransport(
    {
      ...
      identityPolicy: "terminalAuthoritative",
    },
    {
      fetch: async () =>
        responseStream([
          {
            ...
            response: { id: "resp-gateway-created" },
          },
          { type: "response.output_text.delta", delta: "gateway output" },
          {
            type: "response.completed",
            response: {
              id: "resp-gateway-completed",
              status: "completed",
              output: [
                {
                  type: "message",
...
test("coalesces small portable gateway deltas before the durable Kernel boundary", async () => {
  const transport = new DirectResponsesTransport(
    {
      endpoint: "https://gateway.example/v1/responses",
      model: "gateway-model",
          ...
          { type: "response.output_text.delta", delta: "e" },
          {
            ...
            response: {
              ...
              output: [
                {
                  type: "message",
                  content: [{ type: "output_text", text: "abcde" }],
                },
              ],
            },
          },
  ...
  assert.deepEqual(await collect(transport.stream(manualRequest(), signal())), [
    { type: "output.delta", delta: "abcd" },
    { type: "output.delta", delta: "e" },
    { type: "completed", checkpoint: null },
  ]);
});
