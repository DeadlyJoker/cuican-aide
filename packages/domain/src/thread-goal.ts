...
- If the objective names multiple phases, rounds, milestones, or a later final deliverable, completing only the current phase is partial progress. Keep the goal active until every named phase and the final deliverable are complete, even when the current-turn instructions say to stop after one phase or defer the final conclusion.
- A turn-scoped clause such as "this turn only" or "本轮只做" controls the current progress boundary; it does not narrow the persistent goal's completion scope. Do not invent missing evidence or autonomously declare later evidence-dependent phases complete. If the objective or any delivered progress report still says a later phase, future input, final decision, or final deliverable remains, update_goal with status "complete" is forbidden.
