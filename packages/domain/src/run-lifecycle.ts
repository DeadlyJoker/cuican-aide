...
export type RunLifecycleEvent =
  | (RunEventBase & {
      ...
      data: Readonly<{
        ...
        /** Defers automatic Goal continuation after this Run; absent means automatic. */
...
export type RunState = Readonly<{
  ...
  origin?: AutomationInvocationOrigin | null;
  goalBinding: RunGoalBinding | null;
  /** Absent on ordinary and legacy Runs; deferred Runs leave an active Goal user-driven. */
  goalContinuationMode?: "deferred";
...
function createRun(event: RunLifecycleEvent): RunState {
  ...
  if (
    event.data.goalContinuationMode !== undefined &&
    event.data.goalContinuationMode !== "deferred"
  ) {
    throw new RunLifecycleError("goal_continuation_mode_invalid");
  }
  if (
    event.data.goalContinuationMode === "deferred" &&
    (event.data.collaborationMode !== "default" ||
    ...
    throw new RunLifecycleError("deferred_goal_run_invalid");
  }
  if (
  ...
  return {
    ...
    ...(event.data.goalContinuationMode === undefined
      ? {}
      : { goalContinuationMode: event.data.goalContinuationMode }),
    goalAccounting:
      event.data.goalBinding === null
        ? null
        : createRunGoalAccountingCursor(
