import {
  ...
  type ModelHistoryMessageBackedItem,
} from "@crewon/domain";
...
export type ThreadForkHistoryProjection = Readonly<{
  ...
  rawMessageItems: readonly ModelHistoryMessageBackedItem[];
}>;
...
export function projectThreadForkHistory(
  ...
  return {
    ...
    rawMessageItems: rawItems.filter(
      (item): item is ModelHistoryMessageBackedItem =>
        item.sequence <= lastSourceSequence &&
...
export function selectThreadForkMessages(
  ...
  const survivingSequences = new Set(
    projection.items
      .filter(isModelHistoryMessageBacked)
  ...
  for (const [index, item] of projection.rawMessageItems.entries()) {
    ...
    if (
      ...
      !messageOriginMatchesHistory(item, message)
    ) {
      throw new ApplicationError("internal", "thread_message_history_mismatch");
    }
    const survives = survivingSequences.has(item.sequence);
    if (
      survives
        ? message.invalidation !== null
        : message.invalidation === null ||
          message.invalidation === undefined ||
          message.invalidation.historySequence !== item.sequence
    ) {
      throw new ApplicationError("internal", "thread_message_history_mismatch");
    }
    if (survives) {
...
function messageOriginMatchesHistory(
  item: ModelHistoryMessageBackedItem,
  ...
  return (
    ...
    canonicalJson(message.origin) === canonicalJson(item.origin)
  );
}
