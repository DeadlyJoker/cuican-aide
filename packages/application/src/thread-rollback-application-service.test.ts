...
test("commits an overshoot rollback from bounded raw history", async () => {
  const fixture = threadFixture([
    ...
    automationHistoryMessage(3, "second"),
...
class RecordingRollbackStore implements ThreadStore, ThreadRollbackStore {
  ...
  async loadThreadInSpace(
    ...
    const thread = await this.loadThread(locator);
...
function invalidatedMessages(
  ...
  const historyMessages = fixture.history.filter(isModelHistoryMessageBacked);
...
function automationHistoryMessage(
  sequence: number,
  content: string,
): Extract<
  ModelHistoryItem,
