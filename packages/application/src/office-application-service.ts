...
type OfficeDependencies = Readonly<{
...
export class OfficeApplicationService {
  readonly #dependencies: OfficeDependencies;
  constructor(dependencies: OfficeDependencies) {
  ...
  async create(actor: ActorContext, command: CreateOfficeCommand) {
    ...
    for (const agentVersionId of referencedAgentVersions(command)) {
      if (
        (await this.#dependencies.store.loadAgentVersion({
        ...
        throw new ApplicationError(
          "validation",
          "office_agent_version_not_published",
        );
      }
    }
  ...
  async get(
    ...
    validateActor(actor);
    requireText(officeVersionId, 128, "office_version_id_invalid");
    const definition = await this.#dependencies.store.loadOfficeDefinition({
      tenantId: actor.tenantId,
      spaceId: actor.spaceId,
      officeVersionId,
    });
    if (definition === null)
      throw new ApplicationError("notFound", "office_not_found");
  ...
  async list(
    actor: ActorContext,
    query: { before: OfficeListCursor | null; limit: number },
  ) {
    validateActor(actor);
    if (
      !Number.isSafeInteger(query.limit) ||
      query.limit < 1 ||
      query.limit > OFFICE_LIMITS.list
    ) {
      throw new ApplicationError("validation", "office_list_limit_invalid");
    }
    await this.authorize(actor, "office:list", null, null);
    return this.#dependencies.store.listOfficeDefinitions({
      tenantId: actor.tenantId,
      spaceId: actor.spaceId,
      ...query,
    });
  ...
  async authorizeRun(
    ...
    const target = definition.executionTargets.find(
      (candidate) => candidate.targetId === targetId,
    );
    if (target === undefined)
      throw new ApplicationError("notFound", "office_target_not_found");
    return target;
  }

  private async authorize(
    actor: ActorContext,
    action: OfficeAuthorizationAction,
    officeId: string | null,
...
function validateActor(actor: ActorContext) {
  for (const value of [
    ...
    requireText(value, 128, "actor_invalid");
}
function validateCreate(command: CreateOfficeCommand) {
  if (
    ...
    throw new ApplicationError("validation", "office_revision_invalid");
  requireText(command.idempotencyKey, 200, "idempotency_key_invalid");
  requireText(command.title, OFFICE_LIMITS.title, "office_title_invalid");
  if (command.officeId !== null)
    requireText(command.officeId, 128, "office_id_invalid");
  if (
    command.members.length > OFFICE_LIMITS.members ||
    command.executionTargets.length < 1 ||
    command.executionTargets.length > OFFICE_LIMITS.targets
  )
    throw new ApplicationError(
      "validation",
  ...
  for (const member of command.members) {
    requireText(member.memberId, 128, "office_member_invalid");
    requireText(member.displayName, 160, "office_member_invalid");
    requireText(member.agentVersionId, 128, "office_member_invalid");
    if (ids.has(member.memberId))
      throw new ApplicationError("validation", "office_member_duplicate");
    ids.add(member.memberId);
  }
  ids.clear();
  for (const target of command.executionTargets) {
    requireText(target.targetId, 128, "office_target_invalid");
    requireText(target.agentVersionId, 128, "office_target_invalid");
    if (ids.has(target.targetId))
      throw new ApplicationError("validation", "office_target_duplicate");
    ids.add(target.targetId);
  }
}
function referencedAgentVersions(command: CreateOfficeCommand) {
  return new Set([
    ...command.members.map((value) => value.agentVersionId),
    ...command.executionTargets.map((value) => value.agentVersionId),
  ]);
}
...
function mapStoreError(error: unknown) {
  const code =
    error instanceof Error ? error.message : "office_store_unavailable";
  if (code.includes("conflict") || code.includes("idempotency"))
    return new ApplicationError("conflict", code);
  return new ApplicationError("internal", "office_store_unavailable", {
    cause: error,
  });
}
