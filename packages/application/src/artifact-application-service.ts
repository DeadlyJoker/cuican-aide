...
export class ArtifactApplicationService implements ToolOutputArtifactPort {
  ...
  async getArtifact(
    ...
    return (await this.#authorizedArtifact(actor, artifactId, false)).record;
  }
  ...
  async readArtifact(
    ...
    return this.#authorizedArtifact(actor, artifactId, true);
  }

  async #authorizedArtifact(
    actor: ActorContext,
    artifactId: string,
    includeContent: boolean,
  ): Promise<ArtifactContent> {
    requireOpaqueId(artifactId, "artifact_id_invalid");
    const stored = includeContent
      ? await this.#store.read({ tenantId: actor.tenantId, artifactId })
      : await metadataContent(this.#store, actor.tenantId, artifactId);
    if (stored === null || stored.record.spaceId !== actor.spaceId) {
      throw new ApplicationError("notFound", "artifact_not_found");
    }
    const decision = await this.#authorization.authorize({
      ...
      resource: {
        ...
        spaceId: stored.record.spaceId,
        artifactId: stored.record.artifactId,
        ownerActorId: stored.record.ownerActorId,
        runId: stored.record.source.runId,
      },
    });
    if (decision.outcome !== "allow") {
      throw new ApplicationError("authorization", decision.reasonCode);
    }
    if (this.#expired(stored.record)) {
      throw new ApplicationError("notFound", "artifact_expired");
    }
    if (includeContent && stored.record.scan.status === "blocked") {
      throw new ApplicationError("notFound", "artifact_not_found");
    }
    return {
...
async function metadataContent(
  ...
  const record = await store.get({ tenantId, artifactId });
  return record === null ? null : { record, content: new Uint8Array() };
}
