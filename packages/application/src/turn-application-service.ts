...
export class TurnApplicationService {
  ...
  async startTurn(
    ...
    const runEvent: Extract<RunLifecycleEvent, { type: "run.created" }> = {
      ...
      data: {
        ...
        ...(command.executionIntent === "resumeGoal"
          ? { goalContinuationMode: "deferred" as const }
          : {}),
      },
    };
  ...
  #prepareGoal(
    ...
    if (command.executionIntent === "resumeGoal") {
      if (current === null || current.status !== "paused") {
        throw new ApplicationError(
          "validation",
          "goal_resume_requires_paused_goal",
        );
      }
      ...
      return {
        ...
        binding: goalBinding(next, this.#digest(next.objective)),
      };
    }
