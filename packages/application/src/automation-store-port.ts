...
export type ScheduledAutomationListQuery = Readonly<{
  before: AutomationListCursor | null;
  limit: number;
}>;
...
export interface AutomationStore {
  ...
  /** Internal scheduler scan across tenants; callers must not expose it as a user query. */
...
export function validateAutomationCreateCommand(
  ...
  requireAutomationExactObject(
    ...
    [
      ...
      ...(command.executionMode === undefined ? [] : ["executionMode"]),
      "idempotencyKey",
      "kind",
      "prompt",
  ...
  if (
    ...
    command.executionMode !== "manualOnly" &&
    command.executionMode !== "scheduled"
  ) {
    throw new ApplicationError(
      "validation",
      "automation_create_command_invalid",
