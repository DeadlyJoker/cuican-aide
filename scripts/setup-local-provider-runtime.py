...
MIN_RSA_BITS = 2048
DEFAULT_AGENT_PLATFORM_URL = "http://127.0.0.1:8002"
DEFAULT_PERMISSION_SERVICE_URL = "http://127.0.0.1:8010"

AGENT_PLATFORM_REQUIRED_FILES = (
    "backend/app/main.py",
    "backend/app/modules/identity_source/production_composition.py",
    "backend/app/modules/provider_run/production_composition.py",
    "contracts/identity/v1/provider_identity_source.v1.json",
    "contracts/provider/v3/provider_contract.v3.json",
    "contracts/provider/v3/provider_dynamic_execution.v3.json",
)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--agent-platform-url",
        default=DEFAULT_AGENT_PLATFORM_URL,
        help=(
            "Provider v3 Agent Platform origin "
            f"(default: {DEFAULT_AGENT_PLATFORM_URL})."
        ),
    )
    parser.add_argument(
        "--permission-service-url",
        default=DEFAULT_PERMISSION_SERVICE_URL,
        help=(
            f"Permission service origin (default: {DEFAULT_PERMISSION_SERVICE_URL})."
        ),
    )
    parser.add_argument(
        "--check",
...
def require_checkout(path: Path) -> Path:
    ...
    missing = [
        relative
        for relative in AGENT_PLATFORM_REQUIRED_FILES
        if not (resolved / relative).is_file()
    ]
    if missing:
        raise HarnessError(
            "Agent Platform checkout does not provide the Provider v3 harness: "
            f"{resolved} (missing {', '.join(missing)})"
        )
    main = (resolved / "backend" / "app" / "main.py").read_text(encoding="utf-8")
    for mount in ('app.mount("/identity/v1"', 'app.mount("/provider/v3"'):
        if mount not in main:
            raise HarnessError(
                "Agent Platform checkout does not mount the Provider v3 harness: "
                f"{resolved} (missing {mount})"
            )
    return resolved
...
def agent_platform_environment(runtime_root: Path, permission_service_url: str) -> str:
...
export PERMISSION_SERVICE_URL=__PERMISSION_SERVICE_URL__
...
# The focused local harness does not start Agent Platform's optional observability
# or enterprise-channel dependencies. Disable their background loops explicitly
# instead of producing permanent Kafka/Elasticsearch/Milvus connection retries.
export TRACE_DISABLED=true
export HEALTH_SCHEDULER_DISABLED=true
export APPLICATION_CHANNEL_WORKER_DISABLED=true
export WECHAT_WORK_BOT_RUNTIME_DISABLED=true

unset _crewon_provider_runtime_dir
"""
    return template.replace("__CREWON_PROVIDER_RUNTIME_DIR__", runtime_dir).replace(
        "__PERMISSION_SERVICE_URL__", shlex.quote(permission_service_url)
    )


def crewon_environment_block(runtime_root: Path, agent_platform_url: str) -> str:
    ...
    lines = [
        ...
        assignment("CREWON_IDENTITY_SOURCE_URL", f"{agent_platform_url}/identity/v1/"),
        ...
        assignment(
            "CREWON_PROVIDER_AGENT_PLATFORM_URL",
            f"{agent_platform_url}/provider/v3/",
        ),
        assignment(
...
def ensure_gitignore_entry(path: Path) -> None:
    ...
    missing = [entry for entry in required if not ignored_by_entry(entries, entry)]
    if not missing:
        return
...
def validate_gitignore(path: Path) -> None:
    ...
    for required in (".crewon/dev/", ".crewon/dev-ui.env"):
        if not ignored_by_entry(entries, required):
            raise HarnessError(f".gitignore is missing {required}")


def ignored_by_entry(entries: list[str], required: str) -> bool:
...
def validate_crewon_dev_script(path: Path) -> None:
    ...
    if 'join(repositoryRoot, ".crewon", "dev-ui.env")' not in content:
        raise HarnessError(
            "scripts/crewon-control-web-dev.mjs does not load .crewon/dev-ui.env"
        )
    if "agent-platform.env.sh" in content:
        raise HarnessError(
            "scripts/crewon-control-web-dev.mjs loads a server-side environment file"
        )


def generate(
    repo_root: Path,
    agent_platform_root: Path,
    ...
    atomic_write(
        ...
        agent_platform_environment(runtime_root, permission_service_url).encode(
            "utf-8"
        ),
        0o600,
    )
    ...
    updated = replace_controlled_block(
        existing,
        crewon_environment_block(runtime_root.resolve(), agent_platform_url),
    )
...
def check(
    ...
    if agent_env.read_text(encoding="utf-8") != agent_platform_environment(
        runtime_root, permission_service_url
    ):
    ...
    validate_crewon_dev_script(repo_root / "scripts" / "crewon-control-web-dev.mjs")
    ...
    try:
        if arguments.check:
            check(
                repo_root,
                agent_platform_root,
                arguments.agent_platform_url.rstrip("/"),
                arguments.permission_service_url.rstrip("/"),
            )
