...
if defined CREWON_BAZEL_TEST_SKIP_FILTERS (
...
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "$skipFilters = @();" ^
  "$skipFiltersSource = $env:CREWON_BAZEL_TEST_SKIP_FILTERS; if ([string]::IsNullOrEmpty($skipFiltersSource)) { $skipFiltersSource = $env:CODEX_BAZEL_TEST_SKIP_FILTERS };" ^
  "if (-not [string]::IsNullOrEmpty($skipFiltersSource)) { $skipFilters = @($skipFiltersSource -split ',' | Where-Object { $_ -ne '' }) };" ^
