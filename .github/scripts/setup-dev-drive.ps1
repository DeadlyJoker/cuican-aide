...
if (Test-Path "D:\") {
    ...
    try {
        $VhdPath = Join-Path $env:RUNNER_TEMP "crewon-dev-drive.vhdx"
        $SizeBytes = 64GB
        ...
        $Volume = $Partition | Format-Volume -FileSystem ReFS -NewFileSystemLabel "CrewonDevDrive" -DevDrive -Confirm:$false -Force -ErrorAction Stop
