...
4. Fix behavior in `CommandWorkspace.tsx` when state or interactions are wrong.
5. Fix layout in both `app.css` and `original-shell-overrides.css` when fullscreen shell styles are wrong.
6. Keep real agent-platform data limited to approved design slots; do not replace primary artifact copy unless explicitly requested.
...
- no flicker occurs after platform data loads or after switching shell views
