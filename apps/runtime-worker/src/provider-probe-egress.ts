...
/**
 * Read-only desktop catalog policy with narrow support for transparent-proxy
 * Fake-IP DNS. TLS still authenticates the configured hostname, while literal
 * IP endpoints and every ordinary private address remain denied.
 */
export class DesktopModelCatalogEgressPolicy
  implements ProviderProbeEgressPolicy
{
  authorize(input: ProviderProbeEgressInput): ProviderProbeEgressDecision {
    const hostname = input.endpoint.hostname
      .toLowerCase()
      .replace(/^\[|\]$/gu, "");
    const publicOrProxyHttps =
      input.endpoint.protocol === "https:" &&
      isIP(hostname) === 0 &&
      input.addresses.every(
        ({ address }) =>
          providerAddressKind(address) === "public" ||
          benchmarkProxyAddress(address),
      );
    const loopbackEndpoint =
...
function benchmarkProxyAddress(address: string): boolean {
