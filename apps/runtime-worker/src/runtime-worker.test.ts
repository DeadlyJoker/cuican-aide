...
test("never resurrects rolled-back turns across successive compactions", async (context) => {
  ...
  const replacement = await appendAutomationInvocationAndRun(
    fixture.automations,
    fixture.store,
    fixture.threadId,
  ...
  const replacementInstruction = replacement.message.content;
  ...
  assert.deepEqual(await replacementWorker.wake(), {
    kind: "completed",
    runId: replacement.runState.runId,
  });
    ...
    [
      {
        ...
        retainedUserMessages: ["surviving turn", replacementInstruction],
      },
      {
        sequence: 12,
        mode: "auto",
        replacesThroughSequence: 11,
        retainedUserMessages: [replacementInstruction, "surviving tail"],
      },
    ],
  );
  const replacementHistory = history.find(
    (item) =>
      item.type === "message" && item.content === replacementInstruction,
  );
  assert.equal(replacementHistory?.type, "message");
  assert.equal(
    replacementHistory?.type === "message" ? replacementHistory.source : null,
    "automation_invocation",
  );
  assert.deepEqual(
    replacementHistory?.type === "message" &&
      replacementHistory.source === "automation_invocation"
      ? replacementHistory.origin
      : null,
    replacement.runState.origin,
  );
  assert.equal(
  ...
  assert.deepEqual(
    ...
    [
      { type: "message", role: "user", content: replacementInstruction },
...
test("fails closed when an Automation Work binding route diverges from its canonical Run", async (context) => {
  const fixture = await createFixture(
    context,
    (clock) => new InMemoryRunStore({ clock }),
  );
  const baselineWorker = fixture.worker({ transport: successfulTransport() });
  assert.deepEqual(await baselineWorker.wake(), {
    kind: "completed",
    runId: fixture.runId,
  });
  ...
  const invocation = await appendAutomationInvocationAndRun(
    fixture.automations,
    fixture.store,
    fixture.threadId,
    "route-bound automation",
    "invalid-route",
  );
  assert.ok(invocation.runState.origin !== null);
  assert.ok(invocation.runState.origin !== undefined);
  const transport = successfulTransport();
  const worker = fixture.worker({
    transport,
    claimStore: automationInvocationClaimStore(fixture.store, {
      ...invocation.runState.origin.binding,
  ...
  assert.deepEqual(await worker.wake(), {
    kind: "failed",
    runId: invocation.runState.runId,
    code: "automation_invocation_payload_invalid",
  });
...
test("projects AR-039 generic Responses terminals through the durable retry boundary", async (context) => {
  const reference = JSON.parse(
    readFileSync(
      new URL(
  ...
  for (const fixtureCase of reference.cases) {
    ...
    assert.equal(fixtureCase.expected.durableRetryProjection.sampling, "retry");
    assert.deepEqual(await worker.wake(), {
      kind: "failed",
      runId: fixture.runId,
    ...
    assert.deepEqual((await fixture.loadRun()).failure, {
      code: fixtureCase.expected.code,
      retryable:
        fixtureCase.expected.durableRetryProjection.afterBudgetExhausted !==
        "fail",
    });
    assert.deepEqual((await fixture.attempts()).map(attemptSummary), [
      {
...
function registerRuntimeWorkerConformance(
  ...
  describe(name, () => {
    ...
    test("adopts a different prepared Tool approval without Provider side effects", async (context) => {
      ...
      const providerCalls = { execute: 0, reconcile: 0, cancel: 0 };
      const toolRuntime: ToolRuntimePort = {
        definitions: () => [
          {
            schemaVersion: "crewon.tool-definition.v0",
      ...
      const claim = await fixture.store.claimNextWorkItem({
        ...
        leaseId: "replacement-seed-lease",
        leaseDurationMs: 10_000,
      });
      ...
      assert.notEqual(
        adopted.kind === "waitingApproval" ? adopted.approvalId : null,
        oldApproval.approvalId,
      );
      const waiting = await fixture.loadRun();
      assert.equal(waiting.status, "waitingApproval");
      const replacementApprovalId = waiting.waitingApproval?.approvalId;
      assert.ok(replacementApprovalId !== undefined);
      assert.equal(
        (await fixture.approvals.getApproval(actor(), oldApproval.approvalId))
          .status,
        "superseded",
      );
      assert.equal(
        (await fixture.approvals.getApproval(actor(), replacementApprovalId))
          .status,
        "required",
      );
      assert.equal(
        (await fixture.approvals.getApproval(actor(), replacementApprovalId))
          .receiptId,
        replacementReceipt.receipt.receiptId,
      );
      assert.deepEqual(await fixture.store.listPendingWorkItems(10), []);
      assert.deepEqual(providerCalls, { execute: 0, reconcile: 0, cancel: 0 });
      const requiredEvents = (await fixture.events()).filter(
        (event) => event.type === "run.approval.required",
      );
...
async function createFixture(
  ...
  const automations = new AutomationApplicationService({
    store: store as DomainStore & AutomationStore,
    authorization,
    clock,
    ids,
    digester: new Sha256Digester(),
    routeResolver: { resolveRoute: async () => route },
  });
  ...
  return {
    ...
    worker: (options: {
      ...
      store?: DomainStore;
      claimStore?: DomainStore;
    }) => {
      const workerStore = options.store ?? options.claimStore ?? store;
...
async function appendAutomationInvocationAndRun(
  ...
  assert.ok(thread !== null);
  const created = await automations.createAutomation(actor(), {
    kind: "automation.create",
    idempotencyKey: `automation-create-${idempotencySuffix}`,
    threadId,
    expectedThreadRevision: thread.revision,
    title: "Compaction replacement",
    prompt: content,
    requestedAgentVersionId: ROUTE.agentVersionId,
    schedule: {
      scheduleType: "daily",
  ...
  return automations.runAutomationNow(actor(), {
    kind: "automation.runNow",
    idempotencyKey: `automation-run-${idempotencySuffix}`,
...
function automationInvocationClaimStore(
  store: DomainStore,
  binding: AutomationInvocationBinding,
): DomainStore {
  return new Proxy(store, {
    get(target, property) {
      if (property === "claimNextWorkItem") {
        return async (
          input: Parameters<DomainStore["claimNextWorkItem"]>[0],
        ) => {
          const claim = await target.claimNextWorkItem(input);
