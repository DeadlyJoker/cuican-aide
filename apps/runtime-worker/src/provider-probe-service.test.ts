...
test("checks active Provider authority before and after the network request", async () => {
  ...
  assert.equal(
    (await service.probe(probeRequest(), new AbortController().signal)).status,
    "bindingMismatch",
  );
...
test("rejects an old Worker generation after same-provider credential rotation", async () => {
  let networkCalls = 0;
  const rotated = {
    ...state(),
    catalog: {
      ...state().catalog!,
      revision: 4,
      runtimeBindingId: "desktop-supervisor:generation-8",
    },
  };
  const service = serviceWith(fakeStore([rotated]), {
    createProbe: () => {
      networkCalls += 1;
      return { probe: async () => okResult() };
    },
  ...
  const result = await service.probe(
    {
      ...
      expectedRuntimeBindingId: "desktop-supervisor:generation-8",
    },
    new AbortController().signal,
  );
  assert.equal(result.status, "bindingMismatch");
  assert.equal(result.runtimeBindingId, "desktop-supervisor:generation-7");
  assert.equal(networkCalls, 0);
});
...
test("enforces per-binding concurrency and rate limits", async () => {
  ...
  const first = service.probe(probeRequest(), new AbortController().signal);
...
test("the total deadline bounds a hung secret resolver", async () => {
  ...
  const result = service.probe(probeRequest(), new AbortController().signal);
...
test("fails closed when the rate-limit clock is invalid or moves backward", async () => {
  ...
  await assert.rejects(
    service.probe(probeRequest(), new AbortController().signal),
...
function fakeStore(
  ...
  return {
    loadModelProviderSettingsState: async () =>
      load?.() ??
      structuredClone(values[Math.min(index++, values.length - 1)]!),
