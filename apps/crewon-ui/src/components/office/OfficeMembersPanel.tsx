...
import { officeMemberRuntimeLabel } from "../../lib/office/officeMemberRuntimeLabels";
...
/**
 * Options for the recruit-agent select. Extracted so the identity-upgrade
 * hint on legacy members stays directly testable — the select menu only
 * renders while open, so static markup no longer exposes option details.
 */
export function recruitableAgentOptions({
  agents,
  isZh,
  members,
}: {
  ...
  return agents.flatMap((agent) => {
    ...
    const needsIdentityUpgrade = members.some(
      (member) => member.agentId === agent.agentId && !member.memberId?.trim(),
    );
    return [
      {
        detail: `${agent.role}${
        ...
        <div className="office-recruit">
          ...
          {agents.length > 0 ? (
            <CommandComposerSelect
              ...
              className="h-[var(--control-h-lg)] w-full max-w-none"
              disabled={pendingActionId !== null}
              options={recruitableAgentOptions({
                agents,
                isZh,
