  ...
  it("renders only supported actions for Control Knowledge", () => {
    const controlPanel: LibraryPanel = {
      ...
      knowledge: {
        ...
        sources: [
          {
            knowledgeId: "knowledge-source-1",
            name: "Control handbook",
            glyph: "H",
            accent: "green",
            status: "stored",
    ...
    expect(markup).toContain("Write memory");
    expect(markup).toContain("Refresh knowledge");
    expect(markup).toContain("Stored");
    expect(markup).not.toContain("Indexed");
    expect(markup).not.toContain("Reset global memory");
