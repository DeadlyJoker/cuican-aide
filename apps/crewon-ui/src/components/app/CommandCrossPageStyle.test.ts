 ...
 * trailing controls -- but they had drifted apart on every axis. Both room rows
 * now use a 64px border-box, so their vertical padding is part of the declared
 * height instead of silently expanding it to roughly 84px.
 */
describe("team room header", () => {
  ...
  it("counts header padding inside the shared 64px floor", () => {
    const sizing: Record<string, string | undefined> = {};

    for (const file of SHELL_FILES) {
      for (const rule of rulesOf(file)) {
        if (rule.atRules.length > 0) continue;
        if (!/^(\.office-top|\.workflow-room-top)$/.test(rule.selector))
          continue;
        sizing[rule.selector] = rule.body
          .match(/box-sizing:\s*([^;]+)/)?.[1]
          .trim();
      }
    }

    expect(sizing).toEqual({
      ".office-top": "border-box",
      ".workflow-room-top": "border-box",
    });
  });
