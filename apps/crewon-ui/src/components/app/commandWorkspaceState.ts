...
const THREAD_EXECUTION_TARGET_STORAGE_KEY =
  "crewon.command.thread-execution-target.v1";
const MAX_STORED_THREAD_EXECUTION_TARGETS = 200;

type ExecutionTargetStorage = Pick<Storage, "getItem" | "setItem">;

export function readThreadExecutionTarget(
  storage: ExecutionTargetStorage,
  threadId: string,
): string | null {
  if (!validStoredExecutionTargetKey(threadId)) {
    return null;
  }
  try {
    const entries = parseThreadExecutionTargets(
      storage.getItem(THREAD_EXECUTION_TARGET_STORAGE_KEY),
    );
...
export function writeThreadExecutionTarget(
  storage: ExecutionTargetStorage,
  threadId: string,
  executionTarget: string,
): void {
  if (
    !validStoredExecutionTargetKey(threadId) ||
    !validStoredExecutionTarget(executionTarget)
  ) {
    return;
  }
  try {
    const current = parseThreadExecutionTargets(
      storage.getItem(THREAD_EXECUTION_TARGET_STORAGE_KEY),
    );
    const next = Object.fromEntries(
      Object.entries({ ...current, [threadId]: executionTarget }).slice(
        -MAX_STORED_THREAD_EXECUTION_TARGETS,
      ),
    );
    storage.setItem(THREAD_EXECUTION_TARGET_STORAGE_KEY, JSON.stringify(next));
  } catch {
    // Storage can be unavailable in hardened browser contexts. Thread
    // execution still works; only cross-reload restoration is skipped.
  }
}

function parseThreadExecutionTargets(
    ...
    Object.entries(value)
      .filter(
        (entry): entry is [string, string] =>
          validStoredExecutionTargetKey(entry[0]) &&
          validStoredExecutionTarget(entry[1]),
      )
      .slice(-MAX_STORED_THREAD_EXECUTION_TARGETS),
  );
}

function validStoredExecutionTargetKey(value: string): boolean {
  return value.length > 0 && value.length <= 128 && !/[\u0000-\u001f]/u.test(value);
}

function validStoredExecutionTarget(value: unknown): value is string {
  return (
    typeof value === "string" &&
    value.length > 0 &&
    value.length <= 512 &&
...
export function shouldResetExecutionTarget(input: {
  catalogStatus: "loading" | "ready" | "unavailable";
  optionAvailable: boolean;
  selectedProviderAgentMatches: boolean;
}): boolean {
  return (
    input.catalogStatus === "ready" &&
    (!input.optionAvailable || !input.selectedProviderAgentMatches)
  );
}
