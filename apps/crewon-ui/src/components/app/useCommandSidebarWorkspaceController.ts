...
export function useCommandSidebarWorkspaceController({ accountId, addWorkspaceLabel, cwd, linkedThreads, noWorkspaceLabel, onCreateWorkspace, workspaceAuthority }: { accountId: string | null; addWorkspaceLabel: string; cwd: string; linkedThreads: CommandLinkedThread[]; noWorkspaceLabel: string; onCreateWorkspace?: (cwd: string) => void; workspaceAuthority: CommandWorkspaceAuthority }) {
  ...
  /*
   * `storedWorkspaces` is reloaded asynchronously when the account (or
   * authority) changes. Until that load lands, the derived `workspaces` list is
   * built from the previous account's state plus thread folders (usedAt 0), and
   * persisting it would clobber the real roster with zeroed timestamps. This
   * flag marks which account the state belongs to; the write effect below
   * refuses to persist anything older.
   */
  const [rosterLoadedKey, setRosterLoadedKey] = useState(
    () => `${workspaceAuthority}:${accountId ?? "anonymous"}`,
  );
  ...
  useEffect(() => {
    ...
    // Batched with the roster load, so the next render's derived list and this
    // marker always describe the same account.
    setRosterLoadedKey(`${workspaceAuthority}:${accountId ?? "anonymous"}`);
  ...
  useEffect(() => {
    if (
      rosterLoadedKey !== `${workspaceAuthority}:${accountId ?? "anonymous"}`
    ) {
      return;
    }
  ...
  }, [accountId, workspaceAuthority, workspacePathsKey, rosterLoadedKey]);
  ...
  const currentWorkspaceCollapsed =
    workspaceAuthority === "legacy"
      ...
      : collapsedWorkspaceGroups.has("current");
