...
describe("CrewON resource catalog MVP", () => {
  ...
  it("snapshots employee, skill, service, and knowledge cards", () => {
    ...
    expect(markup).toContain(
      'class="catalog-market-header knowledge-catalog-header"',
    );
    expect(markup).toContain('class="knowledge-catalog-title"');
    expect(markup).toContain("本地 0 · 云端 1");
  ...
  it("snapshots active local TypeScript AgentVersions as runnable agents", () => {
    const markup = renderToStaticMarkup(
      <AgentsView
        active
        ...
        localAgents={[
          {
            filePath: "control:agent-version:local-web-executor",
            config: {
              agentId: "local-web-executor",
              name: "本地执行 Agent",
              role: "本地 TypeScript Runtime · gpt-5.5",
              glyph: "L",
              accent: "cyan",
              model: "gpt-5.5",
              models: ["gpt-5.5"],
    ...
    expect(markup).toContain("本地执行 Agent");
    expect(markup).toContain("本地 TypeScript Runtime · gpt-5.5");
    expect(markup).toContain("可运行");
    expect(markup).toMatchSnapshot();
  });
