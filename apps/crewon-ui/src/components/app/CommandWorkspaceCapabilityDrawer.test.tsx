...
describe("CommandWorkspaceCapabilityDrawer", () => {
  ...
  it("offers apps and plugins from the launcher instead of a browser tab", () => {
    const markup = renderToStaticMarkup(
      <CommandWorkspaceCapabilityDrawer
        busyToolId={null}
        commandValue=""
    ...
    expect(markup).toContain('data-tool-id="apps"');
    expect(markup).toContain("应用与插件");
    expect(markup).not.toContain('data-tool-id="panel"');
  });

  it("snapshots the browser, terminal, and review workbench surfaces", () => {
    const surfaces = {
