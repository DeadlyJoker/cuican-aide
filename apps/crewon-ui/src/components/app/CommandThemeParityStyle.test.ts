...
function darkSendButtonContract(source: string) {
  return Array.from(source.matchAll(/([^{}]+)\{([^{}]*)\}/g))
    .filter(
      ([, selector]) =>
        ...
        selector
          .split(",")
          .some((part) => /\.input-tools\s+\.send-button/.test(part)),
    )
    .map(([, selector, declarations]) => {
      const states = selector
        .split(",")
        .map((part) => {
          const isInteractive = /:(?:hover|focus-visible)/.test(part);
          const isStop = part.includes('[data-action="stop"]');

          if (isStop && isInteractive) return "stop-interactive";
          if (isInteractive) return "interactive";
          if (part.includes(":disabled")) return "disabled";
      ...
      return {
        states,
        background: declarationValue(declarations, "background"),
        color: declarationValue(declarations, "color"),
        opacity: declarationValue(declarations, "opacity") || undefined,
      };
    });
}
...
describe("theme parity", () => {
  ...
  it("sizes canvas grids against the container, not the viewport", () => {
    ...
    for (const match of source.matchAll(
      ...
      if (
        /\.(?:office-card-grid|expert-team-grid|team-office-capability-grid|team-capability-live-layout)\b/.test(
          body,
        )
      ) {
  ...
  it("keeps dark composer send icons opposite their circular surface", () => {
    const contract = Object.fromEntries(
      STYLE_FILES.map((file) => {
        const rules = darkSendButtonContract(readStyles(file));
        expect(
          rules.length,
          `missing dark send-button rules in ${file}`,
        ).toBeGreaterThan(0);

        for (const rule of rules) {
          const isInteractive = rule.states.some((state) =>
            state.includes("interactive"),
          );
          expect(rule.background, `${file}: ${rule.states.join(", ")}`).toBe(
            isInteractive ? "var(--n-0)" : "var(--n-1000)",
          );
          expect(rule.color, `${file}: ${rule.states.join(", ")}`).toBe(
            isInteractive ? "var(--n-1000)" : "var(--n-0)",
          );
        }
