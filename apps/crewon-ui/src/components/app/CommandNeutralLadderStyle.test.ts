...
/*
 * The starter cards moved below the composer, where they have to line up with it
 * to avoid looking like stray chips. An equal-column grid is what holds that
 * alignment, so a switch back to wrapping flex should fail here.
 */
describe("starter card row", () => {
  it("lays the cards out as an equal-column grid", () => {
    const contract = sliceBetween(
      readStyles("app.css"),
      ".screen-shell.command-screen .quick-row {",
      "}",
    );
  ...
  it("rests the cards on the canvas so the composer stays the raised surface", () => {
    const contract = sliceBetween(
      readStyles("original-shell-overrides.css"),
