...
export type ModelProviderDraft = {
  ...
  envKey: string;
  /** Provider model id, or a LiteLLM model alias from model_list. */
  modelId: string;
...
export function modelProviderDraftError(
  ...
  if (!draft.modelId.trim()) {
    return locale === "zh" ? "请填写模型 ID" : "Model ID is required";
  }
  if (
    new TextEncoder().encode(draft.modelId.trim()).byteLength > 256 ||
    /[\u0000-\u001f\u007f]/u.test(draft.modelId)
  ) {
    return locale === "zh" ? "模型 ID 格式无效" : "Model ID is invalid";
  }
