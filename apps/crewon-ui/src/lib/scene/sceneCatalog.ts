...
export type ExecutionTargetOption = {
   ...
   * that is the distinction users pick along: a single agent or a team.
   */
 ...
 * completes it alone, or a team reporting to a lead. Where the
...
type ExecutionTargetOfficeRecord = ExecutionTargetConfigRecord<
  Pick<OfficeConfig, "title"> & { workspace?: OfficeConfig["workspace"] }
> & {
  controlOffice?: {
    memberCount: number;
    officeVersionId: string;
    runtimeStatus: "ready" | "agentInactive" | "multiMember";
  };
};
...
export function executionTargetOptionsFromDomain({
  ...
  locale?: Locale;
  offices: ExecutionTargetOfficeRecord[];
  ...
  const teamOptions = offices.map((record) => {
    ...
    const controlOffice = record.controlOffice;
    const ready =
      controlOffice?.runtimeStatus === "ready" ||
      (controlOffice === undefined && members.length > 0 && !duplicateTitle);
    const unavailableId = encodeURIComponent(
      controlOffice?.officeVersionId ||
        record.config.workspace?.recordId?.trim() ||
        record.filePath,
    );
    return {
      detail:
        controlOffice?.runtimeStatus === "ready"
          ? locale === "zh"
            ? `${controlOffice.memberCount} 名成员 · 已就绪`
            : `${controlOffice.memberCount} ${controlOffice.memberCount > 1 ? "members" : "member"} · Ready`
          : controlOffice?.runtimeStatus === "agentInactive"
            ? locale === "zh"
              ? "成员智能体当前不可用，不能开始任务"
              : "The member agent is unavailable"
            : controlOffice?.runtimeStatus === "multiMember"
              ? locale === "zh"
                ? `已配置 ${controlOffice.memberCount} 名成员 · 团队协作暂不可用`
                : `${controlOffice.memberCount} configured members · team collaboration is unavailable`
              : duplicateTitle
                ? locale === "zh"
      ...
      value:
        controlOffice === undefined
          ? duplicateTitle
            ...
            : `team:${title}`
          : ready
            ? `team:${controlOffice.officeVersionId}`
            : `team:unavailable:${unavailableId}`,
    };
  });
  const fallback =
