...
describe("app server event handlers", () => {
  ...
  it("accepts legacy turn completion only while the legacy runtime is authoritative", () => {
    const legacyHandlers = createAppServerEventHandlers(
      createParams({ controlThreadAuthority: false }),
    );

    legacyHandlers.handleNotification({ method: "turn/completed" } as never);

    expect(notificationSpy.local).toHaveBeenCalledOnce();
    expect(notificationSpy.turn).toHaveBeenCalledOnce();

    vi.clearAllMocks();
    const controlHandlers = createAppServerEventHandlers(
      createParams({ controlThreadAuthority: true }),
    );

    controlHandlers.handleNotification({ method: "turn/completed" } as never);

    expect(notificationSpy.local).not.toHaveBeenCalled();
    expect(notificationSpy.turn).not.toHaveBeenCalled();
  });
