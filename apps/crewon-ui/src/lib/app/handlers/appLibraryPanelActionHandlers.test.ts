...
function createHandlers(
  ...
  return createAppLibraryPanelActionHandlers({
    action,
...
describe("app library panel action handlers", () => {
  beforeEach(() => {
    factorySpy.lastParams = null;
    vi.clearAllMocks();
  });

  it("fails closed inside the action factory for queued Control Automation mutations", async () => {
    const setLibraryPanel = vi.fn();
    const runAutomationConfig = vi.fn();
    const updateAutomationRun = vi.fn();
    const writeAutomationConfigFile = vi.fn();
    ...
    expect(factorySpy.create).not.toHaveBeenCalled();
    expect(await handlers.immediateHandlers[0]?.()).toBe(true);
    expect(setLibraryPanel).toHaveBeenCalledWith(
      expect.objectContaining({ actions: [], kind: "automation" }),
    );
    expect(runAutomationConfig).not.toHaveBeenCalled();
    expect(updateAutomationRun).not.toHaveBeenCalled();
    expect(writeAutomationConfigFile).not.toHaveBeenCalled();
  });
