...
export function commandShellViewUrl(currentHref: string): string {
  const nextUrl = new URL(currentHref);
  nextUrl.hash = "#view-command";
  return `${nextUrl.pathname}${nextUrl.search}${nextUrl.hash}`;
}

function replaceCommandShellViewUrl() {
  ...
  window.history.replaceState(
    null,
    "",
    commandShellViewUrl(window.location.href),
  );
