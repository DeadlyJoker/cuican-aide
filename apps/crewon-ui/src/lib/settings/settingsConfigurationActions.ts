...
import type { PersonalizationSettingsStore } from "./personalizationSettingsStore";
...
export async function refreshPersonalizationSettingsPanelAction(
  ...
  if (params.personalizationStore) {
    setCapabilityPanel(personalizationLoadingPanel(null, locale));
    try {
      setCapabilityPanel(
        personalizationPanelFromSettings({
          locale,
          settings: params.personalizationStore.read(),
        }),
      );
    } catch (error) {
      setCapabilityPanel(
        personalizationErrorPanel({ cwd: null, error, locale }),
      );
    }
    return;
  }
...
export async function refreshKeyboardSettingsPanelAction(
  ...
  if (!client) {
    setCapabilityPanel(keyboardPanel({ configRead: null, cwd: null, locale }));
    return;
  }
