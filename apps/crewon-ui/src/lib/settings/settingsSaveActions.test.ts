...
describe("settings save actions", () => {
  ...
  it("persists personalization through the Control-local authority", async () => {
    ...
    const handlers = createSettingsSaveHandlers({
      ...
      fieldValue: (fieldId) =>
        ({
          "personalization-developer-instructions": "Use evidence.",
          "personalization-instructions": "Act as my product assistant.",
    ...
    expect(saved).toEqual({
      ...
      instructions: "Act as my product assistant.",
      memoryMode: "on",
    });
    expect(refreshes).toBe(1);
  });

  it("does not report a successful config save without a config client", () => {
    let panel: CapabilityPanel | null = { title: "Config", body: "Ready" };
    const handlers = createSettingsSaveHandlers({
      client: null,
      fieldValue: (fieldId) => (fieldId === "config-model" ? "gpt-5" : ""),
      isConnected: true,
      locale: "en",
      persistLocale: () => {},
      persistTheme: () => {},
    ...
    expect(panel).toMatchObject({
      error: "Local configuration service is unavailable",
    });
  });
});
