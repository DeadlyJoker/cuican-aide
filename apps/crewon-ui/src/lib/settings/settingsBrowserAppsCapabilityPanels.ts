...
export function browserAppsPanel(
  ...
  return {
    ...
    body:
      apps.length === 0
        ? locale === "zh"
          ? "还没有可用的网页应用。添加应用后，助理可以在你授权的范围内使用它们。"
          : "No web apps are available yet. Once added, the assistant can use them within your approval settings."
        : undefined,
    rows: apps.slice(0, 24).map((app) => ({
      id: app.id,
      title: app.name,
      ...
      badge:
        app.isEnabled && app.isAccessible
          ? locale === "zh"
            ? "可使用"
            : "Ready"
          : undefined,
