  ...
  return {
    ...
    body:
      inventory.servers.length === 0
        ? locale === "zh"
          ? "还没有添加工具或服务。添加后，助理会在你允许的范围内使用它们。"
          : "No tools or services have been added yet. Once added, the assistant can use them within your approval settings."
        : undefined,
    rows: inventory.servers.slice(0, 24).map((server) => {
      ...
      const title =
        ...
        server.config?.name ||
        server.name;
      const tools = status ? Object.keys(status.tools).length : 0;
      ...
      return {
        ...
        title,
        meta: [
          locale === "zh" ? `${tools} 项能力` : `${tools} capabilities`,
          locale === "zh" ? `${resources} 项内容` : `${resources} resources`,
          status
            ? locale === "zh"
              ? "已连接"
              : "Connected"
        ...
        badge: status ? (locale === "zh" ? "可使用" : "Ready") : undefined,
      };
    }),
    actions: [
      {
