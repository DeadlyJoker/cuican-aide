...
export function personalizationSettingsText(
  ...
  return personalizationSettingsTextFromValues({
    developerInstructions:
      configRead?.config.developer_instructions?.trim() ?? "",
    instructions: configRead?.config.instructions?.trim() ?? "",
    locale,
    memoryMode: personalizationMemoryMode(configRead),
    source:
      locale === "zh"
        ? `配置层: ${configRead?.layers?.length ?? 0}`
        : `Config layers: ${configRead?.layers?.length ?? 0}`,
    summary:
      locale === "zh"
...
function personalizationSettingsTextFromValues(params: {
  ...
  const { developerInstructions, instructions, locale, memoryMode } = params;
...
export function personalizationPanel(params: {
  ...
  return personalizationPanelFromValues({
    body: personalizationSettingsText(configRead, locale),
    developerInstructions: configRead?.config.developer_instructions ?? "",
    instructions: configRead?.config.instructions ?? "",
    locale,
    localRuntime: false,
    memoryMode: personalizationMemoryMode(configRead),
    subtitle: configScopedSubtitle(cwd, locale),
...
export function personalizationPanelFromSettings(params: {
  locale: Locale;
  settings: PersonalizationSettings;
}): CapabilityPanel {
  const { locale, settings } = params;
  return personalizationPanelFromValues({
    body: personalizationSettingsTextFromValues({
      ...settings,
      locale,
      source: locale === "zh" ? "保存位置: 当前设备" : "Saved on: this device",
      summary:
        locale === "zh"
          ? "设置会持久保存在当前设备，并作为有长度上限的偏好上下文应用到助理后续消息。记忆能力不可用时，助理不会假装已经记住。"
          : "Settings persist on this device and are applied to later assistant messages as bounded preference context. The assistant will not claim memory when that capability is unavailable.",
    }),
    ...
    subtitle:
      locale === "zh"
        ? "当前设备 · Control 运行时"
        : "This device · Control runtime",
  });
}

function personalizationPanelFromValues(params: {
  body: string;
  developerInstructions: string;
  instructions: string;
  locale: Locale;
    ...
    fields: [
      {
        ...
        description:
          locale === "zh"
            ? localRuntime
              ? "定义助理是谁、负责什么、以什么视角工作，并作为本机助理偏好应用。"
              : "定义助理是谁、负责什么、以什么视角工作。保存为系统指令。"
            : localRuntime
              ? "Define who the assistant is, what it owns, and its perspective. Applied as a local assistant preference."
      ...
      },
      {
        ...
        description:
          locale === "zh"
            ? localRuntime
              ? "定义价值取向、表达方式、边界和长期行为原则，并作为本机助理偏好应用。"
              : "定义价值取向、表达方式、边界和长期行为原则。保存为开发者指令。"
            : localRuntime
              ? "Define values, voice, boundaries, and durable principles. Applied as a local assistant preference."
      ...
      },
      {
        ...
        description:
          locale === "zh"
            ? localRuntime
              ? "保存生成与使用记忆的偏好；仅在当前运行时提供受治理的记忆能力时执行。"
              : "控制是否从对话形成长期记忆，以及是否在新任务中检索已有记忆。"
            : localRuntime
              ? "Save generation and retrieval preferences; they run only when the current runtime provides governed memory."
