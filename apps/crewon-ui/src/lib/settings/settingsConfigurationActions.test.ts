...
describe("settings configuration actions", () => {
  ...
  it("loads personalization from the Control-local authority", async () => {
    const sink = panelSink();
    ...
    expect(sink.panel).toMatchObject({
      subtitle: "This device · Control runtime",
      fields: [
        expect.objectContaining({ value: "Keep answers concise." }),
        expect.objectContaining({ value: "Use evidence." }),
