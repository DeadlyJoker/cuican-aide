...
const messages: Record<Locale, Messages> = {
  zh: {
    ...
    connectionHints: {
      connected: "对话服务已连接",
      connecting: "正在连接对话服务",
      disconnected: "对话服务已断开，请求暂不发送",
      demo: "演示模式",
    },
    connectionLost: "无法连接对话服务，已保留当前会话，等待重连。",
    crewon: "Crewon",
    demo: "演示",
    demoResponse:
      "当前处于演示模式。启动 crewon app-server 的 WebSocket 监听后，这条请求会发送给真实代理。",
    dismiss: "关闭",
    draftUnsaved: "草稿未发送",
    emptyDescription:
      "连接本地 app-server 后即可浏览项目会话、继续对话，并在 macOS、Windows 和 web 使用同一套体验。",
    ...
    modeCode: "单体对话",
    modeCodeDescription:
      "和一个智能体连续对话，适合审查、修复、测试和运行项目命令。",
    modeOffice: "群聊对话",
    modeOfficeDescription:
      "进入办公室群聊，选择已有智能体协作并沉淀任务、审批和交付物。",
    modeTitle: "切换对话类型",
    ...
    searchThreads: "搜索",
    selectedToolsSummary: (tools) =>
      `已启用：${tools.map((tool) => tool.label).join("、")}`,
    send: "发送",
    sending: "发送中",
    settings: "设置",
    showSidebar: "显示侧栏",
  ...
  en: {
    ...
    connectionHints: {
      connected: "Conversation service connected",
      connecting: "Connecting to conversation service",
      disconnected: "Conversation service disconnected; requests are paused",
      demo: "Demo mode",
    },
    connectionLost:
      "Could not connect to the conversation service. Current sessions are preserved for retry.",
    crewon: "Crewon",
    demo: "Demo",
    demoResponse:
      "Demo mode is active. Start crewon app-server with a WebSocket listener to run this request against the real agent.",
    dismiss: "Dismiss",
    draftUnsaved: "Draft not sent",
    emptyDescription:
      "Connect to local app-server to browse project sessions, continue chats, and use the same workspace across macOS, Windows, and web.",
    emptyThreadDescription:
      "Send a message below and Crewon will continue this session.",
    ...
    modeCodeDescription:
      "Talk with one agent for review, fixes, tests, and project commands.",
    modeOffice: "Group chat",
    modeOfficeDescription:
      "Open an office group chat, choose existing agents, and track tasks, approvals, and artifacts.",
    ...
    selectedToolsSummary: (tools) =>
      `Enabled: ${tools.map((tool) => tool.label).join(", ")}`,
    send: "Send",
    sending: "Sending",
    settings: "Settings",
