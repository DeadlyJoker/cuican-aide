...
import {
  clientOwnsModelResponseTimeout,
...
describe("thread turn timeout", () => {
  it("leaves durable Control run deadlines to the server", () => {
    expect(clientOwnsModelResponseTimeout("control")).toBe(false);
    expect(clientOwnsModelResponseTimeout("legacy")).toBe(true);
  });
