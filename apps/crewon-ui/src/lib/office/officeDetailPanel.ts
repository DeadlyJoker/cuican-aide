...
export function officeRecruitJoinMessage(params: {
  ...
  return {
    ...
    text:
      locale === "zh"
        ? `已从智能体库招募 ${member.name}，模型 ${agent.model}，MCP：${enabledMcp}，Skill：${enabledSkills}。`
        : `Recruited ${member.name} from agents. Model ${agent.model}; MCP: ${enabledMcp}; skills: ${enabledSkills}.`,
  };
}
