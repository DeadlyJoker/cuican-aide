...
describe("office detail panel content", () => {
  ...
  it("builds office recruit copy and prompts", () => {
    ...
    const capabilitySummary = officeRecruitCapabilitySummary(agent);

    expect(capabilitySummary).toEqual({
      enabledMcp: "Git",
      enabledSkills: "Review",
    });
    const bareAgent = agentConfig();
    bareAgent.mcp = bareAgent.mcp.map((option) => ({
      ...option,
      enabled: false,
    }));
    bareAgent.skills = bareAgent.skills.map((option) => ({
      ...option,
      enabled: false,
    }));
    expect(officeRecruitCapabilitySummary(bareAgent)).toEqual({
      enabledMcp: "",
      enabledSkills: "",
    });
    expect(
    ...
    ).toBe(`已从智能体库招募 ${member.name}（模型 ${bareAgent.model}）。`);
