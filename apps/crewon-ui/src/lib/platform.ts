...
/**
 * Whether the Electron preload bridge is actually present, ignoring `?surface=`.
 *
...
export function hasDesktopBridge(): boolean {
  return (
    ...
    window.crewonDesktop?.version === 1
  );
}
...
/**
 * A packaged desktop build loads from a local app file, so
 ...
 * bundle. Electron development also loads over http from Vite, and in that case
