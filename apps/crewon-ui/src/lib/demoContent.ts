...
import type { CapabilityPanel } from "../components/CapabilityDock";
import type {
  AccountStatus,
...
export function demoSettingsPanel(
  ...
  if (section === "hooks") {
    return {
      ...
      subtitle: zh(locale) ? "2 Hook · 0 警告 · 0 错误" : "2 hooks · 0 warnings · 0 errors",
      body: zh(locale)
...
function demoActivityData(locale: Locale): ActivityData {
  ...
  return { trace, approvals, budget, budgetCapUsd: 8, artifacts };
}
