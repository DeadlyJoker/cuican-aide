...
test("creates, replays, lists, and reads a Control Office definition", async (context) => {
  const runtime = await testRuntime(context);
  const create = () =>
    runtime.app.inject({
      method: "POST",
      url: "/api/v1/offices",
      headers: jsonMutationHeaders("office-create-1"),
      payload: {
        expectedRevision: 0,
        title: "Delivery",
        members: [
  ...
  const replayedResponse = await create();
  ...
  const created = createdResponse.json<OfficeMutationResponse>();
  const replayed = replayedResponse.json<OfficeMutationResponse>();
  assert.equal(created.disposition, "created");
  assert.equal(replayed.disposition, "replayed");
  assert.deepEqual(replayed.office, created.office);

  const listedResponse = await runtime.app.inject({
    method: "GET",
    url: "/api/v1/offices?limit=100",
    headers: readHeaders(),
  });
  assert.equal(listedResponse.statusCode, 200, listedResponse.body);
  assert.deepEqual(listedResponse.json<ListOfficesResponse>(), {
    data: [created.office],
    nextCursor: null,
  });

  const readResponse = await runtime.app.inject({
    method: "GET",
    url: `/api/v1/offices/${created.office.officeVersionId}`,
    headers: readHeaders(),
  });
  assert.equal(readResponse.statusCode, 200, readResponse.body);
  assert.deepEqual(readResponse.json<GetOfficeResponse>(), {
  ...
  const threadResponse = await runtime.app.inject({
    method: "POST",
    url: "/api/v1/threads",
    headers: jsonMutationHeaders("office-thread-create"),
    payload: { title: "Office task" },
  });
  ...
  const authorizationResponse = await runtime.app.inject({
    method: "POST",
    url: `/api/v1/offices/${created.office.officeVersionId}:authorize-run`,
    headers: jsonMutationHeaders("office-authorize-1"),
    payload: { targetId: "primary", threadId: thread.threadId },
  });
  assert.equal(
  ...
  assert.deepEqual(authorizationResponse.json<AuthorizeOfficeRunResponse>(), {
    target: { targetId: "primary", agentVersionId: "agent-version-1" },
  });
});
...
test("creates, reads, lists and invokes one redacted scheduled Automation", async (context) => {
  ...
  const createRequest = {
    ...
    schedule: {
      ...
      nextRunAt: "2099-08-29T10:00:00Z",
      intervalSeconds: 0,
      time: "18:00",
      weekday: 6,
  ...
  assert.equal(
    replayResponse.json<AutomationMutationResponse>().disposition,
    "replayed",
  );
  ...
  assert.deepEqual(getResponse.json<GetAutomationResponse>(), {
  ...
  assert.deepEqual(listResponse.json<ListAutomationsResponse>(), {
  ...
  const invocation = runResponse.json<RunAutomationNowResponse>();
  ...
  assert.equal(invocation.run.goalBinding, null);
  assert.equal(runResponse.body.includes("origin"), false);
  assert.equal(runtime.outboxWakeups(), 1);
});

test("background scheduler creates one due Run and restart replays without duplication", async (context) => {
  const runtime = await testRuntime(context);
  const createdThread = await runtime.app.inject({
    method: "POST",
    url: "/api/v1/threads",
    headers: jsonMutationHeaders("scheduled-thread-create"),
    payload: { title: "Background schedule" },
  });
  assert.equal(createdThread.statusCode, 201, createdThread.body);
  const thread = createdThread.json<ThreadMutationResponse>().thread;
  const createdAutomation = await runtime.app.inject({
    method: "POST",
    url: "/api/v1/automations",
    headers: jsonMutationHeaders("scheduled-automation-create"),
    payload: {
      threadId: thread.threadId,
      expectedThreadRevision: thread.revision,
      title: "Due status report",
      prompt: "Produce the scheduled status report.",
      agentVersionId: null,
      schedule: {
        scheduleType: "once",
        nextRunAt: "2026-08-08T00:00:01Z",
        intervalSeconds: 0,
        time: "00:00",
        weekday: 6,
        timezone: "UTC",
  ...
  assert.equal(createdAutomation.statusCode, 201, createdAutomation.body);

  const dependencies = {
    automations: runtime.dependencies.automations,
    store: runtime.store,
    threads: runtime.dependencies.threads,
    now: () => "2026-08-08T00:01:00Z",
  };
  const firstProcess = new AutomationScheduler(dependencies, {
    scanIntervalMs: null,
  });
  await firstProcess.wake();
  await firstProcess.close();
  const firstRuns = await runtime.app.inject({
    method: "GET",
    url: `/api/v1/threads/${thread.threadId}/runs?limit=25`,
    headers: readHeaders(),
  });
  assert.equal(firstRuns.statusCode, 200, firstRuns.body);
  const firstPage = firstRuns.json<ListThreadRunsResponse>();
  assert.equal(firstPage.data.length, 1);

  const restartedProcess = new AutomationScheduler(dependencies, {
    scanIntervalMs: null,
  });
  await restartedProcess.wake();
  ...
  assert.deepEqual(
    afterRestart.json<ListThreadRunsResponse>().data.map(({ runId }) => runId),
    firstPage.data.map(({ runId }) => runId),
  );
});

test("exposes only the safe Provider snapshot and replays one bounded probe", async (context) => {
  const runtime = await testRuntime(context);
  ...
  const snapshot = snapshotResponse.json<GetModelProviderSettingsResponse>();
  ...
  context.after(() => foreignApp.close());
  const unauthorizedSnapshot = await foreignApp.inject({
    method: "GET",
    url: "/api/v1/model-provider-settings",
  });
  assertError(
  ...
  const publicMutationDoesNotExist = await app.inject({
    method: "PUT",
    url: "/api/v1/model-provider-settings",
    headers: mutationHeaders("renderer-must-not-coordinate-provider"),
    payload: {
      operationId: "renderer-owned-operation",
      runtimeBindingId: "renderer-owned-binding",
    },
  });
  assertError(publicMutationDoesNotExist, 404, "notFound", "route_not_found");
...
test("serves health and strict authenticated Run create/get/cancel commands", async (context) => {
  ...
  const threadSnapshotResponse = await runtime.app.inject({
    method: "GET",
    url: "/api/v1/threads/thread-1",
    headers: readHeaders(),
  });
  assert.equal(threadSnapshotResponse.statusCode, 200);
  assert.deepEqual(threadSnapshotResponse.json<GetThreadResponse>(), {
    thread: messageAppendedBody.thread,
    eventSequence: 2,
  });
  assert.equal(
    "eventSequence" in threadSnapshotResponse.json<GetThreadResponse>().thread,
    false,
  );
