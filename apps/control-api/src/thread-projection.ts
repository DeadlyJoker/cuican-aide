...
export function projectThreadSnapshot(state: ThreadState): GetThreadResponse {
  if (
    !Number.isSafeInteger(state.lastEventSequence) ||
    state.lastEventSequence < 0 ||
    state.lastEventSequence !== state.revision
  ) {
    throw new Error("thread_snapshot_event_sequence_invalid");
  }
  return {
    thread: projectThread(state),
