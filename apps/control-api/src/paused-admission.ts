...
export function resolvePausedAdmission(
  ...
  if (securityMode === "production") {
    if (declared) {
      throw new Error("CREWON_CONTROL_PAUSED_ADMISSION_forbidden");
    }
    return null;
  }
...
export type ActivationInputController = Readonly<{
  close(): void;
}>;

/**
 * Reads one exact activation record without waiting for EOF. Any malformed,
 * oversized, or additional input immediately returns the gate to fail-closed.
 */
