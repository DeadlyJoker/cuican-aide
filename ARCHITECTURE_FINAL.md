...
本文是 CrewON 重构完成后的唯一目标架构。`ARCHITECTURE.md` 只提供架构导航；当前实现和验证状态只以
`artifacts/migration/ws1-implementation-status.md` 的带日期状态索引为准。`ARCHITECTURE_NEXT.md`、
`artifacts/architecture-next/poc-*`、worktree queue 和 parity report 只记录迁移历史或支持证据，不再定义终态。
实现、评审和验收发生冲突时，以本文和 Accepted ADR 为准。
...
flowchart LR
    ...
    subgraph Runtime["Runtime Components"]
        ...
        Context["Bounded Context Projector"]
        Workflow["Workflow Runtime"]
        Policy["Policy + Approval"]
        Tools["Tool Broker + MCP Runtime"]
...
packages/
  ...
  office/                     Office and expert-team projections over unified Runs
  memory/                     provenance, eligibility, retrieval and retention
  ...
  config/                     typed non-secret settings and secret references
  legacy-importer/            read-only rollout/config/import tooling
...
- Event、Model History、rollback marker、operation audit 是 append-only facts。Step、Attempt 和 Tool receipt 是
  CAS 驱动的 current-state row，只允许单向状态推进；terminal 后不可回退，状态变化的历史证据由 Event/operation audit 保留。
...
Runtime Worker 从 canonical Model History 投影 bounded context，并将 immutable segment contract 交给
`agent-kernel`。Kernel 只在一个 bounded sampling segment 内拥有模型调用、Tool request 归一化、budget、cancel、checkpoint
和 canonical event 语义；它返回 Tool/approval/handoff boundary 后即释放控制权。Tool Step/Attempt/Receipt、Policy、Approval、
副作用执行与 durable next-step orchestration 归 Worker/Application，而不是 Kernel。`agent-responses` 只负责 Responses-compatible
...
deviceId
executionAuthority:
  runAttempt(runId + stepId + attemptId + workItemId + leaseId + leaseEpoch + expiresAt)
  or durableOperation(operationKind + operationId + leaseId + leaseEpoch + expiresAt)
workspaceBindingId
capability
actionDigest
...
`runAttempt` 只能来自真实的 durable Run/Step/Attempt/WorkItem lease；Thread 级文件浏览、导入、配置检查等非 Run 命令使用
独立 `durableOperation` authority。禁止为了复用 Tool runtime 给非 Run 操作填假的 Run/Step/Attempt ID。两类 command 可以共享
签名、mTLS transport、Gateway receipt 与 Native capability dispatcher，但必须有各自的 Store lease/receipt authority。
...
Provider settings 按 fact 拆分 authority：Control Store 是 active non-secret catalog、revision、pending operation 和
runtime binding 的唯一 authority；OS Keychain/Team Secret Store 只拥有 secret bytes 与 credential availability。桌面 native
metadata 只能是带 catalog revision/digest 的 recovery journal/projection，不能成为第二份 active catalog。Provider 切换通过
`prepare -> supervisor proof -> finalize|abort|expire`，pending 期间必须 fence 新 Run、Goal continuation 和 probe。
...
- Mobile 使用 Authorization Code + PKCE 或平台认可的等价 native flow；token 必须校验 issuer、audience、expiry、device/app
  binding 和 revocation。Bearer mutation 不依赖 browser cookie CSRF，但仍要求幂等键和服务端 Origin/channel policy。
...
- Team 的 tenant/user quota、probe/command rate 和并发 admission 使用共享数据库或受信 edge authority；进程本地
  cache/rate limiter 只能减载，不能成为跨副本 correctness 或计费 authority。
...
- Migration leader、N/N-1 schema 窗口、mTLS/service credential/command-signing key rotation、网络分区与 Gateway takeover
  都必须有演练；每个 capability 有独立 readiness/SLO，不能用进程级 `/health` 冒充能力可用。
...
删除按 `artifacts/migration/module-disposition.md` 与 `artifacts/migration/legacy-app-server-cutover.md` 的 capability family
逐项验收。Agent、Workflow、Office、Human Gate、Automation、Memory、Artifact、Identity/PIM、Resource、Config、MCP、
Skill/Plugin、外部 Agent、terminal/filesystem/search/Git/watch 和 packaging bootstrap 都必须分别达到 import/drain、
zero production call、无 fallback、signed packaged acceptance；Agent loop parity 不能替代外围 family 的删除证据。
