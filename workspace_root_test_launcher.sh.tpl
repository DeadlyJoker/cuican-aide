...
run_selected_libtest() {
  ...
  local skip_filters="${CREWON_BAZEL_TEST_SKIP_FILTERS:-${CODEX_BAZEL_TEST_SKIP_FILTERS:-}}"
...
if [[ -n "${CREWON_BAZEL_TEST_SKIP_FILTERS:-${CODEX_BAZEL_TEST_SKIP_FILTERS:-}}" || ( -n "${TOTAL_SHARDS}" && "${TOTAL_SHARDS}" != "0" ) ]]; then
  run_selected_libtest
fi
