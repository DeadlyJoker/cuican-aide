...
| PostgreSQL real-host       | 通过                           | PostgreSQL 16 Store 基线全量为 `568 pass / 1 inverse skip / 0 fail`；当前结算改动另以串行真实 PG 契约集 `24/24` 验证 cross-process `SIGKILL`、sibling 反序结算、Gate、cancel、reconcile 与 replay。production-main 崩溃纵切实际启动 Control/Release/崩溃前后 Worker，Agent POST=1、retrieve GET=1、Verification POST=1，不是 mock Store。并发执行整个 Store 测试目录会因共享 advisory-lock 测试彼此污染，不作为生产证据；PG 契约门禁必须串行。                                                                                                  |
| Packaged crash recovery    | 通过                           | 2026-08-19 当前 HEAD 重建 unsigned `.app` 后真实运行 Run `01a018d5-4f36-76f9-a034-724984966bd1`。Control client/SSE 证明 Agent→Human Gate→Verification 和唯一 `run.completed`；start 为 `committed -> replayed`，Gate 决策为 `recorded -> replay`，admission receipt=1、canonical Run=1、model samples=2、Attempts=2。Worker 与 GUI `SIGKILL` 后 guardian 都清理完整进程树并释放 3210。产物仅含 GUI、官方 Node 24.18.1（SHA-256 `f480…ff5`）、guardian 和四个 TS bundle，removed-marker scan 为 0。                                             |
