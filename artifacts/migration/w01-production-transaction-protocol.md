...
| PostgreSQL real-host       | 通过                             | 2026-08-14 当前 HEAD 在一次性 PostgreSQL 16 上完成 Store `561 pass / 0 fail`、Runtime Worker `332/332`、Control API `126/126`；Store 唯一 skip 是仅在已配置 URL 时跳过的“缺少 URL”反向占位测试，全部真实 PG 用例均执行。覆盖 Agent→Verification、Tool approval resume、coordinator-only multi-sibling cancellation、multi-reconcile proof、reclaimed Attempt authority、双连接/双 Worker 竞争和子进程 `SIGKILL` 恢复；同时修复 terminal RunStep schema、resume WorkItem strict decoder 与旧 schema fixture 的 FK 清理顺序。   |
