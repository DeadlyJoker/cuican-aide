...
| Terminal convergence    | 通过                         | success、Verification、gate、reconcile 与 cancel 均由 Store 单事务收敛。并行取消使用 node-owned lane 与 dedicated cancellation coordinator：每个 running sibling 只由自己的 WorkItem lease 结算，coordinator 对 foreign lease 返回 retained/retryRequired。SQLite 与真实 PostgreSQL 双 running sibling 均证明最终 canonical Run 为 canceled、无 live Attempt、无 stranded WorkItem；late response 不能覆盖 canceled authority |
| PostgreSQL real-host    | 通过                         | real PostgreSQL 16 串行执行完整 Store 套件：546 pass、0 fail、一个与 Workflow 无关的 Provider 环境标记用例 skip。双 running sibling cancellation、late-response reconciliation、双连接竞争和双进程/SIGKILL lease recovery 均实际执行通过；`PostgresDomainStore` 继续作为 Run、Attempt、dispatch evidence、Workflow composition 与 terminal convergence 的单一进程内 authority |
