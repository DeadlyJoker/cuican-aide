...
| PostgreSQL real-host       | 通过                          | 2026-08-19 在一次性 PostgreSQL 16 上对当前 W01 Store 做完整串行 sweep：`606 total / 605 pass / 1 reverse missing-URL skip / 0 fail`。真实执行覆盖 sibling 反序结算、Gate、cancel、reconcile、retrieved/pending Tool continuation、operator-required deep replay/tamper、downstream-completed receipt replay 与 schema migration；唯一 skip 是“未配置 URL”反向占位测试，没有把条件 skip 冒充 real-host 通过。PG 契约门禁须串行，避免共享 advisory-lock 测试彼此污染。无 PG 环境当前 Store full 为 `385 pass / 65 conditional skip / 0 fail`。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
