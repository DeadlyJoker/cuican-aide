...
| Reconciliation             | 部分通过（仍有 P0）           | 已提交的 assistant/完整 Tool continuation 与未完成 Tool receipt/Attempt 都由 Store 以 `resumeRequired` 原子采用：旧 `responseObserved` dispatch 终结，parent Attempt、DAG node、checkpoint 及 pending Tool authority 绑定当前 reconcile lease；`prepared` 只 execute 一次，`dispatched/unknownOutcome` 只 reconcile，无 receipt 的 crash window 才 fresh prepare。真实 SQLite Worker close/reopen 分别证明 assistant 旧 GET=0、恢复 sample=1，以及 Tool external execute=1、reconcile=1、Attempt 数量=1、唯一 `tool.completed` 后 terminal convergence。PostgreSQL 对等代码、canonical Tool Run lifecycle 与条件矩阵已落，但当前本机无真实 PG URL。仍有两项 P0：新 sample GET 到 nonterminal response 后尚无原子 continuation commit；无 provider locator/idempotency 的 `possiblySent` 只能安全重试而不能自动判定。durability-uncertain 不会误写业务 failed，但本项仍不得标为完全通过。 |
...
| PostgreSQL real-host       | 通过                          | PostgreSQL 16 当前 HEAD 串行 Store 全量为 `570 pass / 1 conditional skip / 0 fail`；其中 cross-process `SIGKILL`、sibling 反序结算、Gate、cancel、reconcile、downstream-completed receipt replay 与 schema migration 均实际执行。production-main 崩溃纵切实际启动 Control/Release/崩溃前后 Worker，Agent POST=1、retrieve GET=1、Verification POST=1，不是 mock Store。PG 契约门禁必须串行，避免共享 advisory-lock 测试彼此污染。                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
