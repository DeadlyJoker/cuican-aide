...
| Reconciliation             | 部分通过（仍有 P0）           | 已提交的 assistant/完整 Tool continuation 现在由 Store 以 `resumeRequired` 原子采用：旧 `responseObserved` dispatch 终结，Attempt、DAG node 和 checkpoint 绑定当前 reconcile lease；同 lease、新 epoch 与下一 sample `prepared/notSent` 均幂等。真实 SQLite Worker close/reopen 证明旧 GET=0、恢复 sample=1、唯一 terminal convergence；PostgreSQL 对等代码与条件矩阵已落，但当前本机无真实 PG URL。仍有三项 P0：未提交 Tool continuation 的 receipt/Tool Attempt 尚未随父 Attempt 接管；新 sample GET 到 nonterminal response 后尚无原子 continuation commit；无 provider locator/idempotency 的 `possiblySent` 只能安全重试而不能自动判定。durability-uncertain 不会再误写业务 failed，但本项仍不得标为完全通过。 |
