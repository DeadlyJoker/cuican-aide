...
| Reconciliation             | 部分通过（仍有 P0）           | `responseObserved` 的 terminal response 可通过 GET-only retrieve 原子结算；`possiblySent`、`notDispatched`、terminal candidate replay 与 downstream-completed receipt replay 均已有 SQLite/PostgreSQL 证据。仍缺已提交非终态 assistant/Tool continuation 的 crash-resume：Store 必须把 Attempt 与 continuation checkpoint 原子采用到 reconcile WorkItem lease，终止已消费的旧 dispatch，再由 Worker 从 bounded history/provider checkpoint 继续而不重复 Tool effect。当前 durability-uncertain 只会进入 unknown/recovery，不再错误写业务 failed；在 SQLite、PostgreSQL 与真实 Worker crash matrix 完成前，本项不得标为通过。 |
