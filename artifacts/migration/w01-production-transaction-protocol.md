...
| Terminal convergence    | 实现中                       | success、Verification、gate、reconcile 与单个 active node 的 cancel 均由 Store 单事务收敛；queued、running/notSent、waitingHuman、unknown/possiblySent 以及 responseObserved late outcome 已覆盖。审计发现两个及以上 sibling 同时 running/unknown 时 SQLite 与 PostgreSQL 仍会返回 `workflow_cancellation_reconciliation_required`，必须补成每节点独立 reconcile authority 后才能恢复为通过                                                                                           |
