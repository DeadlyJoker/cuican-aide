...
| PostgreSQL real-host       | 关键增量通过                  | 既有 PostgreSQL 16 串行 Store、cross-process `SIGKILL`、sibling 反序结算、Gate、cancel、reconcile、downstream-completed receipt replay、schema migration，以及 production Control/Release/Worker 崩溃纵向都曾实际执行。本轮一次性 PostgreSQL 16 full sweep 先得到 `602 pass / 3 fail / 1 reverse skip`，准确暴露两个 fixture 漂移和一个 operator replay authority 构造错误；修复后对应真实 PG focused 为 continuation/retrieved `2/2`、operator first commit/deep replay/tamper `1/1`，均为 0 skip。修复后的无 PG 环境 Store full 为 `383 pass / 65 conditional skip / 0 fail`；没有把条件 skip 冒充 real-host 结果。PG 契约门禁须串行，避免共享 advisory-lock 测试彼此污染。                                                                                                                                                                                                                                                                                                                                                                  |
