...
日期：2026-08-10
状态：In progress
状态索引：Canonical current migration status
asOfSource：2026-08-09 shared dirty working tree；尚无可引用的集成 commit，因此本文件不把未提交工作伪装成 commit 证据

本文件记录当前 source tree 已验证的事实，不把局部测试外推为 WS1 或完整迁移完成。
`parallel-worktree-integration.md`、parity report、PoC 和独立 worktree 结果只作为支持证据；若状态描述冲突，以本文件
更新日期更晚的条目为准。

## 最终不变量的当前未关闭 Gate

- Team Provider public availability 仍缺跨副本 quota/rate authority、tenant egress policy与私有服务身份的 live staging 证明；
  当前 production composition 必须保持 unavailable，而不能把进程内 limiter 当作完成。
- Native workspace 已有 provider-neutral bounded executor、durable operation/revision/delivery-attempt authority、signed command contract、
  stable-handle directory registry、Native capability/epoch dispatcher、Gateway accepted/terminal SQL receipt authority、本地 Gateway
  Workspace service/session、Gateway 间 Workspace peer forwarding，以及独立 Native signed-command/event/ACK journal；journal 与 Native
  dispatcher/WSS 的真实接线、Control/Tauri composition、受监督 Device process、packaged crash/replay，以及
  故障/网络文件系统上单次内核枚举调用的强制 wall-clock 中断尚未完成。
- Workflow/Office/Human Gate、Memory、Resource/PIM、MCP/plugin/skill、Config 与 legacy import 仍有旧 Rust authority 或只完成了
  provider-neutral foundation；在每项 durable command/query/event、客户端和迁移验收完成前，旧 app-server sidecar 仍不能删除。
- Team 最终态仍缺完整 Identity/PIM live staging、跨副本 scheduler/quota/rate、backup/restore、LISTEN/NOTIFY、网络分区/chaos 与
  N/N-1 rolling compatibility 证据；桌面还缺 Windows/NSIS、正式签名/updater、三平台 Native transport/security 和 packaged acceptance。
...
### 2026-08-10 tenant+space sensitive-read fence

- 用户入口读取 Thread、Run 与 Tool Approval 时使用独立 `load*InSpace` Store Port；Automation locator 也强制携带 `spaceId`。
  InMemory 在返回 clone 前复核 scope，SQLite/PostgreSQL 直接把 `tenant_id + space_id + resource_id` 写进查询条件，避免先读取另一
  space 的 Thread title/state JSON、Run snapshot、Tool arguments/approval state 或 Automation prompt 再由 Application 丢弃。
- receipt-first replay 不改：receipt query 仍按 server-scoped idempotency authority 查找，replay 后对冻结结果做 exact authorization；
  只有 fresh/current payload read 使用 scoped locator。Artifact 继续采用 metadata-first：有界 metadata 完成 scope/auth/expiry/blocked
  后才读取正文，因此不需要把正文暴露给 locator probe。
- 证据：Application `93/93`；InMemory/SQLite Store `232 pass + 46 PostgreSQL-unconfigured skip`；真实 PostgreSQL Store
  `380 pass + 1 expected sentinel skip`，0 failure。

### 2026-08-10 Native workspace N3a stable directory handle

- `crewon-device` 新增 trusted `WorkspaceDirectoryRegistry`：raw path 只在注册时使用，listing 只消费 opaque
  `workspaceBindingId + incarnationId` 与已打开目录句柄；registry restart 产生新 incarnation，旧 binding fail closed。
- macOS/Linux 使用 `O_DIRECTORY | O_NOFOLLOW | O_CLOEXEC`、`DIR*`、前后 `fstat` identity 与
  `fstatat(..., AT_SYMLINK_NOFOLLOW)`；Windows 使用 `CreateFileW` directory handle、root/child reparse拒绝、volume serial + file
  index identity 与单条 `NtQueryDirectoryFile`。symlink/reparse/special、非法 UTF-8/UTF-16、控制/bidi名称均拒绝，排序按 UTF-8 raw
  bytes。
- scan 受 200 result、255-byte name、10,000 scanned entries、1 MiB scanned names、64 KiB output、30-second deadline/cancel
  硬限制；exclusive lease 在成功/失败路径归还或关闭句柄。单次已阻塞的内核枚举调用目前不能被 token 强制打断，所以故障/网络文件系统
  的绝对30秒返回仍是下一层受监督 worker/platform cancel Gate，不能把条目间检查冒充完整 wall-clock 保证。
- 证据：`just test -p crewon-device` 15/15；`just fix -p crewon-device`、Bazel lock update/check；macOS实测，Linux/Windows test
  target交叉编译。Windows未在真实主机执行，因此不宣称三平台运行验收。

### 2026-08-10 Native workspace N3b1 signed operation contract

- 新增 additive `crewon.device-workspace-list-command.v0`，以 `commandKind=workspaceList` 和 `operation=listTopLevel` 表示独立
  durable operation；它冻结 Device/execution/delivery lease、workspace/incarnation、device/runtime binding、policy、全部 N1 limits、
  action/command digest、idempotency、trace 与 Ed25519 authorization，且 exact-key parser 拒绝 path、tenant、space、Run、Step、
  Attempt 或 WorkItem 注入。只读 v0 的 approval proof 必须为 `null`。
- 该 contract 与旧 `crewon.device-command.v0` 并存，不改变 Tool/Run wire。TS/Rust 都提供 strict parser、canonical signing payload
  和 Ed25519 verify；JSON Schema 与 shared fixture 冻结同一 golden SHA-256 以及 path/approval/limits reject vectors。
- 证据：Contracts `44/44` + typecheck；`just test -p crewon-device-protocol` 1/1；`just fix -p crewon-device-protocol`；旧
  Device command golden digest未变化。它当前只是 wire authority；durable delivery lease、Gateway/native receipt 和真实 transport 未接时
  capability仍保持 unavailable。

### 2026-08-10 Native workspace N3c capability/epoch dispatcher

- `NativeDeviceConnection` 直接验证独立 `DeviceWorkspaceListCommand`，不转换成 Tool
  `DeviceExecutionCommand`，也不制造 Run/Step/Attempt ID。它复核 connection `deviceId`、当前
  `deviceBindingId + runtimeBindingId`、command/connection lease，并严格命中
  `workspaceBindingId + incarnationId` 的已打开稳定句柄。
- epoch permit 只覆盖稳定句柄的独占 acquisition；可阻塞的 directory scan 在 permit 之外执行，
  完成后再过第二道 epoch fence。因此旧 scan 阻塞时新 connection epoch 仍可 takeover，而旧结果
  会被 `device_connection_epoch_stale` 丢弃，不会返回给上层。
- 输出是 exact `crewon.workspace-list-result.v0`，只含 execution/action/command identity、安全排序 entries 与
  `truncated`，并在序列化后再校验 command `maxOutputBytes`。这仍是瞬时 Native result，不是 durable receipt。
- 证据：`just test -p crewon-device` `20/20`；`just test -p crewon-device-protocol` `1/1`；
  `just fix -p crewon-device`。确定性 barrier test 覆盖实际 production orchestration：scan 阻塞期间高 epoch
  welcome 立即成功，旧 orchestration 放行后在 post-scan fence fail closed。

### 2026-08-10 Native workspace N3b3 Slice 1 event/dispatch contracts

- 新增独立 `crewon.device-workspace-list-event.v0` 与 ACK；accepted 为 sequence 1，terminal 为 sequence 2。
  envelope 同时绑定 `receiptId`、connection epoch、workspace/incarnation、device/runtime binding、action/command digest，
  completed 只能携带 N3c exact `crewon.workspace-list-result.v0`。
- 新 Worker/peer Workspace dispatch envelope 使用独立路径与 schema，不把 Workspace command 塞入旧 Tool
  `crewon.device-dispatch-request.v0`。peer response 必须回显 server-owned route 与 connection epoch，旧 Tool shape 在新 parser
  上 fail closed。
- TS/Rust 共用 valid/reject fixture，拒绝 capability/runtime/digest、receipt/epoch、result identity、UTF-8/raw-byte
  order、entry/output cap 漂移。旧 Tool parser/schema 文件哈希和 fixture 子树另外冻结，本切片未改旧 wire。
- 证据：Contracts typecheck 与 `54/54`；`just test -p crewon-device-protocol` `3/3`；
  `just fix -p crewon-device-protocol`。本切片本身只冻结 wire；后续 N3b2/N3b3/N3d 条目记录 Store、Session 与 journal
  authority，仍不能把这些后续局部证据外推成 packaged HA 可用。

### 2026-08-10 Native workspace N3b2/N3b3 durable delivery authority

- Application/Store 不再把一次同步 dispatch 当作 crash boundary。每个 Workspace operation 具有 append-only result revision、
  immutable phase receipt 与独立 execute/reconcile/cancel delivery attempt；attempt 从 pending 到 exact lease，再以
  `resolution | superseded | leaseExpired | abandoned` 结算。相同 lease/相同结果稳定 replay，变化结果冲突；cancel 与 execute
  允许竞争，但只产生一个 operation revision winner，loser 固定为 `lostRace`。
- SQLite v23 与 PostgreSQL `workspace_operation_authority` v2 将 operation head、immutable revisions、attempts 与 receipt seed
  分表持久化。Store 使用数据库时钟和 `timeout + 5s` lease margin；过期 execute 不能盲目 re-claim，只能显式 reconcile。
  PostgreSQL receipt replay 在等待同 key writer 后建立新的 repeatable-read snapshot，settlement 不获取 receipt 锁；锁序固定为
  receipt -> operation -> attempt，revision/receipt FK、物理 idempotency PK、claim index 与冗余 JSON/列 authority 在启动和读取时
  fail closed。SQLite v21/v22 -> v23 保留 rollback、Provider、Automation 与旧 Workspace receipt。
- Gateway SQLite dispatch schema v2 / PostgreSQL dispatch authority v3 新增全局
  `device_execution_kinds`。Tool 与 Workspace detail 通过 `(execution_id, command_kind)` composite FK 绑定，same-kind orphan、
  cross-kind 双 detail 和损坏 schema 均拒绝。Workspace accepted 事务按 execution kind/detail -> current route 取锁，使用 DB clock
  与当前稳定 connection/epoch fence，并允许同 epoch heartbeat 续租；terminal 只绑定已冻结 route identity/receipt，不回查 mutable
  route 或 expiry。Store commit/replay 返回后上层才有资格发送 ACK authority。
- 证据：Application full `102/102`；InMemory/SQLite/migration `209/209`；真实 PostgreSQL Store full
  `411 pass + 1 expected sentinel skip`；Gateway InMemory/SQLite `18/18`、真实 PostgreSQL SQL authority `8/8`。Application
  在 `notSent` 后返回 durable `prepared`，在 `possiblySent` 或未知 transport 后返回 durable `unknownOutcome`，same-key replay
  不再丢失 execution handle。Gateway 本地 Workspace service/session 与 Native journal 的后续证据见下一条；public capability
  仍保持 unavailable，直到完整进程链、peer route 和 packaged recovery 接通。

### 2026-08-10 Native workspace N3b3 Slice 3b / N3d1 local crash authority

- Gateway 新增独立 Workspace Worker API、Ed25519 verifier 和 Worker runtime-binding authorizer。老 Worker registry 未显式
  `allowedRuntimeBindingIds` 时默认无 Workspace 权限；旧 Tool API/wire不变。Device Hello capability 固定为符合既有 TS/Rust
  grammar 的 `workspace.list_top_level.v0`，不保留 camelCase alias。
- Gateway 使用 server clock prepare，fresh command 严格验签/expiry；accepted/terminal durable replay receipt-first，不重新依赖已过期
  签名。accepted 和 terminal 都必须先提交 SQLite/PostgreSQL authority 再 ACK；terminal 已提交后即使 ACK 发送失败，Worker仍得到
  durable terminal。`fresh | durableReplay` 是显式 tagged mode，accepted durable 可跨原 expiry重发 exact command，并受独立 transport
  deadline约束。重启后的新 Session 即使没有内存 pending，也可从 Store frozen command/route 接收、深验、提交并 ACK Native 重传事件。
- 新独立 `crewon-device-journal` 使用 SQLite WAL、`synchronous=FULL`、foreign keys 与 `BEGIN IMMEDIATE`，在同一事务提交完整 signed
  command + accepted seq1；terminal seq2 和 cumulative ACK 分别原子提交。读取重新走 protocol parser并核对 canonical JSON、SHA-256、
  冗余身份列、receipt唯一性、时间单调链和 schema/integrity。GET/list使用单一 read transaction；未 ACK 恢复页严格限制为100项。
- Native journal orchestrator将fresh签名、当前runtime/epoch/expiry和workspace incarnation metadata admission置于accepted commit之前，但
  stable-handle acquire与scan严格在journal writer事务之外；因此同workspace并发scan不会形成handle→journal与journal→handle锁环。
  accepted commit后先通过infallible event-only observer暴露seq1，再acquire/scan；terminal必须journal commit后才返回。进程内execution
  reservation在accepted可见前建立，独立orchestrator不会把仍在运行的accepted误判为crash；真正accepted-only重启不重开path或盲目scan，
  而是写deterministic unknownOutcome。terminal replay零scan，reconnect只投影未ACK events，不泄露signed command。
- Gateway Team router 先在本地 shared Store做 receipt-first replay；只有非终态且本机不是当前 owner才读取共享 route并单跳转发。
  peer API同时验证Gateway mTLS身份、payload sourceGateway、完整route fence和目标registry中的sourceWorker credential/runtime binding；
  peer断言身份标记为`gatewayAssertion`，不伪报目标端直接验证过Worker mTLS。client强制TLS1.3、CA/hostname与目标证书SHA-256 pin、
  总deadline/body caps及`notSent | possiblySent`边界。owner SIGKILL后新owner只恢复同一 accepted receipt，reconcile/cancel不重复listing。
- 证据：Gateway Node 24 + 真实 PostgreSQL full `101/101`，本地 Workspace deterministic/real WSS、双Gateway mTLS/PG与Tool/Workspace
  process failover均全绿；
  `just test -p crewon-device-journal` `13/13`、`just test -p crewon-device` `26/26`，随后 scoped `just fix`、Bazel lock
  update/check通过。

### 2026-08-10 Native workspace N3d3 Device Runtime / N4 private and public boundaries

- 新独立 `crewon-device-runtime` binary 只从512 KiB有界、deny-unknown的stdin bootstrap取得TLS、journal和trusted workspace
  authority；bootstrap采用单行delimiter，不等待Supervisor保留的stdin句柄EOF，原始stdin与TLS PEM使用zeroizing storage，Debug不暴露
  path/secret。它只连接固定`wss://.../device/v1`，强制TLS1.3
  mTLS hostname/CA并可叠加leaf pin，不读取proxy环境变量。Hello只投影正数ACK head；Welcome严格绑定本次connection/device/epoch。
- Device reader/writer与最多4个blocking scan分离，ACK/cancel不被目录枚举阻塞。进程先订阅durable journal event，再读取replay snapshot，
  并在启动live consumer前先enqueue完整replay，因而旧generation的late terminal既不会永久漏发，也不会以seq2抢在seq1之前。
  active cancel严格绑定execution/lease/epoch；accepted-only或terminal的exact late/duplicate cancel安全no-op，不打掉健康WSS。
  Welcome/epoch和dispatch scheduler均建立后，binary输出只含device/runtime/epoch的非秘密exact readiness line，供Tauri在启动Worker前验证。
- Runtime Worker新增PC-only loopback private Workspace server和严格Control client：freeze从同一Store/active runtime冻结server-owned binding，execute
  发送前二次admit exact frozen authority并重验delivery lease；reconcile/cancel不重读mutable Thread route，也不重新签名。Worker→Gateway使用
  Ed25519 command signer与TLS1.3 mTLS；Gateway返回后的任何损坏都保守归类`possiblySent`。Team明文private入口不存在，production resolver
  未配置时保持unavailable。
- Public contract已冻结create/list/get/reconcile/cancel与execution-scoped SSE：mutation强制CSRF+Idempotency，公开projection只含
  thread/execution/revision/status及bounded result，`eventSequence == operation.revision`。Application只读链为validation→coarse
  `thread:workspace:read`→Thread存在性隐藏→exact auth→Store；写链使用独立`thread:workspace:write`，读权限不能触发Device副作用。
  InMemory/SQLite/PostgreSQL list均以scope filter、raw UTF-8 executionId order和`limit+1`证明next cursor；snapshot和revision journal保留单一
  durable authority。
- Public Control已接create/list/get/reconcile/cancel和execution SSE；create区分201 committed/200 replayed，action带caller
  `expectedOperationRevision`且receipt replay先于current revision fence。SSE在hijack前验证missing/ahead/malformed，terminal后关闭，
  reconnect仍从durable revision journal恢复。typed client使用独立Last-Event-ID、128 KiB frame/256 KiB buffer硬限制和same-key
  unknown-outcome retry；Control-only UI runtime使用Thread/authority generation abort、完整分页后才启动非终态stream，409只rehydrate
  不自动覆盖。可见compact panel已按现有设计语言提供中英copy、desktop/readOnly/unavailable显式authority、unknownOutcome恢复与
  bounded顶层结果快照；它尚未接App/Tauri authority，因此不能据此宣称renderer入口完成。
- Packaged Worker sidecar现从512 KiB有界、成功/失败均清零的stdin读取兼容v1与Workspace v2 bootstrap。真实SQLite Thread
  freeze经port0 loopback private server和TLS1.3 mTLS Gateway完成；ready只打印非秘密origin/runtime binding。无Workspace配置时不监听，
  deployment mismatch、私服/composition失败会关闭Gateway/Store/Worker资源。
- 证据：Device Runtime最终pre-fix `12/12`，Runtime/Journal/Device三crate合并`51/51`，随后三crate scoped `just fix` clean；
  Bazel lock update/check通过（Bazel test仅被Apple CLTools SDK下载403阻断）。Runtime Worker `154 pass + 1 PG-unconfigured skip`；
  Application `115/115`；Store `261 pass + 46 PG-unconfigured skip`，真实PostgreSQL Workspace read focused `32/32`；Contracts
  `69/69`。Control public full `81 pass + 3 PG-unconfigured skip`、真实PostgreSQL Control e2e `7/7`；Control client `62/62`；
  UI Control runtime focused `66/66`、panel focused `6/6`并通过production build；packaged Worker full `166 pass + 1 PG-unconfigured
  skip`。staging现会构建target-specific `crewon-device-runtime` externalBin、bundle `device-gateway.mjs`，用attested staged Node 24
  对五个bundle逐一`--check`，并真实启动native空stdin验证确定性fail-closed；staging tests `8/8`。当前仍缺App/Tauri native
  Workspace authority、Tauri对Gateway/Device Runtime的监督与三平台实包验收；“资源已入包”不等于进程纵向已可用。
- Tauri既有Provider reload的进程/事务窗口已先行关闭：`BEGIN IMMEDIATE`在旧Control/Worker两个exact generation收到真实
  `Terminated`前不提交；channel/monitor error不能伪ack，kill timeout会由Supervisor继续持有SQLite fence直到late exact ack。
  Provider candidate现在依次证明Worker ready、Control ready并预建monitor，之后才finalize和install lifecycle；finalize前任一失败会
  abort并恢复旧release/runtime，finalize后不可补偿的CAS/poison保持journal并全停。Tauri full `40/40`、clippy all-targets通过。
- Tauri native Workspace authority已落为私有两阶段catalog：安装级device identity稳定；同canonical path/App重启复用
  workspace/incarnation/runtime identity，换path三者全部旋转；clear使用catalog级单调revision并撤销current trusted path。
  pending重启时拒绝启动material，必须显式commit/abort。Unix使用`O_NOFOLLOW`+fd mode/dev/inode/version，Windows使用reparse
  handle identity和WRITE_THROUGH atomic replace。App-lifetime ephemeral CA、Gateway/Worker/Device证书与Ed25519 command key均由native
  生成，private PEM zeroize/Debug redacted；Gateway先bind port0，收到validated ready port后才生成Device/Worker payload，Worker private
  server同样使用port0。Tauri full随该slice增至`43/43`，Windows file-authority最小MSVC夹具通过；renderer command和Supervisor接线尚未开放。
- Workspace UI 的操作权限不再接受外部伪造的“active thread”或cwd/path信号：`ControlWorkspaceState.threadStatus`只来自同一次
  Control Thread snapshot（含409 rehydrate），切Thread/authority generation会清空或隔离迟到状态；操作面板仅在Control thread为active、native
  workspace确实已选择且desktop mutation authority可用时开放创建。archived/deleted、Team read-only、native unavailable均fail closed。该安全收紧的
  control-runtime focused `67/67`、panel focused `19/19`、UI lint/build均通过。随后App composition已接通：Control未配置时不创建slot或
  native port、legacy DOM保持不变；配置但断线时fail closed，Web/Team只读，desktop仅在native status available时可写。native commit后依次
  恢复Thread/Run SSE、Goal snapshot/SSE与Workspace list/SSE；conflict、active authority、unknown outcome与rehydration failure只显示安全文案，
  raw error/path/binding不会进DOM。create固定`maxEntries=200`，adapter/hook/App/panel/Command focused `91/91`、UI lint/build通过。
  旧sidebar路径roster/folder picker与旧Files动作仍需单独切断，Tauri selector command未完成前native mutation继续稳定显示unavailable。
- Control configured UI现已切断旧本地路径authority：sidebar不再读写workspace roster localStorage、不探测JS folder picker、不渲染绝对路径
  输入/分组/切换/移除；Control Thread统一进入脱敏的“任务/Tasks”组，新任务固定`cwd:null`。composer legacy workspace dropdown隐藏，Files、
  Review、Attach Context及stale path capability item在任何AppServer filesystem调用前被安全拦截，keyboard/new-thread/terminal cwd也不继承旧cwd。
  Control未配置的legacy DOM、roster和picker保持不变。focused `62/62`、UI lint/build通过；UI全量为`1792 pass + 1 snapshot fail`，
  剩余shared Workbench CSS snapshot正独立审阅，未在本slice盲目接受。
- packaged desktop 的已有 Workspace 现在走真实四进程顺序：Gateway先以loopback port0启动并清除generation-private临时TLS/registry文件，
  Device收到严格Welcome/epoch后ready，Worker用Workspace v2 stdin并回报exact private runtime-binding/origin，最后Control拿该私有route启动；无已选
  Workspace时保持原Worker v1→Control。四个child都纳入同generation生命周期，Gateway/Device的Terminated或monitor error会触发全停。
  Provider reload只推进Worker/Control generation，复用同一WorkspaceRuntimeContext、证书/command material与存活Gateway/Device，并重新等待
  Workspace private ready；readiness失败会先kill尚未托管的candidate Worker，不能留下orphan。当前证据为Tauri full `49/49`、clippy
  `-D warnings`、Control full `84 pass + 3 conditional skips`、Gateway full `90 pass + 6 conditional skips`以及Runtime Worker真实SQLite+mTLS `2/2`。
  这只证明“启动已有native authority”；目录选择/clear、全四进程切换事务和renderer命令仍在下一slice，不能提前标最终可用。
- Workspace切换所需的Control candidate admission fence已独立落地：仅standalone显式`CREWON_CONTROL_PAUSED_ADMISSION=1`可启用，
  production只要看到该变量即在打开authority前拒绝。candidate可listen并只开放exact health GET/HEAD；其余GET/POST/SSE在body、identity、
  authorization和Store handler前统一503。父进程写入单个bounded `activate\n`（不等EOF）后才放行，并收到exact安全确认行；malformed、oversize、
  额外记录会fail closed。Control full `88 pass + 3 conditional skips`，paused real-process focused `5/5`。该substrate尚需由Tauri在catalog
  commit后接线，单独存在不等于Workspace switch已完成。

### 2026-08-10 Native workspace N1/N2 bounded authority

- provider-neutral workspace-list command 不接受 raw path，只接受 server-owned workspace/incarnation、device/device-binding/runtime
  binding、policy、actor/thread revision与硬限制。Application 自己重算 canonical `actionDigest` 和完整 dispatch `commandDigest`；
  N1/N2 使用同一 golden vector，factory/adapter不能伪造另一套 digest。
- executor 使用 trusted stable-directory capability 的 bounded page/cursor；entry result 200、单名255 UTF-8 bytes、扫描10,000项、
  扫描名称1 MiB、输出64 KiB、30秒deadline/cancel。它不读取第N+1项，拒绝invalid UTF-8/lone surrogate、symlink、reparse、
  special/control name和非raw-byte顺序；取消后迟到handle exactly-once release，cleanup错误有明确fail/report语义。
- durable authority提供 execute/reconcile/cancel三阶段 immutable receipt 与独立 delivery attempt；顺序为 validation -> coarse auth ->
  receipt-first -> replay exact auth。fresh prepare在同一事务冻结Thread revision、operation、revision seed、execute attempt和receipt；
  dispatch丢失保留prepared，只能显式reconcile。terminal CAS单向，相同result replay、不同terminal payload conflict，cancel/execute
  竞争只允许一个revision winner。
- InMemory、SQLite v23与PostgreSQL独立`workspace_operation_authority v2`均深验receipt/thread/execution/action/command/fingerprint、
  attempt/result revision链和冗余列。SQLite v21/v22->v23保留rollback invalidation/receipt、Automation和Provider authority；PG使用
  64-bit receipt advisory、fresh replay snapshot及operation-first settlement锁序，并深验component DDL/PK/FK/UNIQUE/CHECK/index。
- 证据：Application `102/102`、Device Dispatch `37/37`、InMemory/SQLite/migration `209/209`、真实 PostgreSQL Store full
  `411 pass + 1 expected sentinel skip`。当前仍没有public route/renderer fallback；Gateway本地session与Native journal authority虽已落，
  但它们尚未组成受监督、可发布的进程链，因此 capability保持unavailable。

### 2026-08-10 Thread snapshot -> SSE cursor handoff

- `GET /threads/{threadId}` 从同一个 canonical ThreadState read point 返回 exact `{thread,eventSequence}`；cursor 使用
  `lastEventSequence`，不塞进公开 `ThreadView`。Control client 与 UI 对 envelope、Thread identity、revision/cursor mismatch
  fail closed。
- `ControlThreadRuntime` 在投影 messages/runs 前先冻结 Thread snapshot cursor，Thread SSE 从该 sequence/`Last-Event-ID` 开始；
  per-thread generation + AbortController 防止切 Thread 后迟到 snapshot/event 覆盖当前状态。
- 证据：Contracts `39/39` + generate-check；Control client `41/41`；Control API `63 pass + 3 PostgreSQL process skip`；
  UI focused `30/30`、lint、production build。

### 2026-08-10 Provider authority、桌面切换与公开探针

- Provider 非秘密 catalog 使用 SQLite v20 / PostgreSQL 独立 component v1 的 two-phase
  `prepare -> finalize | abort | expire` authority；operation、actor audit、TTL、terminal receipt、历史回放和 current-head
  authority 均持久化。pending 期间 Run、TurnStart、Goal activation/continuation 和 probe admission fail closed。
- PC Tauri coordinator 在同一 native admission fence 内停止 Control/Worker、证明 candidate Worker 的 exact
  `runtimeBindingId` ready、finalize 后复用原 renderer session/CSRF 重启 Control；candidate 失败先恢复旧 runtime 再 abort，
  补偿失败关闭 supervisor。Provider secret 与 private probe token 通过 bounded private bootstrap 进入 Worker，不进入 renderer、
  Vite、URL、日志或 catalog。
- Public Control 只开放 authorized non-secret GET snapshot 与 CSRF/idempotency protected active-provider probe；Worker 在真实
  `/models` 请求前后复核 tenant/provider/revision/runtime binding，使用独立 pinned socket、DNS/egress检查、HTTPS SNI/CA、
  64 KiB body、100 models、deadline/cancel 和 secret lease release。Team 缺 tenant route/egress/private identity 时保持 unavailable。
- UI configured path 不调用 legacy AppServer config/probe；桌面 mutation 只走 native coordinator，Team/Web 只读且不显示假按钮。
  API key 在第一个 async gap 前从 panel state 清除，error/retry path 不恢复 secret。独立 P0/P1 审计未发现公开投影、绑定复核、
  admission 或 secret 边界缺口。
- 证据：Application full `81/81`；Provider InMemory/SQLite `22/22`；真实 PostgreSQL focused `13 pass + 1 sentinel skip`，
  Store full `329 pass + 1 sentinel skip`；Runtime Worker `136 pass + 1 PostgreSQL-unconfigured skip`；Control `60 pass + 3
PostgreSQL process skip`；Tauri `19/19` + clippy；UI focused `30/30`、lint、production build。正式签名/updater、Windows/NSIS、
  Team mTLS/global quota/tenant egress和 live Provider canary 仍是发布 Gate。

### 2026-08-10 manualOnly Automation Control cutover

- Automation 定义与 invocation 使用 canonical `automation_invocation` origin，冻结 definition/instruction/route digest、invocation、
  Run 和 AgentVersion identity；Message、Model History、Run、WorkItem、Outbox 和 receipt 在一次 compound transaction 提交。
  InMemory、SQLite v21 和 PostgreSQL `automation_authority` v1 已通过 rollback/fork/compaction、active Run、Work fence、receipt replay
  和真实 PG lock/snapshot race conformance。
- Public Control 新增 create/get/list/run-now；mutation 强制 CSRF、Idempotency-Key 和 Thread/Automation revision CAS。公开
  `AutomationView` 固定 `executionMode=manualOnly`、`automaticScheduling=false`，不包含 tenant/space/actor、schedule、digest、
  route 或 invocation binding。`runNow` 只接受 Store 返回的 canonical origin Run，不从 Thread 的普通 Run 列表推断历史。
- typed client 与 `ControlAutomationRuntime` 对 unknown outcome 只用同 key 重试一次，409 重新读取 Thread/Automation；accepted Run
  交给现有 Thread runtime 从独立 Run cursor 接 SSE。UI 复用“日程安排”入口但只显示手动定义/立即运行，不显示频率、下一次执行、
  enabled 或其他未实现调度语义；Control 不可用时不回退 legacy AppServer。
- independent P0/P1 hardening 把 malformed 2xx validation 放回 same-key receipt retry 内，公开 run-now 只增加 redacted
  `invocation:{automationId,runId}`；RunView、listThreadRuns 与 Run SSE 对 expected Thread/Run、完整 status/terminal invariant 和每种 event
  discriminant 深验。列表覆盖 1000/1001 与重复 cursor。Automation page 用 runtime/session + connected + workspace authority epoch、
  AbortSignal 和 generation fence 丢弃迟到 list/create/run；Control cohort 在 collection/item/dispatch/action factory/旧 notification
  polling 五层禁止 legacy Automation 读写或失败回退。
- 证据：Contracts `39/39` + generate-check、Control API `63 pass + 3 PostgreSQL process skip`、Control client `47/47`、Automation
  runtime focused `35/35`、UI authority focused `36/36`、lint 与 production build。UI full `1726/1727` 的唯一失败是共享树既有
  workbench tab CSS snapshot 漂移，
  未被本切片越界接受；in-app Browser 当前没有可用实例，因此没有把静态 snapshot 冒充真实浏览器验收。

### 2026-08-10 Artifact metadata-first authorization

- `readArtifact` 先读取有界 metadata，完成 tenant/space、`artifact:read`、expiry 和 blocked 检查后才读取正文；denied、cross-space、
  expired、blocked 路径以 spy 证明正文 Store 调用为 0。授权后的正文还必须与 metadata canonical deep-equal，并匹配 UTF-8
  byteLength 和 SHA-256 digest；null、authority drift 或 corruption fail closed。Application full `86/86`、Artifact focused `8/8`。
