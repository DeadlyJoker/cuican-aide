...
- W01 production transaction protocol、Workflow Start、scheduler fan-out、node admission、node settlement、Human Gate、
  ...
  结算 sibling，最终也没有 stranded WorkItem。2026-08-14 当前 HEAD 在一次性 PostgreSQL 16 上完成 Store
  `561 pass / 0 fail`、Runtime Worker `332/332`、Control API `126/126`；Store 唯一 skip 是仅在已配置 URL 时跳过的
  “缺少 URL”反向占位测试，全部真实 PG 用例均执行。双 running sibling、late-response reconciliation、双 Worker 竞争和
  子进程 `SIGKILL` 恢复均实际通过。这不代表整个产品迁移完成。
...
- 上述验收暴露并修复 reclaimed WorkItem 与旧 Attempt epoch 的 fence：只有当前 lease 已验证、WorkItem ID 相同且 Attempt epoch
  ...
  SQLite 有默认回归；PostgreSQL 16 的 coordinator-only cancellation、multi-reconcile、reclaimed prepared/responseObserved
  Attempt、Tool approval resume lease adoption 与 execution schema migration 已在本轮真实执行。实跑同时修复了取消时 terminal
  RunStep 缺少 canonical schema、approval resume WorkItem 未进入 PG strict decoder，以及旧 execution schema fixture 未按 FK
  反向依赖顺序清理 handoff 表的问题。
