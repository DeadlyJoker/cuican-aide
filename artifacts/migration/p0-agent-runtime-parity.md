...
| AR-042 | `crewon-api/src/sse/responses.rs::process_responses_event` + TS transport/Kernel/Runtime Worker                   | reasoning summary 有序、bounded 投影；raw reasoning 仅 transient，任何 partial 均不进入 canonical model history | PARITY       | shared reasoning fixture、Direct transport 与 durable Worker 证据；累计预算、first-terminal、retry discard 与 HTTP/WS sequence policy |
...
使用 provider-neutral `reasoning.delta { channel, index, delta }` / `reasoning.part.added`，单个 delta 上限 16 KiB、单 response reasoning
累计上限 64 KiB；超限、负/非整数 index、malformed payload 均 fail closed。HTTP/SSE 必须有严格递增 sequence，WebSocket 的
...
reasoning 已失效；取消/error 沿用同一 abort/terminal 路径，不会提交 assistant history。真实 Direct transport 与 durable Worker 测试验证
summary 顺序可观测且 raw content 不落盘，最终 history 只包含 authoritative completed assistant output。
