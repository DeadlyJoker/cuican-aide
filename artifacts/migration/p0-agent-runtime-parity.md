...
| AR-019 | `rmcp_client.rs::stdio_mcp_parallel_tool_calls_opt_in_runs_concurrently`                                          | server opt-in 并发策略                                                                                | REDESIGN     | mutation receipt/reconcile 与 TLS 1.3 remote transport Gate 已落地；真实 packaged keyring endpoint/chaos 仍未验收                 |
...
| AR-043 | `crewon-api` Responses HTTP/WebSocket turn state + TS Direct/WebSocket transports                                 | opaque provider turn state 只属于一个 Run，不进入模型历史；重复、冲突、畸形或超过 4 KiB 时 fail closed | PARITY       | shared grammar；HTTP/WS 在流事件前 durable observe；Run/Attempt 原子状态、restart replay、sample/Tool boundary 与 terminal release     |
...
terminal。三个 Rust 已知 reasoning kind 由 AR-042 独立冻结，不能落入未来 kind ignore。`response.output_item.added` 与
`response.custom_tool_call_input.delta` 是由对应 done 事件提供完整 authority 的标准中间 framing，继续在 require-created 后有意 no-op。
...
TS focused regression 另覆盖 `whenPresent` 缺 sequence、`required` 缺 sequence 拒绝、两种
authoritative-done 中间 framing no-op、已知 malformed delta 与 terminal 后 unknown 拒绝。

### AR-042 Responses reasoning projection

`responses-reasoning.reference.json` 冻结 Rust 的 summary text delta、raw reasoning text delta 与 summary part-added 顺序。TS transport
使用 provider-neutral `reasoning.delta { channel, index, delta }` / `reasoning.part.added`。Rust shared fixture 只证明合法 reasoning 事件的
类型与 section/index 顺序投影；16 KiB 单 delta、64 KiB 单 response 累计限制，以及 malformed/index reject 是 TS trust-boundary hardening，
不宣称 Rust 具有同等 reject/bounds。HTTP/SSE 必须有严格递增 sequence，WebSocket 的
`whenPresent` 可省略但一旦出现仍严格校验；首个 terminal 后所有 reasoning 都由统一 cutoff 拒绝。

Kernel 只把 summary delta 与 section break 投影为既有 canonical `model.reasoning.summary`，并定义 bounded `kind`、`summaryIndex`、
`delta` data。provider raw reasoning content 只在 transport/Kernel 边界做受限 transient 校验，不成为 durable canonical event，也不进入
`completedHistoryItems`、下一次 sampling request 或 canonical model history。stream retry 的 `discardedOutput` 同时标记已观察到的 partial
reasoning 已失效；取消/error 沿用同一 abort/terminal 路径，不会提交 assistant history。真实 Direct transport、WebSocket→HTTP loopback
与 durable Worker 测试验证 summary 顺序可观测、reasoning-only WS partial 在 fallback 时标记 discarded、raw content 不落盘，最终 history
只包含 authoritative completed assistant output。
