...
terminal。AR-041 不宣称 reasoning parity：Rust 已处理而 TS 尚无等价 `ModelTransportEvent` 的
`response.reasoning_summary_text.delta`、`response.reasoning_text.delta` 与 `response.reasoning_summary_part.added` 均显式保持
`responses_event_unsupported` fail closed，不能落入未来 kind ignore。`response.output_item.added` 与
...
TS focused regression 另覆盖 `whenPresent` 缺 sequence、`required` 缺 sequence 拒绝、三种 reasoning kind fail closed、两种
