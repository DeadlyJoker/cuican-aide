...
`responses-generic-terminal.reference.json` 的六个 poisoned stream（包括整个 `event.response` 缺失）由 Rust `collect_events`、TS `ResponsesProtocolDecoder` 和 Runtime Worker
