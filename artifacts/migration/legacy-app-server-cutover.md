...
| Office definitions and explicit Workflow delegation          | Control API + canonical Domain Store + canonical Workflow Run              | Office room selects an immutable WorkflowVersion, Thread and bounded JSON input                      | receipt-first atomic provenance, member coverage, scoped pagination and no Office-specific scheduler/runtime                         |
...
| Office chat, expert aliases and automatic orchestration      | explicit Workflow delegation is available; auto-dispatch, chat and memory handoff remain unavailable          | separate bounded product contracts; no legacy compatibility layer                                     |
